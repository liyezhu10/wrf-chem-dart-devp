! Added January 2015 by J. Poterjoy
!
! Spline interpolation is required for the KDDM step of the localized particle filter
...
implicit none
private
public spline_cubic_set, spline_cubic_val
...
!    and increasing.
!
!    Input, real(r8) TVAL, a point at which the B spline basis
!    function is to be evaluated.
!
!    Output, real(r8) YVAL, the value of the function at TVAL.
!
...
!  Parameters:
!
!    Input, real(r8) BETA1, the skew or bias parameter.
!    BETA1 = 1 for no skew or bias.
!
!    Input, real(r8) BETA2, the tension parameter.
!    BETA2 = 0 for no tension.
!
!    Input, real(r8) TDATA(5), the nodes associated with the
!    basis function.  The entries of TDATA are assumed to be distinct
!    and increasing.
!
!    Input, real(r8) TVAL, a point at which the B spline
...
!  Parameters:
!
!    Output, real(r8) MBASIS(4,4), the basis matrix.
!
...
!    ALPHA = || P2 - P1 || / ( || P3 - P2 || + || P2 - P1 || )
!
!    Output, real(r8) MBASIS(3,3), the basis matrix.
!
...
!
!    Input, real(r8) MBASIS(N,N), the basis matrix.
!
!    Input, integer ( kind = 4 ) NDATA, the dimension of the vectors TDATA
!    and YDATA.
!
!    Input, real(r8) TDATA(NDATA), the abscissa values.  This routine
!    assumes that the TDATA values are uniformly spaced, with an
!    increment of 1.0.
!
!    Input, real(r8) YDATA(NDATA), the data values to be
!    interpolated or approximated.
!
!    Input, real(r8) TVAL, the value of T at which the spline is to be
!    evaluated.
!
!    Output, real(r8) YVAL, the value of the spline at TVAL.
!
...
!
!    Input, real(r8) T, the point at which the Bezier curve should
!    be evaluated.  The best results are obtained within the interval
!    [0,1] but T may be anywhere.
!
!    Input, real(r8) XCON(0:N), YCON(0:N), the X and Y coordinates
...
!
!    Output, real(r8) XVAL, YVAL, the X and Y coordinates of the point
...
!    N must be at least 0.
!
!    Input, real(r8) X, the point at which the Bezier function should
!    be evaluated.  The best results are obtained within the interval
!    [A,B] but X may be anywhere.
!
!    Input, real(r8) A, B, the interval over which the Bezier function
...
!
!    Input, real(r8) Y(0:N), a set of data defining the Y coordinates
!    of the control points.
!
!    Output, real(r8) BEZ_VAL, the value of the Bezier function at X.
!
...
!
!    Input, real(r8) X, the evaluation point.
!
!    Output, real(r8) BERN(0:N), the values of the N+1 Bernstein basis
!    polynomials at X.
!
...
!    N must be at least 0.
!
!    Input, real(r8) A, B, the endpoints of the interval on which the
!    polynomials are to be based.  A and B should not be equal.
!    Input, real(r8) X, the point at which the polynomials are to be
...
!
!    Input, real(r8) YDATA(0:N), the data values at N+1 equally
!    spaced points in [A,B].  If N = 0, then the evaluation point should
!    be 0.5 * ( A + B).  Otherwise, evaluation point I should be
!    ( (N-I)*A + I*B ) / N ).
!
!    Input, real(r8) XVAL, the point at which the Bernstein polynomial
!    approximant is to be evaluated.  XVAL does not have to lie in the
!    interval [A,B].
!
!    Output, real(r8) YVAL, the value of the Bernstein polynomial
...
!  Parameters:
!
!    Input, real(r8) X1, X2, the endpoints of the interval of
!    definition of the cubic.  X1 and X2 must be distinct.
!
!    Input, real(r8) F1, F2, the values of the function at X1 and
!    X2, respectively.
!
!    Input, real(r8) D1, D2, the derivative values at X1 and
!    X2, respectively.
!
!    Input, integer ( kind = 4 ) NE, the number of evaluation points.
!
!    Input, real(r8) XE(NE), the points at which the function is to
!    be evaluated.  If any of the XE are outside the interval
!    [X1,X2], a warning error is returned in NEXT.
!
!    Output, real(r8) FE(NE), the value of the cubic function
!    at the points XE.
!
...
!
!    Input, real(r8) XTAB(NTAB), the X values at which data was taken.
!    These values must be distinct.
!
!    Input, real(r8) YTAB(NTAB), the corresponding Y values.
!
!    Output, real(r8) DIFTAB(NTAB), the divided difference coefficients
...
!
!    Input, real(r8) XTAB(NTAB), the X values upon which the
!    divided difference polynomial is based.
!
!    Input, real(r8) DIFTAB(NTAB), the divided difference
!    polynomial coefficients.
!
!    Input, real(r8) XVAL, the value where the polynomial
!    is to be evaluated.
!
!    Output, real(r8) YVAL, the value of the polynomial at XVAL.
!
...
!
!    Input, real(r8) XTAB(NTAB), the X data.  The values in XTAB
!    should be distinct, and in increasing order.
!
!    Input, real(r8) YTAB(NTAB), the Y data values corresponding
...
!    equal to NTAB-1.
!
!    Output, real(r8) PTAB(NTAB), the value of the least
!    squares polynomial at the points XTAB(1:NTAB).
!
!    Output, real(r8) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
!    needed to evaluate the polynomial.
!
!    Output, real(r8) EPS, the root-mean-square discrepancy of the
!    polynomial fit.
!
...
!  Parameters:
!
!    Input, real(r8) X, the point at which the polynomial is
!    to be evaluated.
!
!    Input, integer ( kind = 4 ) NDEG, the degree of the least squares
!    polynomial.
!
!    Input, real(r8) B(1:NDEG), C(0:NDEG), D(2:NDEG), arrays
!    defined by LEAST_SET_OLD, and needed to evaluate the polynomial.
!
!    Output, real(r8) VALUE, the value of the polynomial at X.
!
...
!
!    Input, real(r8) X(POINT_NUM), the abscissas of the data points.
!    At least NTERMS of the values in X must be distinct.
!
!    Input, real(r8) F(POINT_NUM), the data values at the points X(*).
!
!    Input, real(r8) W(POINT_NUM), the weights associated with
...
!
!    Output, real(r8) B(NTERMS), C(NTERMS), D(NTERMS), are quantities
...
!
  ...
  integer ( kind = 4 ) j
  real(r8) p
  real(r8) pj(point_num)
  real(r8) pjm1(point_num)
  real(r8) s(nterms)
  real(r8), parameter :: tol = 0.0D+00
...
!
!    Input, real(r8) B(NTERMS), C(NTERMS), D(NTERMS), the information
!    computed by LEAST_SET.
!
!    Input, real(r8) X, the point at which the least squares polynomial
!    is to be evaluated.
!
!    Output, real(r8) PX, the value of the least squares
!    polynomial at X.
!
...
!    is to be evaluated.
!
!    Output, real(r8) PX, PXP, the value and derivative of the least
...
!
!    Input, real(r8) TDATA(NDATA), the abscissas of the data
!    points.  The values in TDATA must be in strictly ascending order.
!
!    Input, real(r8) YDATA(DIM_NUM,NDATA), the data points
...
!
!    Input, real(r8) TVAL, the value of T at which the parabolic
...
!
!    Output, real(r8) YVAL(DIM_NUM), the value of the parabolic
!    interpolant at TVAL.
!
  ...
  real(r8) tval
  real(r8) tdata(ndata)
  real(r8) ydata(dim_num,ndata)
  real(r8) y1
  real(r8) y2
  real(r8) y3
  real(r8) yval(dim_num)
!
!  Check.
!
  if ( left < 1 ) then
...
!  Parameters:
!
!    Input, real(r8) ARG1, ARG2, two values to check.
!
!    Output, real(r8) PCHST,
...
!  Parameters:
!
!    Input/output, real(r8) X, Y.  On output, the values of X and
...
!
!    Output, real(r8) R8_UNIFORM_01, a new pseudorandom variate,
...
!
!    Input, real(r8) A(3,N), the R83 matrix.
!
!    Input, real(r8) X(N), the vector to be multiplied by A.
!
!    Output, real(r8) B(N), the product A * x.
!
...
!    Input, integer ( kind = 4 ) N, the order of the linear system.
!
!    Input/output, real(r8) A(3,N).
!    On input, the tridiagonal matrix.
!    On output, the data in these vectors has been overwritten
!    by factorization information.
!
!    Input, real(r8) B(N), the right hand side of the linear system.
!
!    Output, real(r8) X(N), the solution of the linear system.
!
...
!    generator.
!
!    Output, real(r8) A(3,N), the R83 matrix.
!
...
!    N must be positive.
!
!    Input, real(r8) A(5,N), the matrix.
!
...
!
!    Input, real(r8) X(N), an array sorted into ascending order.
!
!    Input, real(r8) XVAL, a value to be bracketed.
!
...
!
!    Input, real(r8) T(N), an array sorted into ascending order.
!
!    Input, real(r8) TVAL, a value to be bracketed by entries of T.
!
...
!
!    Input, real(r8) X(N), the vector to be checked.
!
...
!
!    Input, real(r8) ALO, AHI, the low and high values.
!
!    Output, real(r8) A(N), N evenly spaced values.
...
!
!    Output, real(r8) A(N), the array to be initialized.
!
...
!
!    Input, real(r8) A(N), the array to be checked.
!
...
!
!    Input, real(r8) A(N), the vector to be printed.
!
!    Input, character ( len = * ) TITLE, a title.
!
...
!
!    Output, real(r8) R(N), the vector of pseudorandom values.
!
...
!    Input, integer ( kind = 4 ) N, the number of elements of A.
!
!    Input, real(r8) A(N), the unsorted array to examine.
!
!    Input, real(r8) TOL, a nonnegative tolerance for equality.
...
!    Input, real(r8) TDATA(NDATA), the abscissas of the data.
!
!    Input, real(r8) YDATA(NDATA), the data values.
!
!    Input, real(r8) TVAL, a point at which the spline is
!    to be evaluated.
!
!    Output, real(r8) YVAL, the value of the function at TVAL.
!
...
!
!    Input, real(r8) DATA_VAL(DIM_NUM,3*INTERVAL_NUM+1), the control
!    values.
!
!    Input, integer ( kind = 4 ) POINT_NUM, the number of sample points at
!    which the Bezier cubic spline is to be evaluated.
!
!    Input, real(r8) POINT_T(POINT_NUM), the "T" values associated
...
!
!    Output, real(r8) POINT_VAL(DIM_NUM,POINT_NUM), the value
...
!
  ...
  real(r8) bernstein_val(0:cubic)
  real(r8) data_val(dim_num,cubic*interval_num+1)
  integer ( kind = 4 ) dim
  integer ( kind = 4 ) interval
  integer ( kind = 4 ) offset
  integer ( kind = 4 ) point
  real(r8) point_t(point_num)
  real(r8) point_val(dim_num,point_num)
  real(r8) t
  real(r8) t_01
...
!
!    Input, real(r8) TDATA(NDATA-1), the breakpoints.  The values
!    of TDATA should be distinct and increasing.
!
!    Input, real(r8) YDATA(NDATA), the values of the spline in
!    the intervals defined by the breakpoints.
!
!    Input, real(r8) TVAL, the point at which the spline is
!    to be evaluated.
!
!    Output, real(r8) YVAL, the value of the spline at TVAL.
!
...
!    at least 2.
!
!    Input, real(r8) T(N), the points where data is specified.
!    The values should be distinct, and increasing.
!
!    Input, real(r8) Y(N), the data values to be interpolated.
!
...
!
!    Input, real(r8) YBCBEG, the left boundary value, if needed.
!
...
!
!    Input, real(r8) YBCEND, the right boundary value, if needed.
!
!    Output, real(r8) YPP(N), the second derivatives of
!    the cubic spline.
!
...
!    Input, integer ( kind = 4 ) N, the order of the matrix.
!
!    Input, real(r8) A1(N), A2(N), A3(N), A4(N), A5(N), the nonzero
...
!
!    Input, real(r8) T(N), the knot values.
!
!    Input, real(r8) Y(N), the data values at the knots.
!
!    Input, real(r8) YPP(N), the second derivatives of the
!    spline at the knots.
!
!    Input, real(r8) TVAL, a point, typically between T(1) and
!    T(N), at which the spline is to be evalulated.  If TVAL lies outside
!    this range, extrapolation is used.
!
!    Output, real(r8) YVAL, YPVAL, YPPVAL, the value of the spline, and
...
!    Input, real(r8) Y(N), the data values at the knots.
!
!    Input, real(r8) YPP(N), the second derivatives of the spline at
!    the knots.
!
...
!
!    Input, real(r8) TVAL, a point, typically between T(1) and T(N), at
...
!
!    Input, real(r8) TDATA(NDATA), the abscissas of the data points.
!    The entries of TDATA are assumed to be strictly increasing.
!
!    Input, real(r8) Y(NDATA), YP(NDATA), the value of the
!    function and its derivative at TDATA(1:NDATA).
!
!    Output, real(r8) C(4,NDATA), the coefficients of the
!    Hermite polynomial.
...
!    The entries of TDATA are assumed to be strictly increasing.
!
!    Input, real(r8) C(4,NDATA), the coefficient data computed by
!    SPLINE_HERMITE_SET.
!
!    Input, real(r8) TVAL, the point where the interpolant is to
!    be evaluated.
!
!    Output, real(r8) SVAL, SPVAL, the value of the interpolant
...
!
!    Input, real(r8) TDATA(NDATA), YDATA(NDATA), the values of
!    the independent and dependent variables at the data points.  The
!    values of TDATA should be distinct and increasing.
!
!    Input, real(r8) A, B, the interval over which the
!    integral is desired.
!
!    Output, real(r8) INT_VAL, the value of the integral.
!
...
!
!    Input, real(r8) INT_X(N+1), the points that define the intervals.
!    Interval I lies between INT_X(I) and INT_X(I+1).
!
!    Input, real(r8) INT_V(N), the desired value of the integral of the
!    linear spline over each interval.
!
!    Output, real(r8) DATA_X(N), DATA_Y(N), the values of the
...
!    to be evaluated.
!
!    Output, real(r8) YVAL, YPVAL, the value of the spline and
...
!    Input, real(r8) YDATA(NDATA), the data values.
!
!    Input, real(r8) TVAL, the value where the spline is to
!    be evaluated.
!
!    Output, real(r8) YVAL, the value of the spline at TVAL.
!
  ...
  integer ( kind = 4 ) left
  real(r8) mbasis(4,4)
  real(r8) mbasis_l(3,3)
  real(r8) mbasis_r(3,3)
  integer ( kind = 4 ) right
  real(r8) tdata(ndata)
  real(r8) tval
  real(r8) ydata(ndata)
...
!    corresponding to the abscissas.
!
!    Input, real(r8) TVAL, the abscissa value at which the spline
!    is to be evaluated.  Normally, TDATA(1) <= TVAL <= T(NDATA), and
!    the data will be interpolated.  For TVAL outside this range,
!    extrapolation will be used.
!
!    Output, real(r8) YVAL(DIM_NUM), the value of the spline at TVAL.
!
...
!    at least 2.
!
!    Input, real(r8) X(N), the strictly increasing independent
!    variable values.
!
!    Input, real(r8) F(N), dependent variable values to be
...
!    direction.
!
!    Output, real(r8) D(N), the derivative values at the
...
!    variable values.
!
!    Input, real(r8) F(N), the function values.
!
!    Input, real(r8) D(N), the derivative values.
!
!    Input, integer ( kind = 4 ) NE, the number of evaluation points.
!
!    Input, real(r8) XE(NE), points at which the function is to
!    be evaluated.
!
!    Output, real(r8) FE(NE), the values of the cubic Hermite
!    function at XE.
!
...
!    values of TDATA should be distinct and increasing.
!
!    Input, real(r8) TVAL, the point at which the spline is to
!    be evaluated.
!
!    Output, real(r8) YVAL, YPVAL, the value of the spline and
!    its first derivative dYdT at TVAL.  YPVAL is not reliable if TVAL
