...
real(r8)            :: mloc(3)
real(r8)            :: obs_val,obs_val_up,obs_val_dwn,wrf_psf,level
...
! Interpolate WRF CO data to MOPITT pressure level midpoint
   ...
   if (istatus /= 0) then
      write(string1, *)'APM ERROR: istatus,kstr,obs_val_up ',istatus,kstr,obs_val_up
      write(string2, *)'APM ERROR: mopitt_prs_mid,wrf_psf,mopitt_psurf,mopitt_psf ', &
      mopitt_prs_mid,wrf_psf,mopitt_psurf(key),mopitt_psf
      write(string3, *)'APM ERROR: i, nlevels ',i,nlevels
      call error_handler(E_ERR,'set_obs_def_mopitt_co', string1, &
   ...
   if (istatus /= 0) then
      write(string1, *)'APM ERROR: istatus,kstr,obs_val_dwn ',istatus,kstr,obs_val_dwn
      write(string2, *)'APM ERROR: mopitt_prs_mid,wrf_psf,mopitt_psurf,mopitt_psf ', &
      mopitt_prs_mid,wrf_psf,mopitt_psurf(key),mopitt_psf
      write(string3, *)'APM ERROR: i, nlevels ',i,nlevels
      call error_handler(E_ERR,'set_obs_def_mopitt_co', string1, &
...
! Check for WRF CO lower bound
   if (obs_val_up.lt.co_min) then
      obs_val_up=co_min
      write(string1, *)'APM NOTICE: in obs_def_mopitt resetting minimum co value for obs_val_up '
      call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
   endif
   if (obs_val.lt.co_min) then
      obs_val=co_min
      write(string1, *)'APM NOTICE: in obs_def_mopitt resetting minimum co value for obs_val '
      call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
   endif
   if (obs_val_dwn.lt.co_min) then
      obs_val_dwn=co_min
      write(string1, *)'APM NOTICE: in obs_def_mopitt resetting minimum co value for obs_val_dwn '
      call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
   endif
