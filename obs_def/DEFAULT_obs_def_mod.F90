...
use     location_mod, only : location_type, read_location, write_location, &
                             interactive_location, set_location_missing, operator(/=)
use time_manager_mod, only : time_type, read_time, write_time, print_time, &
                             set_time_missing, interactive_time, operator(/=)
...
use     obs_kind_mod, only : assimilate_this_obs_kind, evaluate_this_obs_kind, &
                             ...
                             get_kind_from_menu, has_ext_prior_this_obs_kind, &
...
public :: init_obs_def, get_obs_def_key, get_obs_def_location, get_obs_kind, &
   ...
   assignment(=), get_obs_name, eq_obs_def, ne_obs_def, operator(==), operator(/=), &
   print_obs_def
...
subroutine print_obs_def(obs_def1)
...
call write_location(0, obs_def1%location, charstring=string)
print *, 'location: ', trim(string)
print *, 'kind: ', obs_def1%kind, get_obs_kind_name(obs_def1%kind)
call print_time(obs_def1%time, ' time: ')
print *, 'error_variance: ', obs_def1%error_variance
print *, 'private key: ', obs_def1%key
...
if(assimilate_this_ob .or. evaluate_this_ob) then
   ...
   if (use_precomputed_FO) then
      ! pull out the right member or error out if there are more ensembles now
      ! than precomputed priors
   else
...
! the way this must work is you must list all obs specific types which
! have precomputed forward operators in the obs_seq file.  you can control
! whether to assimilate or evaluate them with the other flags.
if (has_precomputed_FO) then
   ! read in number of ens members and then read in the precomputed priors
endif
...
! whether to assimilate or evaluate them with the other flags.
if (has_precomputed_FO) then
   ! write out number of ens members and then the precomputed priors
endif
...
! FIXME: if this costs enough, skip it for performance reasons
! or only do it in a debugging mode.
if (.false.) then
