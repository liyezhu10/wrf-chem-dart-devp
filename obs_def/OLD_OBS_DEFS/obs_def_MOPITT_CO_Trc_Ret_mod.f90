...
!
! APM: the Trc_Ret code assimilates the rotated and truncated (y_r - y_a)
   val = val + avg_kernel(key,i) * (obs_val - mopitt_prior(key))  
   apm_val = apm_val + avg_kernel(key,i) * (log10(obs_val*1.e-6) - mopitt_prior(key))  
enddo
!
val = val
apm_val = apm_val 
