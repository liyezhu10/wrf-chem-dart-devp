...
!
! $Id: obs_def_AIRNOW_PM10_mod.f90 7039 2014-07-02 22:00:57Z mizzi $
...
!   case(AIRNOW_PM10)
!        call get_expected_airnow_pm10(state, location, obs_val, istatus) 
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/mizzi/obs_def/obs_def_AIRNOW_PM10_mod.f90 $"
...
subroutine get_expected_airnow_pm10(state_vector, location, pm10, istatus)  
 real(r8),            intent(in)  :: state_vector(:)
 type(location_type), intent(in)  :: location
 integer,             intent(out) :: istatus
 real(r8),            intent(out) :: pm10 ! (ug/m^3 dry air)
...
!
! hydrophobic black carbon at this location - this calls the model_mod code.
...
!
! hydrophobic organic carbon at this location - this calls the model_mod code.
...
!
! Expected pm10 (ug/kg)
print *, 'APM use_log_pm10 ',use_log_pm10

if (use_log_pm10) then
   pm10 = (exp(p25) + 1.375*exp(sulf) + exp(BC1) + exp(BC2) + 1.8*(exp(OC1)+exp(OC2)) + &
        exp(DUST1) + exp(DUST2) + exp(DUST3) + .87*exp(DUST4) + exp(SS1) + exp(SS2) +exp(SS3))
else
   pm10 = (p25 + 1.375*sulf + BC1 + BC2 + 1.8*(OC1+OC2) + &
...
pm10 = pm10 * T/298.15 * 760./P*Pa_to_torr
