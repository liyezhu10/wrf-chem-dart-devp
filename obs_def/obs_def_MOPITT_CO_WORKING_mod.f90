...
! Correct mopitt surface pressure
    ...
    if(mopitt_psf.gt.wrf_psf) then
       if((mopitt_psf-wrf_psf).gt.10000.) then
          write(string1, *)'APM: NOTICE - reject MOPITT CO - WRF PSF too large ',mopitt_psf,wrf_psf
          call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
          obs_val=MISSING_R8
          istatus=2
          return
...
! Find kstr - the surface level index
    ...
    elseif (kstr.gt.6) then
       write(string1, *)'APM: ERROR MOPITT CO surface pressure is unrealistic: mopitt_psf, wrf_psf= ',mopitt_psf,wrf_psf
       call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
       stop
    endif
...
! Reject ob when number of MOPITT levels from WRF cannot equal actual number of MOPITT levels
    ...
    if(nnlevels.ne.nlevels) then
       write(string1, *)'APM: NOTICE reject MOPITT CO ob - WRF MOP  levels .ne. MOP levels, nnlvls,nlvls ',nnlevels,nlevels
       call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
       obs_val=MISSING_R8
       istatus=2
       return
