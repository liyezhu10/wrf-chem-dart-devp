...
subroutine get_expected_slant_tec(state_vector, tx_loc, rx_loc, obs_val, istatus)
!-----------------------------------------------------------------------------
! Given DART state vector and transmitter/receiver location, this subroutine
! computes path-integrated electron density
! The istatus variable should be returned as 0 unless there is a problem
! 
! Author: Alex Chartier, Johns Hopkins University Applied Physics Laboratory
! April 2016
!

real(r8),            intent(in) :: state_vector(:)
type(location_type), intent(in) :: tx_loc, rx_loc, loc
real(r8),           intent(out) :: obs_val
integer,            intent(out) :: istatus

real(r8) :: tx_x, tx_y, tx_z, loc_x, loc_y, loc_z
real(r8) :: loc_alt, tx_loc_alt, elec_nden
real(r8), parameter :: rad_earth = 6371.0E3_r8 ! Radius of Earth
real(r8), parameter :: step = 1.0E3_r8 ! Parameter to step along the path
real(r8), parameter :: ionosphere_bottom_alt = 1.0E5_r8 ! Bottom of ionosphere

if ( .not. module_initialized ) call initialize_module
istatus = 1
! Pseudo-code version of what needs to happen here (assuming Tx is above Rx)
loc = rx_loc
obs_val = 0.0

if (vert_is_height(tx_loc)) then
   tx_loc_alt = tx_loc(3)

if (vert_is_height(loc)) then
   loc_alt = loc(3)

call geo2carte(tx_loc(3), tx_loc(1), tx_loc(2), tx_x, tx_y, tx_z, rad_earth)
call geo2carte(rx_loc(3), rx_loc(1), rx_loc(2), rx_x, rx_y, rx_z, rad_earth)
delta_mag = sqrt((tx_x - rx_x)**2 + (tx_y - rx_y)**2 + (tx_z - rx_z)**2)
delta_x = (tx_x - rx_x) / delta_total 
delta_y = (tx_y - rx_y) / delta_total 
delta_z = (tx_z - rx_z) / delta_total 

do while (loc_alt < tx_loc_alt)
   ! geo2carte inputs: height, lat, lon, x, y, z, Re
   call geo2carte(loc(3), loc(1), loc(2), loc_x, loc_y, loc_z, rad_earth)

   loc_x = loc_x + delta_x * step
   loc_y = loc_y + delta_y * step
   loc_z = loc_z + delta_z * step
   call carte2geo(loc_x, loc_y, loc_z, loc(3), loc(1), loc(2), rad_earth)
   loc_alt = loc(3)

   if (loc_alt < ionosphere_bottom_alt) 
      cycle ! Do nothing until you get up to the bottom of the model
      
   else if(get_location(loc, 'altitude') < model_bottom_alt) 
      call interpolate(state_vector, loc, KIND_DENSITY_ION_E, elec_nden, istatus)
      obs_val = obs_val + elec_nden * step

   else
      cycle ! Do nothing above model top for now (later, extrapolate topside)
      !call extrap_elec_nden(state_vector, loc, elec_nden)
      !obs_val = obs_val + elec_nden * step
...
! alexc: Suggest putting geo2carte and carte2geo in a utilities/location file
! somewhere
...
! note: nlevels refers to midpoints, ilevels to interface levels
TOPDOWN : do ilayer = nlevels-1, 1, -1 
   ...
   if (ilayer == 1) then
       write(string1, *) 'Something is wrong if we made it all the way through the thermosphere without hitting 1E21 nitrogen molecules per m^2'
       call error_handler(E_ERR,'get_expected_O_N2_ratio', string1, &
                      source, revision, revdate)
   endif
enddo TOPDOWN
