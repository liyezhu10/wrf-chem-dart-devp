...
! Check WRF carbon monoxide at surface
   if(istatus/=0) then
      ...
      call error_handler(E_MSG,'set_obs_def_mopitt_o3',string1,source,revision,revdate)
      obs_val=missing
      return
   endif              
!
...
! interpolation failed
      if (istatus /= 0) then
         write(string1, *)'APM: NOTICE reject MOPITT CO ob - WRF interpolation failed ',istatus,ilev
         call error_handler(E_MSG,'set_obs_def_mopitt_co',string1,source,revision,revdate)
         obs_val = missing 
         return
      endif
