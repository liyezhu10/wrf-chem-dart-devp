...
if (filter_kind == 9) then
   ...
   OBS_INF: do i = 1, obs_ens_handle%num_vars
      ...
      if(ens_handle%my_pe == owner) then
         ...
         ob_info(1) = vertvalue_obs_in_localization_coord
         ob_info(2) = obs_qc
         ...
         if (.not. lanai_bitwise) then
            ...
            call set_vertical(base_obs_loc, vertvalue_obs_in_localization_coord)
         endif
         ...
         vertvalue_obs_in_localization_coord = ob_info(1)
         obs_qc = ob_info(2)
...
SEQUENTIAL_OBS: do i = 1, obs_ens_handle%num_vars
   ...
   if(ens_handle%my_pe == owner) then
      ...
      ! JPOTERJOY : Temporary fix instead of modifying broadcast_send/recv for additional vectors 
      if (filter_kind == 9) then
   ...
   !-----------------------------------------------------------------------
      ...
      ! JPOTERJOY : Temporary fix instead of modifying broadcast_send/recv
      if (filter_kind == 9) then
...
select case (filter_kind)
 ...
 case (9)
   msgstring = 'Localized Particle Filter (Poterjoy)'
 case default 
...
if (filter_kind == 9) then
   msgstring  = "Localized Particle filter namelist parameters: "
   write(msgstring2, *)  " frac_neff = ", frac_neff
   write(msgstring3, *)  " pf_alpha = ", pf_alpha, " pf_kddm = ", pf_kddm
   call error_handler(E_MSG,'assim_tools_init:', msgstring, text2=msgstring2, text3=msgstring3)
endif
