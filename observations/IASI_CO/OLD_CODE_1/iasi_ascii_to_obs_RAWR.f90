...
!
real*8 :: fac,dofs,qstatus,sum,cum
...
real*8, allocatable, dimension(:,:) :: avg_k,cov_p,cov_r,cov_m,cov_a,cov_use
...
double precision, allocatable, dimension(:,:) :: s_cov_use,s_avg_k
double precision, allocatable, dimension(:,:) :: U_s_avg_k,UT_s_avg_k,VT_s_avg_k,rs_avg_k
double precision, allocatable, dimension(:,:) :: rs_cov_use
...
! Read IASI
  read(fileid,*,iostat=ios) transform_typ, sec, lat, lon, nlvls, dofs 
!  print *, 'trans_typ, sec, lat, lon, nlevels, dofs: ',trim(transform_typ),sec,lat,lon,nlvls,dofs
  nlevels=real(nlvls)
  nlvlsp=nlvls+1
!
! Error Check
...
!-------------------------------------------------------
  do while(ios == 0)
       ...
       allocate (cov_use(nlvls,nlvls))
!       print *, 'read iasi_prs'
       read(fileid,*) (iasi_prs(i),i=1,nlvlsp)
!       print *, 'read iasi_prs',iasi_prs(1)
!       print *, 'read x_r'
       read(fileid,*) (x_r(i),i=1,nlvls)
!       print *, 'read x_r',x_r(1)
!       print *, 'read x_p'
       read(fileid,*) (x_p(i),i=1,nlvls)
!       print *, 'read x_p',x_p(1)
!       print *, 'read avg_k'
       read(fileid,*) ((avg_k(j,i),i=1,nlvls),j=1,nlvls)
!       print *, 'read avg_k',avg_k(1,1)
!       print *, 'read cov_a'
       read(fileid,*) ((cov_a(j,i),i=1,nlvls),j=1,nlvls)
!       print *, 'read cov_a',cov_a(1,1)
!       print *, 'read cov_r'
       read(fileid,*) ((cov_r(j,i),i=1,nlvls),j=1,nlvls)
!       print *, 'read cov_r',cov_r(1,1)
!       print *, 'read cov_m'
       read(fileid,*) ((cov_m(j,i),i=1,nlvls),j=1,nlvls)
!       print *, 'read cov_m',cov_m(1,1)
!       print *, 'read co_tot_col, co_tot_err'
       read(fileid,*) co_tot_col,co_tot_err
!       print *, 'read co_tot_col, co_tot_err',co_tot_col,co_tot_err
       deallocate (iasi_prs,x_r,x_p,avg_k,cov_a,cov_r,cov_m,cov_use)
!       
       index_qc = index_qc + 1
       qc_iasi(index_qc)=0
...
!       print *, 'apm_count ',apm_count
     ...
     ! Read IASI variables
       ...
       read(fileid,*) ((avg_k(j,i),i=1,nlvls),j=1,nlvls)
!       do i=1,nlvls
!          print *, "APM: AK = ",i,(avg_k(i,j),j=1,nlvls)
!       enddo
       read(fileid,*) ((cov_a(j,i),i=1,nlvls),j=1,nlvls)
       read(fileid,*) ((cov_r(j,i),i=1,nlvls),j=1,nlvls)
       read(fileid,*) ((cov_m(j,i),i=1,nlvls),j=1,nlvls)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Rotate the forward operator  
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Assign cov_use
          fac=1.0
!          cov_use(:,:)=fac*cov_m(:,:)+(1.-fac)*cov_r(:,:)
          cov_use(:,:)=fac*cov_r(:,:)
!
! Adjust the retrieval to remove the prior
          ...
          enddo
          allocate(raw_x_r(nlvls),raw_err(nlvls))
!
! Raw retrievals 
          do i=1,nlvls
             raw_x_r(i)=x_r(i)
...
! Calculate SVD of cov_use (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          allocate(rs_avg_k(nlvls,nlvls),rs_cov_use(nlvls,nlvls))
          allocate(rs_x_r(nlvls),rs_x_p(nlvls))
          Z(:,:)=dble(cov_use(:,:))
          call dgesvd('A','A',nlvls,nlvls,Z,nlvls,SV_cvr,U_cvr,nlvls,VT_cvr,nlvls,wrk,lwrk,info)
          deallocate(wrk)
          nlvls_trc=nlvls
!          do j=1,nlvls
!             if(SV_cvr(j).ge.eps_tol) then
!                nlvls_trc=j
!             else
!                SV_cvr(j)=0.
!                U_cvr(:,j)=0.
...
!             if(j.eq.2) then
!                fac=0.8
!             else if(j.eq.4.or.j.eq.5.or.j.eq.6.or.j.eq.7) then
!                fac=2.2
!             else if(j.eq.8) then
!                fac=2.
!             else if(j.eq.9) then
...
!          call mat_prd(UT_cvr,dble(avg_k),rs_avg_k,nlvls,nlvls,nlvls,nlvls)
!          call mat_tri_prd(UT_cvr,dble(cov_use),U_cvr,rs_cov_use,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
!          call lh_mat_vec_prd(UT_cvr,dble(adj_x_r),rs_x_r,nlvls)
!          call lh_mat_vec_prd(UT_cvr,dble(adj_x_p),rs_x_p,nlvls)
          deallocate(U_cvr,V_cvr)
          deallocate(Z,SV,UT_cvr,VT_cvr)
!
! Get new errors (check if err2_rs_r < 0 the qstatus=1)
          allocate(err2_rs_r(nlvls))
          qstatus=0.0
!          do i=1,nlvls
!             if (rs_cov_use(i,i) .lt. eps_tol) then
!                qstatus = 1.0
!             else
!                err2_rs_r(i)=sqrt(rs_cov_use(i,i))
!             endif
!          enddo
!          nrows=0
!          do i=1,nlvls
!             if(abs(SV_cvr(i)).ge.eps_tol) then
!                nrows=nrows+1
!             endif
!          enddo 
          do i=1,nlvls
...
!! Set the following to zero to use the standard forward operator
!                xapr(i)=rs_x_p(i)
!!                xapr(i)=0.
!                do j=1,nlvls
...
!             enddo
             do i=1,nrows
                xcomp(i)=raw_x_r(i)
                xcomperr(i)=raw_err(i)
                xapr(i)=adj_x_p(i)
                do j=1,nlvls
                   avgker(i,j)=avg_k(i,j)
                enddo
             enddo
!
! APM clean up allocatables from my code
             deallocate(adj_avg_k,adj_x_p,adj_x_r)
             deallocate(rs_cov_use,rs_avg_k,rs_x_r,rs_x_p)
...
!
             ...
             deallocate(rs_cov_use,rs_avg_k,rs_x_r,rs_x_p)
             deallocate(rs_avg_xp,err2_rs_r)
