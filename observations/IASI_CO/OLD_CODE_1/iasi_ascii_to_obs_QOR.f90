...
!       print *, 'trans_typ, sec, lat, lon, nlevels, dofs: ',trim(transform_typ),sec,lat,lon,nlevels,dofs
       nlvls=int(nlevels)
       nlvlsp=nlvls+1
       ! print *, nlevels, sec, nlevels, iasilev, lat, lon
       !if (sec < 0.0) then 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Rotate the forward operator with SVD of cov_r  
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
! Calculate SVD of cov_r (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          deallocate(wrk)
!       do i=1,nlvls
!          print *, 'U row ',i, U_cvr(i,1:nlvls)
!       enddo
!       print *, 'SV ',SV_cvr
!       do i=1,nlvls
!          print *, 'VT row ',i, VT_cvr(i,1:nlvls)
!       enddo
!
...
! Rotate terms in the forward operator       
          ...
          call mat_tri_prd(UT_cvr,dble(cov_r),U_cvr,rs_cov_r,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_cvr,dble(adj_x_r),rs_x_r,nlvls)
          call lh_mat_vec_prd(UT_cvr,dble(adj_x_p),rs_x_p,nlvls)
!       do i=1,nlvls
!          print *, 'UT row ',i, UT_cvr(i,1:nlvls)
!       enddo
!        print *, 'rs_x_r ',rs_x_r
!        print *, 'rs_x_r ',rs_x_p
!       do i=1,nlvls
!          print *, 'rs_cov_r ',i, rs_cov_r(i,1:nlvls)
!       enddo
!
...
!          call lh_mat_vec_prd(rs_avg_k,dble(x_p),rs_avg_xp,nlvls)
!       do i=1,nlvls
!          print *, 'rot_prior_term ',rs_x_p(i)-rs_avg_xp(i)
!       enddo
!
...
! APM make assignments to Ave's scaled variables
          if (qstatus .eq. 0) then
             ...
             allocate(iasilev(nlevp))
             iasilev(:)=iasi_prs(1:nlevp)
             do i=1,nlev
                xcomp(i)=rs_x_r(i)
...
!             deallocate(s_cvr,s_cov_r,s_avg_k,s_x_r,s_x_p)
