...
!integer,parameter       :: nlvls_rej=5
integer,parameter       :: nlvls_rej=0
!
! 44 km
...
integer                 :: nlvls, nlvlsp, index_qc, klvls, nlvls_mod, nlvls_mod_kst
...
integer                 :: irej,irr,icc,skip
integer,dimension(nlvls_rej)               :: rej_lvl=(/15, 16, 17, 18, 19/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/16, 17, 18, 19/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/17, 18, 19/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/18, 19/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/19/)
...
double precision,allocatable,dimension(:,:)    :: Z_Row,ZL,SV,U_cov,V_cov,UT_cov,VT_cov
...
! Calculate RAW prior term based on RAW averaged quantities
        nlvls_mod=ias_dim-nlvls_rej
        nlvls=xg_nlvls(i,j)
        kstr=ias_dim-xg_nlvls(i,j)+1
        nlvls_mod_kst=nlvls_mod-kstr+1
        adj_avg_k(:,:)=0.
        adj_x_p(:)=0.
        x_r(:)=0.
        x_p(:)=0.
...
! Adjust the RAW retrieval to remove the RAW prior
        ...
        enddo
!        print *, 'xg_ret_adj_x_r ',(xg_ret_adj_x_r(i,j,l),l=1,ias_dim)
!        do k=kstr,ias_dim
!          print *, 'cov ',k,(xg_ret_cov(i,j,k,l),l=kstr,ias_dim)
!        enddo
!
! Calculate SVD of ret_cov (Z=U_xxx * SV_xxx * VT_xxx)
        allocate(SV_cov(nlvls_mod_kst),SV(nlvls_mod_kst,nlvls))
        allocate(U_cov(nlvls_mod_kst,nlvls_mod_kst),UT_cov(nlvls_mod_kst,nlvls_mod_kst))
        allocate(V_cov(nlvls,nlvls),VT_cov(nlvls,nlvls))
        allocate(ZL(nlvls_mod_kst,nlvls_mod_kst),ZV(nlvls_mod_kst))
        allocate(Z_Row(nlvls_mod_kst,nlvls))
        allocate(rs_avg_k(nlvls_mod,nlvls),rs_cov(nlvls_mod,nlvls_mod))
        allocate(rs_x_r(nlvls_mod_kst),rs_x_p(nlvls_mod_kst))       
        allocate(rr_avg_k(nlvls_mod,nlvls),rr_cov(nlvls_mod,nlvls_mod))
        allocate(rr_x_r(nlvls_mod_kst),rr_x_p(nlvls_mod_kst))       
        irr=0
        do k=kstr,ias_dim
           skip=0
           do irej=1,nlvls_rej
              if(k.eq.rej_lvl(irej)) then
                 skip=1
                 exit
              endif
           enddo
           ...
           irr=irr+1
           do l=kstr,ias_dim
              Z_Row(irr,l-kstr+1)=dble(xg_avg_k(i,j,k,l))
           enddo
        enddo          
!
!        do k=1,ias_dim
!          print *, 'xg_avg_k ',k,(xg_avg_k(i,j,k,l),l=1,nlvls)
!        enddo
!        do k=1,nlvls_mod_kst
!          print *, 'Z_Row ',k,(Z_Row(k,l),l=1,nlvls)
!        enddo
!
        call dgesvd('A','A',nlvls_mod_kst,nlvls,Z_Row,nlvls_mod_kst,SV_cov, &
        U_cov,nlvls_mod_kst,VT_cov,nlvls,wrk,lwrk,info)
!       print *, 'SV ',SV_cov(:)
        nlvls_trc=0
        do k=1,nlvls_mod_kst
...
!        print *, 'SV ',SV_cov(:)
        call mat_transpose(U_cov,UT_cov,nlvls_mod_kst,nlvls_mod_kst)
        call mat_transpose(VT_cov,V_cov,nlvls,nlvls)
        call vec_to_mat(SV_cov,SV,nlvls_mod_kst,nlvls)
!
! Rotate terms in the forward operator
        ...
        do k=kstr,ias_dim
           ...
           enddo
        enddo          
        call mat_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),Z_Row(1:nlvls_mod_kst,1:nlvls), &
        rr_avg_k(1:nlvls_mod_kst,1:nlvls),nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst,nlvls)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!          print *, 'Z_Row ',k,(Z_Row(k,l),l=1,nlvls)
!        enddo
...
!
        ...
        do k=kstr,ias_dim
           ...
           do l=kstr,ias_dim
              ...
              if(skip.eq.1) cycle
              icc=icc+1
              ZL(irr,icc)=dble(xg_raw_cov(i,j,k,l))
           enddo
        enddo          
!        do k=1,nlvls_mod_kst
!           print *, 'raw_cov ',k,(xg_raw_cov(i,j,k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'ZL ',k,(ZL(k,l),l=1,nlvls_mod_kst)
!        enddo
        call mat_tri_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        ZL(1:nlvls_mod_kst,1:nlvls_mod_kst),U_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        rr_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),nlvls_mod_kst,nlvls_mod_kst, &
        nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
...
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'rr_cov ',k,(rr_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!
        irr=0
        do k=kstr,ias_dim
           ...
           if(skip.eq.1) cycle
           irr=irr+1
           ZV(irr)=dble(xg_raw_adj_x_r(i,j,k))
        enddo
        call lh_mat_vec_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),ZV(1:nlvls_mod_kst), &
        rr_x_r(1:nlvls_mod_kst),nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'ZV ',(ZV(l),l=1,nlvls_mod_kst)
!        print *, 'xg_raw_adj_x_r ',(xg_raw_adj_x_r(i,j,l),l=kstr,ias_dim)
!        print *, 'rr_x_r ',(rr_x_r(l),l=1,nlvls_mod_kst)
!
        irr=0
        do k=kstr,ias_dim
           skip=0
           ...
           if(skip.eq.1) cycle
           irr=irr+1
           ZV(irr)=dble(xg_raw_adj_x_p(i,j,k))
        enddo
        call lh_mat_vec_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),ZV(1:nlvls_mod_kst), &
        rr_x_p(1:nlvls_mod_kst),nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'ZV ',(ZV(l),l=1,nlvls_mod_kst)
!        print *, 'xg_raw_adj_x_p ',(xg_raw_adj_x_p(i,j,l),l=kstr,ias_dim)
!        print *, 'rr_x_p ',(rr_x_p(l),l=1,nlvls_mod_kst)
!
! Calculate SVD of rr_cov (Z=U_xxx * SV_xxx * VT_xxx) - SECOND ROTATION
         ZL(1:nlvls_mod_kst,1:nlvls_mod_kst)=rr_cov(1:nlvls_mod_kst,1:nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'rr_cov ',k,(rr_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'ZL ',k,(ZL(k,l),l=1,nlvls_mod_kst)
!        enddo
        call dgesvd('A','A',nlvls_mod_kst,nlvls_mod_kst,ZL,nlvls_mod_kst, &
        SV_cov,U_cov,nlvls_mod_kst,VT_cov,nlvls_mod_kst,wrk,lwrk,info)
!        do k=1,nlvls_mod_kst
!           print *, 'U_cov ',k,(U_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'VT_cov ',k,(VT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'SV_cov ',k,(SV_cov(l),l=1,nlvls_mod_kst)
!
        do k=1,nlvls_mod_kst
           if(SV_cov(k).ge.eps_tol) then
              nlvls_trc=k
...
!
        ...
        call vec_to_mat(SV_cov,SV,nlvls_mod_kst,nlvls)
!        do k=1,nlvls_mod_kst
!           print *, 'U ',k,(U_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!
! Rotate terms in the forward operator
        Z_Row(1:nlvls_mod_kst,1:nlvls)=rr_avg_k(1:nlvls_mod_kst,1:nlvls)
        call mat_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),Z_Row(1:nlvls_mod_kst,1:nlvls), &
        rs_avg_k(1:nlvls_mod_kst,1:nlvls),nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst,nlvls)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
...
!
        ...
        call mat_tri_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        ZL(1:nlvls_mod_kst,1:nlvls_mod_kst),U_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        rs_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),nlvls_mod_kst,nlvls_mod_kst, &
        nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst,nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
...
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'rs_cov ',k,(rs_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
        ZV(1:nlvls_mod_kst)=rr_x_r(1:nlvls_mod_kst)
        call lh_mat_vec_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        ZV(1:nlvls_mod_kst),rs_x_r(1:nlvls_mod_kst),nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'ZV ',(ZV(l),l=1,nlvls_mod_kst)
!        print *, 'rr_x_r ',(rr_x_r(l),l=1,nlvls_mod_kst)
!        print *, 'rs_x_r ',(rs_x_r(l),l=1,nlvls_mod_kst)
!
        ZV(1:nlvls_mod_kst)=rr_x_p(1:nlvls_mod_kst)
        call lh_mat_vec_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst), &
        ZV(1:nlvls_mod_kst),rs_x_p(1:nlvls_mod_kst),nlvls_mod_kst)
!        do k=1,nlvls_mod_kst
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'ZV ',(ZV(l),l=1,nlvls_mod_kst)
!        print *, 'rr_x_p ',(rr_x_p(l),l=1,nlvls_mod_kst)
!        print *, 'rs_x_p ',(rs_x_p(l),l=1,nlvls_mod_kst)
!
...
!
           ...
           obs_old=obs
        enddo
        deallocate(SV_cov,SV)
        deallocate(U_cov,UT_cov)
        deallocate(V_cov,VT_cov)
        deallocate(ZL,ZV)
        deallocate(Z_Row)
        deallocate(rs_avg_k,rs_cov)
        deallocate(rs_x_r,rs_x_p)       
        deallocate(rr_avg_k,rr_cov)
        deallocate(rr_x_r,rr_x_p)       
     enddo
  enddo   
!
!----------------------------------------------------------------------
...
!
    subroutine vec_to_mat(a_vec,A_mat,n,m)
!
! compute dot product of two matrics
    integer :: n,m,i
    double precision :: a_vec(n),A_mat(n,m)
!
! initialze the product array
    A_mat(:,:)=0.
!
