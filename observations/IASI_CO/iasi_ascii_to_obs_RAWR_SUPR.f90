...
real,dimension(ias_dim)         :: x_r, x_p, x_p_col, air_col, raw_x_r, raw_x_p, err2_rs_r, raw_err
real,dimension(ias_dim)         :: xcomp, xcomperr, xapr
real,dimension(ias_dim,ias_dim) :: avgker, avg_k, adj_avg_k, temp_mat
real,dimension(ias_dim,ias_dim) :: raw_cov, cov_a, cov_r, cov_m, cov_use
real,dimension(nlon_qc,nlat_qc) :: xg_lon, xg_lat, xg_twt
real,dimension(nlon_qc,nlat_qc,ias_dim) :: xg_sec, xg_raw_x_r, xg_raw_err
real,dimension(nlon_qc,nlat_qc,ias_dim) :: xg_adj_x_r
real,dimension(nlon_qc,nlat_qc,ias_dim) :: xg_adj_x_p, xg_raw_x_p, xg_norm, xg_nint
real,dimension(nlon_qc,nlat_qc,ias_dim,ias_dim) :: xg_avg_k, xg_raw_cov
...
! xg_adj_x_p was the average prior term
        ...
        xg_adj_x_p(i,j,kstr:ias_dim)=adj_x_p(1:xg_nlvls(i,j))
!
     enddo
  enddo
!
