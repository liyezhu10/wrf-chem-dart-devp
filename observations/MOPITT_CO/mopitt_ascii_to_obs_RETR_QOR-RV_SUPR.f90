...
integer,dimension(nlvls_rej)               :: rej_lvl=(/5, 7, 9/)
...
! Calculate SVD of ret_cov (Z=U_xxx * SV_xxx * VT_xxx)
        ...
        allocate(V_cov(nlvls_mod_kst,nlvls_mod_kst),VT_cov(nlvls_mod_kst,nlvls_mod_kst))
...
!           enddo
!           print *, 'SV_cov ',(SV_cov(l),l=1,nlvls_mod_kst)
!        endif
        nlvls_trc=0
        do k=1,nlvls_mod_kst
...
!           print *,'nlvls_mod_kst ',nlvls_mod_kst
!           print *, 'SV ',SV_cov(:)
!           stop
!        endif
!
...
!
        ...
        call mat_transpose(VT_cov,V_cov,nlvls_mod_kst,nlvls_mod_kst)
        call vec_to_mat(SV_cov,SV,nlvls_mod_kst,nlvls_mod_kst)
!
!        do k=1,nlvls_mod_kst
...
!
!        do kk=1,nlvls_mod_kst
!           if(rs_cov(kk,kk).lt.0.) then
!              print *, '(i,j)',i,j
!              do k=1,nlvls_mod_kst
!                 print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!              enddo
...
!          print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'xg_adj_x_r ',(xg_ret_adj_x_r(i,j,l),l=kstr,nlvls_mod)
...
!           print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls_mod_kst)
!        enddo
!        print *, 'xg_adj_x_p ',(xg_ret_adj_x_p(i,j,l),l=kstr,nlvls_mod)
...
! APM make assignments to Ave's scaled variables
        ...
        do k=1,xg_nlvls(i,j)
           skip=0
           do irej=1,nlvls_rej
              if(k+kstr-1.eq.rej_lvl(irej)) then
                 skip=1
                 exit
              endif
           enddo
           if(skip.eq.1) cycle
           kk=kk+1
           if(nint(err2_rs_r(kk)).eq.0) cycle
           qc_count=qc_count+1
!
! RAW with NO ROT
...
! RET QOR with ROT and SCALE
           xcomp(kk)=rs_x_r(kk)
           xcomperr(kk)=err2_rs_r(kk)
           xapr(kk)=0.
           do l=1,xg_nlvls(i,j)
              avgker(kk,l)=rs_avg_k(kk,l)
           enddo
!           if(xcomp(kk).eq.0.) then
!              print *, '(i,j) ',i,j
!              print *, 'y_r, err ',xcomp(kk),xcomperr(kk) 
!              print *, 'avg_k ',avgker(kk,:)
!              stop
!           endif
!
!--------------------------------------------------------
...
! time (get time from sec MOPITT variable)
           hour1 = int(xg_sec(i,j,k+kstr-1)/3600d0)
           if(hour1.gt.24) then
              print *, 'APM 2: hour error ',hour1,xg_sec(i,j,k+kstr-1)
              stop
           endif
           minute = int( (xg_sec(i,j,k+kstr-1)-hour1*3600d0)/60d0)
           second = int(xg_sec(i,j,k+kstr-1) - hour1*3600d0 - minute*60d0)
           if(hour1.gt.24.or.hour.gt.24) then
              print *, 'APM 3: hour error ',hour1,hour
              stop
           endif
           if ( hour == 24 ) then
              if (xg_sec(i,j,k+kstr-1) < 3.00*3600d0) then
                 day1 = day+1
...
!
           ...
           co_avgker(1:xg_nlvls(i,j))=avgker(kk,1:xg_nlvls(i,j))
