...
integer                 :: i, j, k, l, ll, kk, ik, ikk, k1, k2, kstr 
...
! Calculate number of vertical levels and averages
  ...
  do i=1,nlon_qc  
     do j=1,nlat_qc
        ...
        enddo
!
! APM: test basing xg_ret_err on xg_ret_cov +++
!        do k=1,mop_dim
!           xg_ret_err(i,j,k)=sqrt(xg_ret_cov(i,j,k,k))
!        enddo
!
! APM: test basing prior term on averaged terms
!        adj_x_p(:)=0.
!        adj_avg_k(:,:)=0.
!        do k=1,nlvls
!           do l=1,nlvls
!              adj_avg_k(k,l)=-1.*xg_avg_k(i,j,k,l)
!           enddo
!           adj_avg_k(k,k)=adj_avg_k(k,k)+1.
!        enddo
!        call lh_mat_vec_prd(dble(adj_avg_k),dble(xg_ret_x_p(i,j,:)),adj_x_p,nlvls)
!        do k=1,nlvls
!           xg_ret_adj_x_p(i,j,k)=adj_x_p(k)            
!        enddo
! APM: end test code ---
!     
...
! Set vertical for levels (irot=0) or modes (irot=1)
        ...
        if(irot.eq.1) nlvls_fix=nlvls_trc
        do k=1,nlvls_fix
!
...
! uncomment scaling
           ...
           do l=1,xg_nlvls(i,j)
              print *, 'avgk ', l, (rs_avg_k(l,ll),ll=1,xg_nlvls(i,j))
           enddo
           stop
!
