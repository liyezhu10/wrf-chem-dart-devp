...
character(len=*), parameter :: source   = 'mopitt_ascii_to_obs_SUPR_OBS.f90'
...
nlon_qc=(lon_max-lon_min)/360.*2.*pi*re/1000./qc_del
nlat_qc=cos(lat_mn/rad2deg)*(lat_max-lat_min)/360.*2.*pi*re/1000./qc_del
dlon_qc=(lon_max-lon_min)/nlon_qc
dlat_qc=(lat_max-lat_min)/nlat_qc
!
...
!
       if(lon+180.ge.lon_min .and. lon+180.le.lon_max .and.
       lat+90.ge.lat_min .and. lat+90.le.lat_max) then       
          index_qc = index_qc + 1
...
!-------------------------------------------------------
! find lon_qc, lat_qc
          lon_qc=nint((lon+180)/dlon_qc) - nint(lon_min)/dlon_qc)  + 1
          lat_qc=nint((lat+90)/dlat_qc) -nint(lat_min/dlat_qc) + 1
          if (lat>89.5) then
             lat_qc=nlat_qc
