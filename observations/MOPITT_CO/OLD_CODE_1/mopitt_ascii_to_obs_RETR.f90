...
real*8, allocatable, dimension(:,:) :: avg_k,cov_r,cov_m,cov_a,cov_use
real*8, allocatable, dimension(:,:) :: avgker
!
double precision, allocatable, dimension(:) :: wrk,SV_cvr,SV_cva
double precision, allocatable, dimension(:) :: s_x_r,s_x_p
...
!-------------------------------------------------------
  do while(ios == 0)
       ...
       deallocate (mop_prs,x_r,x_p,avg_k,cov_a,cov_r,cov_m,cov_use)
!       
       index_qc = index_qc + 1
...
!       print *, 'apm_count ',apm_count
     ...
     ! Read MOPITT variables
       ...
       read(fileid,*) cov_m
       read(fileid,*) co_tot_col,co_tot_err
!print *, 'icnt, mop_dim, nlvls ',apm_count,mop_dim,nlvls
!print *, 'prs ',mop_prs
!print *, 'x_r ',x_r
!print *, 'x_p ',x_p
!print *, 'avg_k ',avg_k
...
! Calculate SVD of cov_use (Z=U_xxx * SV_xxx * VT_xxx)
          ...
          allocate(SV_cvr(nlvls),SV_cva(nlvls))
...
!          enddo
!
! Scale the singular values to adjust the error 
!          do j=1,nlvls_trc
!              fac=1.0
!              if(j.eq.1.or.j.eq.nlvls) then
!                 fac=1.0
!              else if(j.eq.2.or.j.eq.3) then
!                 fac=2.5
!              else if(j.eq.4.or.j.eq.5.or.j.eq.6.or.j.eq.7.or.j.eq.8.or.j.eq.9) then
!                 fac=4.0
!              endif
!              SV_cvr(j)=SV_cvr(j)*fac*fac
!          enddo
!
...
!          call mat_tri_prd(UT_cvr,dble(cov_use),U_cvr,rs_cov_use,nlvls,nlvls, &
!          nlvls,nlvls,nlvls,nlvls)
...
!          enddo
!          do i=1,nlvls
!             if (SV_cvr(i) .lt. eps_tol) then
!                qstatus = 1.0
!                err2_rs_r(i)=sqrt(SV_cvr(i))
!             else
!                err2_rs_r(i)=sqrt(SV_cvr(i))
!             endif
!          enddo
...
!          do i=1,nlvls
!             fac=1.0
!             if (i.ge.1 .or. i.le.7) then
!                fac=0.75
!                fac=1.1
!             endif
!             err2_rs_r(i)=fac*fac*err2_rs_r(i)
!          enddo
!
!          nrows=0
!          do i=1,nlvls
...
! APM make assignments to Ave's scaled variables
          if (qstatus .eq. 0) then
             ...
             allocate(mopittlev(nlevp))
             mopittlev(1:nlevp)=mop_prs(1:nlevp)
!
! QORS
!             do i=1,nrows
!                xcomp(i)=rs_x_r(i)
...
!      layer average convention
                level=(mopittlev(kk)+mopittlev(kk+1))/2*100
...
!                which_vert=-2      ! undefind
            ...
            ! 
                ...
                co_press(1:nlvlsp)=mopittlev(1:nlvlsp)*100               !unitless
                co_prior=xapr(kk)  
                co_vmr(1)=xcomp(kk)
                err = xcomperr(kk)
                co_error=err*err
...
!
                ...
                if(kk.eq.1) then
!                   call set_obs_def_mopitt_co(qc_count, co_avgker, co_press, co_prior, co_psurf, &
!                        nlvls, nlvlsp)
