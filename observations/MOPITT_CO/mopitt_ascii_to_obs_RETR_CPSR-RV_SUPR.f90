...
integer,dimension(nlvls_rej)               :: rej_lvl=(/8, 9, 10/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/9, 10/)
!integer,dimension(nlvls_rej)               :: rej_lvl=(/10/)
...
!
! Calculate RAW prior term based on RAW averaged 
        nlvls_mod=mop_dim-nlvls_rej
        nlvls=xg_nlvls(i,j)
...
! Adjust the RET retrieval to remove the RET prior
        ...
        enddo
!        print *, 'xg_ret_adj_x_r ',(xg_ret_adj_x_r(i,j,l),l=1,mop_dim)
!        do k=kstr,mop_dim
...
!
        ...
        do k=kstr,mop_dim
           ...
           do l=kstr,mop_dim
              ...
              if(skip.eq.1) cycle
              icc=icc+1
              ZL(irr,icc)=dble(xg_ret_cov(i,j,k,l))
           enddo
        enddo          
...
!
        ...
        do k=kstr,mop_dim
           ...
           if(skip.eq.1) cycle
           irr=irr+1
           ZV(irr)=dble(xg_ret_adj_x_r(i,j,k))
        enddo
...
!        print *, 'xg_ret_adj_x_r ',(xg_ret_adj_x_r(i,j,l),l=kstr,mop_dim)
...
!
        ...
        do k=kstr,mop_dim
           ...
           if(skip.eq.1) cycle
           irr=irr+1
           ZV(irr)=dble(xg_ret_adj_x_p(i,j,k))
        enddo
        call lh_mat_vec_prd(UT_cov(1:nlvls_mod_kst,1:nlvls_mod_kst),ZV(1:nlvls_mod_kst), &
...
!        enddo
!        print *, 'ZV ',(ZV(l),l=1,nlvls_mod_kst)
!        print *, 'xg_ret_adj_x_p ',(xg_ret_adj_x_p(i,j,l),l=kstr,mop_dim)
...
!        do k=kstr,nlvls_mod_kst
!           print *, 'rr_cov ',k,(rr_cov(i,j,k,l),l=1,nlvls_mod_kst)
!        enddo
!        do k=1,nlvls_mod_kst
!           print *, 'ZL ',k,(ZL(k,l),l=1,nlvls_mod_kst)
!        enddo
