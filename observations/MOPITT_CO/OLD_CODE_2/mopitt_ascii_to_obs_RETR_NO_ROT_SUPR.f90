...
! 89 km
integer,parameter       :: nlon_qc=450, nlat_qc=150, nqc_obs=40 
...
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_sec, xg_raw_x_r, xg_raw_err, xg_ret_err
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_adj_x_r, xg_ret_x_r, xg_ret_x_p
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_raw_adj_x_p, xg_ret_adj_x_p
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_raw_x_p, xg_norm, xg_nint
...
! Calculate errors for NO ROT RAW case
        ...
        enddo
!
! Calculate errors for NO ROT RETR case
        do j=1,nlvls
           ret_err(j)=sqrt(cov_use(j,j))
        enddo
!
! Calculate superobs
        ...
        do i=kstr,mop_dim
           ...
           do j=kstr,mop_dim
              ...
              xg_ret_cov(lon_qc,lat_qc,i,j)=xg_ret_cov(lon_qc,lat_qc,i,j)+cov_use(i-kstr+1,j-kstr+1)*wt
           enddo
        enddo
...
! xg_ret_adj_x_p was the average prior term
        nlvls=xg_nlvls(i,j)
...
!        enddo
!        call mat_tri_prd(UT_cov(1:nlvls,1:nlvls),dble(xg_ret_cov(i,j,kstr:mop_dim,kstr: &
