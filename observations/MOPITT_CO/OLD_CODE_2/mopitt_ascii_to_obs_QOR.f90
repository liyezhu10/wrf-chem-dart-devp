...
! 89 km
integer,parameter       :: nlon_qc=450, nlat_qc=150, nqc_obs=1 
...
real,dimension(mop_dim)         :: x_r, x_p, raw_x_r, raw_x_p, err2_rs_r, raw_err
real,dimension(mop_dim)         :: xcomp, xcomperr, xapr
real,dimension(mop_dim,mop_dim) :: avgker, avg_k, adj_avg_k
real,dimension(mop_dim,mop_dim) :: raw_cov, cov_a, cov_r, cov_m, cov_use
real,dimension(nlon_qc,nlat_qc) :: xg_lon, xg_lat, xg_twt
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_sec, xg_raw_x_r, xg_raw_err
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_adj_x_r
real,dimension(nlon_qc,nlat_qc,mop_dim) :: xg_adj_x_p, xg_raw_x_p, xg_norm, xg_nint
real,dimension(nlon_qc,nlat_qc,mop_dim,mop_dim) :: xg_avg_k, xg_raw_cov, xg_cov
...
double precision,allocatable,dimension(:)      :: SV_cov
double precision,allocatable,dimension(:)      :: rs_x_r,rs_x_p
double precision,allocatable,dimension(:,:)    :: Z,SV,U_cov,V_cov,UT_cov,VT_cov
...
! Calculate raw errors (APM: DOES RAW ERRORS NEED A 1.e6 SCALING?) 
        do i=1,nlvls
           do j=1,nlvls
              raw_cov(i,j)=cov_use(i,j)*raw_x_r(i)*raw_x_r(j)*ln_10*ln_10
           enddo
        enddo
!
! Calculate errors for NO ROT case
        do j=1,nlvls
...
! Calculate superobs
        ...
        do i=kstr,mop_dim
           ...
           do j=kstr,mop_dim
              ...
              xg_cov(lon_qc,lat_qc,i,j)=xg_cov(lon_qc,lat_qc,i,j)+cov_use(i-kstr+1,j-kstr+1)*wt
           enddo
        enddo
  ...
  do i=1,nlon_qc  
     do j=1,nlat_qc
        ...
        do k=1,mop_dim
           ...
           if(nint(xg_nint(i,j,k)).ne.1) then
              print *, 'nint, norm ',xg_nint(i,j,k),xg_norm(i,j,k)
              stop
           endif
...
!
! Calculate prior term based on averaged quantities
! xg_adj_x_p was the average prior term
        nlvls=xg_nlvls(i,j)
...
!        print *, 'raw_x_p ',(xg_raw_x_p(i,j,l),l=kstr,mop_dim)
        ...
        xg_adj_x_p(i,j,kstr:mop_dim)=adj_x_p(1:xg_nlvls(i,j))
!
! Calculate QQRs and apply error covariance rotation
!
! Adjust the retrieval to remove the prior
        xg_adj_x_r(:,:,:)=0.
        do k=1,mop_dim
           if(xg_norm(i,j,k).eq.0) cycle
           xg_adj_x_r(i,j,k)=x_r(k)-xg_adj_x_p(i,j,k)
        enddo
!        print *, 'adj_x_r ',(xg_adj_x_r(i,j,l),l=1,mop_dim)
!        do k=kstr,mop_dim
!          print *, 'cov ',k,(xg_raw_cov(i,j,k,l),l=kstr,mop_dim)
!        enddo
!
! Calculate SVD of cov_use (Z=U_xxx * SV_xxx * VT_xxx)
        ...
        Z(1:nlvls,1:nlvls)=dble(xg_cov(i,j,kstr:mop_dim,kstr:mop_dim))
...
! Rotate terms in the forward operator
        call mat_prd(UT_cov(1:nlvls,1:nlvls),dble(xg_avg_k(i,j,kstr:mop_dim,kstr:mop_dim)), &
...
!        enddo
        call mat_tri_prd(UT_cov(1:nlvls,1:nlvls),dble(xg_cov(i,j,kstr:mop_dim,kstr: &
        mop_dim)),U_cov,rs_cov(1:nlvls,1:nlvls),nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
!        do k=1,nlvls
!          print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls)
!        enddo
!        do k=kstr,mop_dim
!          print *, 'xg_cov ',k,(xg_cov(i,j,k,l),l=kstr,mop_dim)
!        enddo
!        do k=1,nlvls
!          print *, 'rs_cov ',k,(rs_cov(k,l),l=1,nlvls)
!        enddo
        call lh_mat_vec_prd(UT_cov(1:nlvls,1:nlvls),dble(xg_adj_x_r(i,j,kstr:mop_dim)), &
        rs_x_r(1:nlvls),nlvls)
!        do k=1,nlvls
!          print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls)
!        enddo
!        print *, 'xg_adj_x_r ',(xg_adj_x_r(i,j,l),l=kstr,mop_dim)
!        print *, 'rs_x_r ',(rs_x_r(l),l=1,nlvls)
        call lh_mat_vec_prd(UT_cov(1:nlvls,1:nlvls),dble(xg_adj_x_p(i,j,kstr:mop_dim)), &
        rs_x_p(1:nlvls),nlvls)
!        do k=1,nlvls
!          print *, 'UT ',k,(UT_cov(k,l),l=1,nlvls)
!        enddo
!        print *, 'xg_adj_x_p ',(xg_adj_x_p(i,j,l),l=kstr,mop_dim)
!        print *, 'rs_x_p ',(rs_x_p(l),l=1,nlvls)
!
! Get new errors (check if err2_rs_r < 0 the qstatus=1)
        qstatus=0.0
        do k=1,nlvls
           if (rs_cov(k,k) .lt. eps_tol) then
              qstatus = 1.0
...
! APM make assignments to Ave's scaled variables
        kstr=mop_dim-xg_nlvls(i,j)+1
        do k=1,xg_nlvls(i,j)
           if(xg_norm(i,j,k+kstr-1).eq.0) cycle
           qc_count=qc_count+1
!
! QOR with ROT
           xcomp(k)=rs_x_r(k)
...
!
! QOR with NO ROT
!           xcomp(k)=xg_adj_x_r(i,j,k+kstr-1)
