...
real*8, allocatable, dimension(:,:) :: avg_k,T_avg_k,cov_p,cov_r,cov_m,cov_a
real*8, allocatable, dimension(:,:) :: avgker
!
double precision, allocatable, dimension(:) :: wrk,SV_cvm,SV_cva,rs_avg_xp
double precision, allocatable, dimension(:) :: s_x_r,s_x_p,SV_ss_avg_k
double precision, allocatable, dimension(:) :: rs_x_r,rs_x_p,w,err2_rs_m,err2_rs_a
!
double precision, allocatable, dimension(:,:) :: Z,ZT,U_cvm,V_cvm,U_cva,V_cva
double precision, allocatable, dimension(:,:) :: UT_cvm,VT_cvm,UT_cva,VT_cva
double precision, allocatable, dimension(:,:) :: SV,s_cvm,s_cva
double precision, allocatable, dimension(:,:) :: s_cov_m,s_cov_a,s_avg_k,ss_avg_k
double precision, allocatable, dimension(:,:) :: U_ss_avg_k,UT_ss_avg_k,VT_ss_avg_k,rs_avg_k
double precision, allocatable, dimension(:,:) :: rs_cov_a,rs_cov_m
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Precondition the foreward operator with the inverse 
! square root of the covariance to address ill-conditioned
! covariance matrix
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! Calculate SVD of cov_m (Z=U_xxx * SV_xxx * VT_xxx)
          lwrk=max(4*nlvls,5*nlvls)
          allocate (Z(nlvls,nlvls),SV(nlvls,nlvls),U_cvm(nlvls,nlvls),V_cvm(nlvls,nlvls))
          allocate (UT_cvm(nlvls,nlvls),VT_cvm(nlvls,nlvls),wrk(lwrk))
          allocate (SV_cvm(nlvls),s_cvm(nlvls,nlvls))
          Z(:,:)=cov_m(:,:)
          call dgesvd('A','A',nlvls,nlvls,Z,nlvls,SV_cvm,U_cvm,nlvls,VT_cvm,nlvls,wrk,lwrk,info)
          deallocate(wrk)
!
! Calculate inverse square root of cov_m
          call mat_transpose(U_cvm,UT_cvm,nlvls,nlvls)
          call mat_transpose(VT_cvm,V_cvm,nlvls,nlvls)
          call vec_to_mat(SV_cvm,SV,nlvls)
          call diag_inv_sqrt(SV,nlvls)
          call mat_tri_prd(V_cvm,SV,UT_cvm,s_cvm,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
!
...
!
          deallocate(SV_cvm,SV_cva,U_cvm,U_cva,V_cvm,V_cva)
          deallocate(Z,SV,UT_cvm,UT_cva,VT_cvm,VT_cva)
!
! Precondition terms in the forward operator
          allocate(s_avg_k(nlvls,nlvls),s_cov_m(nlvls,nlvls),s_cov_a(nlvls,nlvls))
          allocate(s_x_r(nlvls),s_x_p(nlvls))
!       
          call mat_prd(s_cvm,avg_k,s_avg_k,nlvls,nlvls,nlvls,nlvls)
! s_cov_m should be identity, it is.
          call mat_tri_prd(s_cvm,cov_m,s_cvm,s_cov_m,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call mat_tri_prd(s_cvm,cov_a,s_cvm,s_cov_a,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(s_cvm,x_r,s_x_r,nlvls)
          call lh_mat_vec_prd(s_cvm,x_p,s_x_p,nlvls)
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! +++++ Ave performs a RHS scaling of averaging kernal with s_cva      +++++
! +++++ This is in addition to the LHS scaling with s_cvm                    +++++
! +++++ This is Ave's rotation formulation                                              +++++
!
          allocate (ss_avg_k(nlvls,nlvls))
          call mat_prd(s_avg_k,s_cva,ss_avg_k,nlvls,nlvls,nlvls,nlvls)
! +++++                                                                      +++++
!
! Calculate SVD of the scaled averaging kernal
          lwrk=max(4*nlvls,5*nlvls)
          allocate (Z(nlvls,nlvls),ZT(nlvls,nlvls),SV_ss_avg_k(nlvls))
          allocate (U_ss_avg_k(nlvls,nlvls),VT_ss_avg_k(nlvls,nlvls),wrk(lwrk))
          allocate (UT_ss_avg_k(nlvls,nlvls),rs_avg_k(nlvls,nlvls))
          Z(:,:)=ss_avg_k(:,:)
          call dgesvd('A','A',nlvls,nlvls,Z,nlvls,SV_ss_avg_k,U_ss_avg_k,nlvls,VT_ss_avg_k,nlvls,wrk,lwrk,info)
          deallocate(wrk)
!
! Rotate s_avg_k with transpose U_ss_avg_k  
          call mat_transpose(U_ss_avg_k,UT_ss_avg_k,nlvls,nlvls)
          call mat_prd(UT_ss_avg_k,s_avg_k,rs_avg_k,nlvls,nlvls,nlvls,nlvls)
!
! Rotate remaining terms in the forward operator
          allocate(rs_cov_a(nlvls,nlvls),rs_cov_m(nlvls,nlvls),rs_x_r(nlvls),rs_x_p(nlvls))    
          call mat_tri_prd(UT_ss_avg_k,s_cov_a,U_ss_avg_k,rs_cov_a,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call mat_tri_prd(UT_ss_avg_k,s_cov_m,U_ss_avg_k,rs_cov_m,nlvls,nlvls,nlvls,nlvls,nlvls,nlvls)
          call lh_mat_vec_prd(UT_ss_avg_k,s_x_r,rs_x_r,nlvls)
          call lh_mat_vec_prd(UT_ss_avg_k,s_x_p,rs_x_p,nlvls)
!
...
!
! Get new errors (check if err2_rs_m < 0 the qstatus=1)
          allocate(err2_rs_m(nlvls),err2_rs_a(nlvls))
          call diag_vec(rs_cov_m,err2_rs_m,nlvls)
          call diag_vec(rs_cov_a,err2_rs_a,nlvls)
          do i=1,nlvls
             if (rs_cov_m(i,i) .lt. 0.) then
!
! APM need to add skip for when qstatus = 1.0
                qstatus = 1.0
             else
                err2_rs_m(i)=sqrt(rs_cov_m(i,i))
                err2_rs_a(i)=sqrt(rs_cov_a(i,i))
             endif
          enddo
!
! Truncate to dofs level
          allocate(var(nlvls)) 
          nrows_d=int(dofs)
...
! APM clean up allocatables from my code
          deallocate(s_cvm,s_cva,s_cov_m,s_cov_a,s_avg_k,s_x_r,s_x_p)
          deallocate(ss_avg_k,rs_cov_m,rs_cov_a,rs_avg_k,rs_x_r,rs_x_p)
          deallocate(rs_avg_xp,err2_rs_m,err2_rs_a,var)
          deallocate(Z,ZT,SV_ss_avg_k,U_ss_avg_k)
          deallocate(UT_ss_avg_k,VT_ss_avg_k)
