...
!>
!> You do need to link this with the types_mod, and with either
!> null_mpi_utilities_mod or possibly the real mpi_utilities_mod.
!>
...
!> debugging of an open unit number
...
inquire (file=fname, opened=open, number=iunit, form=format, iostat=rc)

! if already open, return.  the fortran run-time libs will do any
! error checking if the previous open was not closed and the action
! or form is trying to be changed.
if (open) return
...
!>
!> inputs:
!>    doflag  = whether to output log information or not
...
!>
!>    input:  doflag  = whether to output nml information to stdout 
