...
use obs_sequence_mod, only : obs_sequence_type, obs_type, write_obs_seq,       &
                             ...
                             get_obs_def, set_obs_def, set_obs_values
...
call init_obs_sequence(seq_out, num_copies_in, num_qc_in, size_seq_out * 2)
...
if ( get_first_obs(seq_in, obs_in) )  then
   ...
   ObsLoop : do while ( .not. is_this_last )
      ...
      index = get_copy_index_for_observation()
      value(1) = MISSING_R8
      call set_obs_values(obs_out, value, index)
...
function get_copy_index_for_observation()

integer :: get_copy_index_for_observation

! FIXME: this needs to search the metadata for the string 'observation'
! and return the actual index.

get_copy_index_for_observation = 1

end function get_copy_index_for_observation
...
if (size_seq == 0) then
   msgstring = 'Obs_seq file '//trim(filename)//' is empty.'
   call error_handler(E_MSG,'obs_add_pseudo_obs:validate',msgstring)
   return
endif
...
if ( .not. is_there_one )  then
   ...
   call error_handler(E_ERR,'obs_add_pseudo_obs:validate', &
...
ObsLoop : do while ( .not. is_this_last)
   ...
   if (last_time > this_time) then
      ...
      call error_handler(E_ERR,'obs_add_pseudo_obs:validate', msgstring2, &
...
if (obs_count /= size_seq) then
   ...
   if (obs_count > size_seq) then
      ...
      call error_handler(E_ERR,'obs_add_pseudo_obs:validate', msgstring, &
                         source, revision, revdate, &
                         text2=msgstring1)
   else
      ! just warning msg
...
if ( num_copies < 0 .or. num_qc < 0 ) then
   ...
   call error_handler(E_ERR, 'obs_add_pseudo_obs', msgstring3, &
                      source, revision, revdate)
endif
