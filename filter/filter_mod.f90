...
write(msgstring, '(A,I6)') 'running with an ensemble size of ', ens_size
...
if(task_count() > model_size) then
   write(msgstring,  '(A,I16)') '                          MPI process count =', task_count()
   write(msgstring2, '(A,I16)') 'State vector item count per ensemble member =', model_size
   call error_handler(E_ERR,'filter_main', 'Number of MPI processes cannot be larger than the model size', &
      source,revision,revdate,text2=msgstring,text3=msgstring2)
endif
...
AdvanceTime : do
   ...
   write(msgstring, '(A,I10)') 'Number of observations to be assimilated', &
      num_obs_in_set
   ...
   write(msgstring, '(A,I10,A)') 'Ready to assimilate up to', size(keys), ' observations'
   call trace_message(msgstring, 'filter:', -1)
   ...
   ! in the future
   if(ds) then
      write(msgstring, '(A,I10,A)') 'Ready to reassimilate up to', size(keys), ' observations in the smoother'
      call trace_message(msgstring, 'filter:', -1)
...
if (observations_updateable) then
   write(msgstring, *) 'cannot set observations to be updateable in this version of filter'
   call error_handler(E_ERR,'filter_main', msgstring, &
                      source, revision, revdate, text2='see filter/filter.dopplerfold.f90')
endif
