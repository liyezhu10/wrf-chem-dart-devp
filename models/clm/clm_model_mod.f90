...
!  DART state vector are specified in the input.nml:clm_model_nml namelist.
!
!------------------------------------------------------------------
...
! things which can/should be in the clm_model_nml
...
if (do_output()) call error_handler(E_MSG,'static_init_model','clm_model_nml values are')
...
UPDATE : do ivar=1, nfields
   ...
   if ( .not. progvar(ivar)%update ) then
      ...
      write(string3,*)'as per namelist control in clm_model_nml:clm_variables'
...
!>  specified by the input.nml:clm_model_nml:clm_variables  variable.
...
MyLoop : do i = 1, nrows
   ...
   if ( any(table(i,:) == ' ') ) then
      string1 = 'input.nml &clm_model_nml:clm_variables not fully specified'
...
if ( progvar(ivar)%rangeRestricted == BOUNDED_BOTH ) then
   if (maxvalue < minvalue) then
      write(string1,*)'&clm_model_nml state_variable input error for ',trim(progvar(ivar)%varname)
