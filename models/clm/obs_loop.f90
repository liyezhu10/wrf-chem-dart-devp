...
use time_manager_mod, only : time_type, set_time, get_time, print_time,   &
                             set_date, get_date, print_date, NO_CALENDAR, &
...
character(len=*), parameter :: source   = "clm/obs_loop.f90"
...
type(time_type) :: desired_date
integer         :: desired_days, desired_seconds
integer         :: oday, osec
...
integer :: target_year    = 1601
integer :: target_month   = 1
integer :: target_day     = 1
...
desired_date = set_date(target_year,target_month,target_day)
call get_time(desired_date,desired_seconds,desired_days)
...
if ( get_first_obs(seq_in, obs_in) )  then
   ...
   ObsLoop : do while ( .not. is_this_last )
      ...
      ! use those seconds and the input day.

      call get_obs_def(obs_out, this_obs_def)
      call get_time(get_obs_def_time(this_obs_def), osec, oday)
      call set_obs_def_time(this_obs_def, set_time(osec, desired_days))
