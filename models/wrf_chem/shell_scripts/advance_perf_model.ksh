...
#
# $Id: advance_perf_model.ksh 13133 2019-04-25 21:47:54Z nancy@ucar.edu $
#
...
#
   export C_ENSEMBLE_MEMBER=${ENSEMBLE_MEMBER}
   if [[ ${ENSEMBLE_MEMBER} -lt 1000 ]]; then export C_ENSEMBLE_MEMBER=0${ENSEMBLE_MEMBER}; fi
   if [[ ${ENSEMBLE_MEMBER} -lt 100 ]]; then export C_ENSEMBLE_MEMBER=00${ENSEMBLE_MEMBER}; fi
   if [[ ${ENSEMBLE_MEMBER} -lt 10 ]]; then export C_ENSEMBLE_MEMBER=000${ENSEMBLE_MEMBER}; fi
#
   let INFL=0.0
#
# Create $TEMP_DIR for each member and run WRF there
#   export TEMP_DIR=advance_temp_${ENSEMBLE_MEMBER}
   export TEMP_DIR=advance_temp${ENSEMBLE_MEMBER}
...
#
# Copy wrfinput file
   cp ${CENTRALDIR}/WRF/wrfinput*${ENSEMBLE_MEMBER} ./wrfinput_d${DOMAIN}
#   cp ${CENTRALDIR}/WRF/* ./.
# 
...
# and we need to set up a key list to manage target times
   ...
   if ${SPEC_BC}; then
      if ${USE_WRFVAR}; then
         set -A BDYFILES `echo ${CENTRALDIR}/WRF/wrfbdy_d01*mean`
      else
         set -A BDYFILES `echo ${CENTRALDIR}/WRF/wrfbdy_d01*${ENSEMBLE_MEMBER}`
         echo file 1 ${BDYFILES[0]}
         echo file 2 ${BDYFILES[1]}
         echo file 3 ${BDYFILES[2]}
         echo file 4 ${BDYFILES[3]}
      fi
      set KEYLIST=''
#
...
# Copy WRFBDY file to the temp directory
      if ${SPEC_BC}; then
         if ${USE_WRFVAR}; then
            ...
            cp ${CENTRALDIR}/WRF/wrfbdy_d01_${IDAY}_${ISEC}_${C_ENSEMBLE_MEMBER} wrfbdy_d01
         fi
      else
...
#      if [[ ${FILE} -gt ${FILE_STRT} ]]; then export NL_RESTART=.true.; fi
...
# Run wrfchem
      ...
      if [[ ${ENSEMBLE_MEMBER} -le ${SAVE_ENSEMBLE_MEMBER} ]]; then 
         cp wrfout_d${DOMAIN}_${END_STRING} ${WRFOUTDIR}/wrfout_d${DOMAIN}_${END_STRING}_${C_ENSEMBLE_MEMBER}
      fi
      mv wrfout_d${DOMAIN}_${END_STRING} wrfinput_d${DOMAIN}
      rm -rf wrfout*
#
...
#
# Convert wrfout to DART format
   ${CENTRALDIR}/wrf_to_dart > index_wrf_to_dart 2>&1
   mv dart_wrf_vector ${CENTRALDIR}/${OUTPUT_FILE}
#
# Delete temp directory
   cd ${CENTRALDIR}
   if [[ ${DELETE_TEMP_DIR} == true ]]; then rm -rf ${TEMP_DIR}; fi
...
#
# Remove filter_control file to signal completion
rm -rf ${CONTROL_FILE}
exit
...
# $URL: https://svn-dares-dart.cgd.ucar.edu/DART/tags/wrf-chem.r13172/models/wrf_chem/shell_scripts/advance_perf_model.ksh $
# $Id: advance_perf_model.ksh 13133 2019-04-25 21:47:54Z nancy@ucar.edu $
# $Revision: 13133 $
