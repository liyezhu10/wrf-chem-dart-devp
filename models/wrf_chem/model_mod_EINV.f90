...
character(len=129) :: emiss_state_variables(num_state_table_columns,max_state_variables) = 'NULL'
! LXL ---
!
...
namelist /model_nml/ output_state_vector, num_moist_vars, &
                     ...
                     periodic_x, periodic_y, scm, conc_state_variables, &
                     emiss_state_variables
! LXL ---
!
...
! APM/AFAJ ++
   integer :: type_o3, type_co, type_no, &
              type_no2, type_hno3, &
...
!
! LXL +++
   integer :: type_e_o3, type_e_co, type_e_no
   integer :: number_of_conc_variables,number_of_emiss_variables
! APM/AFAJ/LXL ---
!
...
!   if ( add_emiss ) then
      if(file_exist('wrfchemi_d0'//idom)) then
         ...
         call nc_check( nf90_open('wrfchemi_d0'//idom, NF90_NOWRITE, ncid_emiss), &
...
!   endif
   ...
   if(debug) write(*,*) ' ncid_emiss is ',ncid_emiss
! LXL ---
!
!-------------------------------------------------------
! read WRF dimensions
...
!-------------------------------------------------------
!   if ( add_emiss ) then
      call read_emiss_dimensions(ncid_emiss,wrf%dom(id)%e_bt, &
                                    wrf%dom(id)%e_sn, &
                                    wrf%dom(id)%e_we)
!   endif
! LXL ---
...
! get the number of conc/emiss variables wanted in this domain's state
   ...
   wrf%dom(id)%number_of_emiss_variables = get_number_of_wrf_variables(id,emiss_state_variables,var_element_list, var_update_list)
   if(debug) write(*,*) ' number of conc variables is',wrf%dom(id)%number_of_conc_variables
   if(debug) write(*,*) ' number of emiss variables is',wrf%dom(id)%number_of_emiss_variables
! LXL ---
!
...
!
! LXL +++
! Read emiss variables
   do ind = wrf%dom(id)%number_of_conc_variables+1,wrf%dom(id)%number_of_wrf_variables
      ...
      ! get stagger and variable size
      call get_variable_size_from_file(ncid_emiss,id,  &
                                       wrf_state_variables(1,my_index), &
                                       wrf%dom(id)%bt, wrf%dom(id)%bts, &
                                       wrf%dom(id)%sn, wrf%dom(id)%sns, &
                                       wrf%dom(id)%we, wrf%dom(id)%wes, &
      ...
      ! get other variable metadata; units, coordinates and description
      call get_variable_metadata_from_file(ncid_emiss,id,  &
                                       wrf_state_variables(1,my_index), &
                                       wrf%dom(id)%description(ind),         &
                                       wrf%dom(id)%coordinates(ind),         &
                                       wrf%dom(id)%units(ind) )
...
! close data file, we have all we need
   ...
   call nc_check(nf90_close(ncid_emiss),'static_init_model','close wrfchemi_d0'//idom)
! LXL ---
!
...
! Add test for obs_kind /= surface pressure to facilitate recovery of state surface pressure
      if ( sfc_elev_max_diff >= 0 .and. obs_kind /= KIND_SURFACE_PRESSURE) then
! APM ----
!
...
! APM/AFAJ ++
   ...
   ! Check to make sure retrieved integer gridpoints are in valid range
         if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_o3 ) .and. &
              ...
              boundsCheck( k, .false.,                id, dim=3, type=wrf%dom(id)%type_o3 ) ) then
   ...
   ! Interpolation for the O3 field at level k
            ill = wrf%dom(id)%dart_ind(ll(1), ll(2), k, wrf%dom(id)%type_o3)
            iul = wrf%dom(id)%dart_ind(ul(1), ul(2), k, wrf%dom(id)%type_o3)
   ...
   ! Interpolation for the O3 field at level k+1
               ill = wrf%dom(id)%dart_ind(ll(1), ll(2), k+1, wrf%dom(id)%type_o3)
               iul = wrf%dom(id)%dart_ind(ul(1), ul(2), k+1, wrf%dom(id)%type_o3)
...
!         print *, "APM:k ",boundsCheck( k, .false.,                id, dim=3, type=wrf%dom(id)%type_co)     
   ...
   ! Check to make sure retrieved integer gridpoints are in valid range
         if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_co ) .and. &
              ...
              boundsCheck( k, .false.,                id, dim=3, type=wrf%dom(id)%type_co ) ) then
   ...
   ! Interpolation for the CO field at level k
            ill = wrf%dom(id)%dart_ind(ll(1), ll(2), k, wrf%dom(id)%type_co)
            iul = wrf%dom(id)%dart_ind(ul(1), ul(2), k, wrf%dom(id)%type_co)
...
!
! LXL +++
integer, dimension(num_domains) :: e_btDimID
! LXL ---
!
...
!
! LXL +++
   call nc_check(nf90_def_dim(ncid=ncFileID, name="emissions_zdim_d0"//idom,       &
                 len = wrf%dom(id)%e_bt,  dimid = e_btDimID(id)), &
                 'nc_write_model_atts','def_dim emissions_zdim_d0'//idom)
...
!
! LXL +++
      if ( debug ) write(*,*) '3D horizontal id ',dimids_3D ! LXL 10.24
! LXL ---
!
...
!
! LXL +++
         elseif ( wrf%dom(id)%var_size(3,my_index) == wrf%dom(id)%e_bt ) then
           dimids_3D(3)=e_btDimID(id)
! LXL ---
!
         else
...
!
!      if ((base_obs_kind.ne.IASI_O3_RETRIEVAL)          .and. &
!          (base_obs_kind.ne.MOPITT_CO_RETRIEVAL)        .and. &
!          (base_obs_kind.ne.MOPITT_CO_COLUMN)           .and. &
!          (base_obs_kind.ne.MODIS_AOD_RETRIEVAL)        &
!
!      if ( base_obs_kind.ne.OMI_NO2_RETRIEVAL ) then &
...
! get wrf grid dimensions
   ...
   call nc_check( nf90_inq_dimid(ncid, "emissions_zdim", var_id), &
                     'static_init_model','inq_dimid bottom_top')
