...
#
# CODE VERSIONS
export WPS_VER=WPS_3.4
export WRFDA_VER=WRFDA
export WRFDA_TOOLS_VER=WRFDA_TOOLS
export WRF_VER=WRFv3.4_dmpar
export DART_VER=DART/kodiak
#
# EXPERIMENT DETAILS:
export NUM_MEMBERS=30
export NUM_PROCS=8
export MEM_START=1
export REGION=iniens_OSSEAVK
# NAM forecast data
#export FG_TYPE=NAM
...
# GFS forecast data
export FG_TYPE=GFS
export GRIB_PART1=fnl_
export GRIB_PART2=
export USE_PREPBUFR=true
export HOR_SCALE=1500
#
# TIME DATA:
export INITIAL_DATE=2006081000
export FINAL_DATE=2006082000
...
export FCST_RANGE=6
export ASIM_WINDOW=6
export LBC_FREQ=6
(( INTERVAL_SECONDS=${LBC_FREQ}*60*60 ))
export LBC_START=0
#
# DIRECTORIES:
export RUN_DIR=/glade/scratch/barre/${REGION}
export TRUNK_DIR=/glade/scratch/barre
export HYBRID_TRUNK_DIR=${TRUNK_DIR}
export HYBRID_TRUNK_DATA_DIR=${TRUNK_DIR}/${REGION}
export WPS_GEOG_DIR=${TRUNK_DIR}/${WPS_VER}/geog
export DAT_DIR=/home/barre/OSSE_AVK  #/MIZZI/COLO_FIRE_DATA

export HYBRID_SCRIPTS_DIR=${TRUNK_DIR}/hybrid_scripts
export GRIB_DIR=/glade/scratch/barre/data_fnl
...
export DART_INPUT_DIR=/glade/scratch/barre/DART/kodiak/models/wrf/work
#
...
export NL_INTERVAL_SECONDS=21600   #${INTERVAL_SECONDS}
...
export NL_ETA_LEVELS=1.000,0.983,0.970,0.954,0.934,\
0.909,0.880,0.834,0.788,0.742,\
0.697,0.617,0.546,0.480,0.422,\
0.368,0.321,0.278,0.239,0.205,\
0.174,0.147,0.123,0.102,0.0832,\
0.0668,0.0526,0.0402,0.0295,0.0203,\
...
export NL_GFDDA_INNAME="'"wrfdda_d\<domain\>"'"
...
export NL_XWAVENUM=4
export NL_YWAVENUM=3
...
# CREATE UNGRIB FILE LIST
      (( LBC_END=${FCST_RANGE} ))
     # export START_HOUR=${NL_START_HOUR}
      integer ICNT=0
      FILES=''
      if [[ -e ${GRIB_DIR} ]]; then
         (( LBC_ITR=${LBC_START} ))
         while [[ ${LBC_ITR} -le ${LBC_END} ]]; do
               (( HH_ITR=${LBC_ITR}+${HH} ))
		echo ici3 ${HH_ITR}
             # if [[ $HH_ITR -lt 1000 ]]; then export CMEM=${LBC_ITR}; fi
               if [[ ${HH_ITR} -lt 100  ]]; then export CMEM=${HH_ITR}; fi
               if [[ ${HH_ITR} -lt 10   ]]; then export CMEM=0${HH_ITR}; fi
               if [[ ${HH_ITR} -eq 0    ]]; then export CMEM=00; fi
               export FILE=${GRIB_DIR}/${GRIB_PART1}${YYYY}${MM}${DD}_${CMEM}_"00"
               FILES="${FILES} ${FILE}"
        echo ici2 $FILES
            ICNT=ICNT+1
...
# CREATE WRF NAMELIST
	 ...
	 #ln -sf /glade/scratch/barre/WPS_3.4/met_em.d01.${YYYY}-${MM}-${DD}_${HH}* ./.
...
# APM Begin special loop for PRE-EXISTING MET_EM DATA (NOTE: the 24 hr is ad hoc and depends on the forecast period)
         ...
         export LLL_END_DATE=$(${BUILD_DIR}/da_advance_time.exe ${LLL_DATE} ${LBC_FREQ} 2>/dev/null)
         ...
         ln -sf /glade/scratch/barre/WPS_3.4/met_em.d01.${YY}-${MM}-${DD}_${HH}* ./.
...
#
         ...
         cp -f ${WRF_DIR}/run/runrsl_wait.csh ./runrsl.csh
         bsub < runrsl.csh
         RC=$?
         if [[ $RC != 0 ]]; then
...
# SET OBS PATH (DO I NEED OBS FOR RANDOMCV?  THESE OBS MAY BE AT THE WRONG TIME)
	       ...
	       hsi get ${OB_PREPBUFR_DIR}/prepbufr.gdas.${DATE}.wo40.le
               export NL_OB_FORMAT=1
            else
		hsi get ${OB_ASCII_DIR}/${DATE}/ob.ascii
...
# SET WRFDA PARAMETERS
	    ...
	    cp -f ${WRFVAR_DIR}/run/runwrf.csh ./runwrfda.csh
	    bsub < runwrfda.csh
            ${HYBRID_SCRIPTS_DIR}/da_run_hold.ksh wrfdavar
            RC=$?
            if [[ $RC != 0 ]]; then
...
# SET OBS PATH (DO I NEED OBS FOR RANDOMCV?  THESE OBS MAY BE AT THE WRONG TIME)
            ...
            ln -fs ${TRUNK_DIR}/${DART_VER}/models/wrf/work/input.nml ./.
