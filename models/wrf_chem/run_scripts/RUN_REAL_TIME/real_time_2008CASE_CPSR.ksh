...
export EMISS_DAMP_CYCLE=.5
export EMISS_DAMP_INTRA_CYCLE=.5
#
...
export FIRST_FILTER_DATE=2008060112
export FIRST_EMISS_INV_DATE=2008060112
export FIRST_DART_INFLATE_DATE=2008060112
export CYCLE_PERIOD=6
export HISTORY_INTERVAL_HR=1
(( HISTORY_INTERVAL_MIN = ${HISTORY_INTERVAL_HR} * 60 ))
export START_IASI_O3_DATA=2008060112
export END_IASI_O3_DATA=2008063018
...
export EXPERIMENT_DATA_DIR=${ACD_DIR}/2008CASE_REAL_TIME_DATA
export RUN_DIR=${SCRATCH_DIR}/real_2008CASE_CPSR_VARLOC
#
...
export NL_MIN_LON=180.1
export NL_MAX_LON=296.
...
export NL_ETA_LEVELS=1.000,0.993,0.983,0.970,0.954,0.934,\
0.909,0.880,0.834,0.788,0.742,0.697,0.617,0.546,\
0.480,0.422,0.368,0.321,0.278,0.239,0.205,0.174,\
0.147,0.123,0.102,0.0832,0.0668,0.0526,0.0402,0.0295,\
0.0203,0.0124,0.00575,0.000,
#
# PHYSICS NAMELIST:
export NL_MP_PHYSICS=2,2
export NL_RA_LW_PHYSICS=1,1
export NL_RA_SW_PHYSICS=2,2
export NL_RADT=40,40
export NL_SF_SFCLAY_PHYSICS=2,2
export NL_SF_SURFACE_PHYSICS=2,2
export NL_BL_PBL_PHYSICS=2,2
export NL_BLDT=0,0
...
export NL_BIOEMDT=3,3
export NL_PHOTDT=18,18
export NL_CHEMDT=3,3
...
export NL_PLUMERISEFIRE_FRQ=60,60
...
#
   export DOMAIN_IDX=1
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      rm -rf geo_em.d${DOMAIN_CHR}.nc
      ln -fs ${GEOGRID_DIR}/geo_em.d${DOMAIN_CHR}.nc ./.
...
# LINK IN THE METGRID FILES
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf met_em.d${DOMAIN_CHR}.${P_FILE_DATE}
         ln -sf ${RUN_DIR}/${DATE}/metgrid/met_em.d${DOMAIN_CHR}.${P_FILE_DATE} ./.
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
...
#
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         mv wrfinput_d${DOMAIN_CHR} wrfinput_d${DOMAIN_CHR}_$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} 0 -W 2>/dev/null)
         mv wrfbdy_d${DOMAIN_CHR} wrfbdy_d${DOMAIN_CHR}_$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} 0 -W 2>/dev/null)         
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done   
...
# LINK IN THE BACK AND FORW METGRID FILES
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      export P_DD=$(echo $P_DATE | cut -c7-8)
      export P_HH=$(echo $P_DATE | cut -c9-10)
      export P_FILE_DATE=${P_YYYY}-${P_MM}-${P_DD}_${P_HH}:00:00.n
      export DOMAIN_IDX=1
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf ./BK_met_em.d${DOMAIN_CHR}.${P_FILE_DATE}
         ln -sf ${RUN_DIR}/${BACK_DATE}/metgrid/met_em.d${DOMAIN_CHR}.${P_FILE_DATE} ./BK_met_em.d${DOMAIN_CHR}.${P_FILE_DATE}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf ./FW_met_em.d${DOMAIN_CHR}.${P_FILE_DATE}
         ln -sf ${RUN_DIR}/${FORW_DATE}/metgrid/met_em.d${DOMAIN_CHR}.${P_FILE_DATE} ./FW_met_em.d${DOMAIN_CHR}.${P_FILE_DATE}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done
...
#
# APM: NEED TO FIX THIS FOR SINGLE DOMAIN
...
# LINK IN THE BACK AND FORW WRFBDY AND WRFINPUT FILES
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      export P_HH=$(echo $P_DATE | cut -c9-10)
      export P_FILE_DATE=${P_YYYY}-${P_MM}-${P_DD}_${P_HH}:00:00
      rm -rf ./BK_wrfbdy_d${CR_DOMAIN}_${P_FILE_DATE}
      rm -rf ./BK_wrfinput_d${CR_DOMAIN}_${P_FILE_DATE}
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf ./BK_wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE}
         ln -sf ${RUN_DIR}/${BACK_DATE}/real/wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE} ./BK_wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      export P_HH=$(echo $P_DATE | cut -c9-10)
      export P_FILE_DATE=${P_YYYY}-${P_MM}-${P_DD}_${P_HH}:00:00
      rm -rf ./FW_wrfinput_d${CR_DOMAIN}_${P_FILE_DATE}
      rm -rf ./FW_wrfbdy_d${CR_DOMAIN}_${P_FILE_DATE}
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf ./FW_wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE}
         ln -sf ${RUN_DIR}/${FORW_DATE}/real/wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE} ./FW_wrfinput_d${DOMAIN_CHR}_${P_FILE_DATE}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done
...
# LINK NEEDED FILES
   ...
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      rm -rf wrfinput_d${DOMAIN_CHR}
      ln -sf ${REAL_DIR}/wrfinput_d${DOMAIN_CHR}_${FILE_DATE} wrfinput_d${DOMAIN_CHR}
      (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
   done 
#
...
# TEST WHETHER OUTPUT EXISTS
   ...
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      if [[ ! -e exo_coldens_d${DOMAIN_CHR} ]]; then
         touch EXO_COLDENS_FAILED
         exit
      else
         touch EXO_COLDENS_SUCCESS
      fi
      (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
   done 
fi
...
# TEST WHETHER OUTPUT EXISTS
   ...
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      if [[ ! -e wrf_season_wes_usgs_d${DOMAIN_CHR}.nc ]]; then
         touch WESLEY_FAILED
         exit
      else
         touch WESLEY_SUCCESS
      fi
      (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
   done 
fi
...
# LINK NEEDED FILES
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         rm -rf wrfinput_d${DOMAIN_CHR}
         cp ${REAL_DIR}/wrfinput_d${DOMAIN_CHR}_${L_FILE_DATE} wrfinput_d${DOMAIN_CHR}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
         if [[ ${L_DATE} -eq ${DATE} ]]; then
            rm -rf wrfbiochemi_d${DOMAIN_CHR}
         fi
      done 
#
      rm -rf btr*.nc
...
# TEST WHETHER OUTPUT EXISTS
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         if [[ ! -e wrfbiochemi_d${DOMAIN_CHR} ]]; then
            touch WRFCHEM_BIO_FAILED
            exit
         else
            touch WRFCHEM_BIO_SUCCESS
            mv wrfbiochemi_d${DOMAIN_CHR} wrfbiochemi_d${DOMAIN_CHR}_${L_FILE_DATE}
         fi
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
...
#
   rm -rf GLOBAL*.txt
   ln -sf ${EXPERIMENT_WRFFIRECHEMI_DIR}/GLOB*.txt ./.
...
# TEST WHETHER OUTPUT EXISTS
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         if [[ ! -e wrffirechemi_d${DOMAIN_CHR}_${L_FILE_DATE} ]]; then
            touch WRFFIRE_FAILED
            exit
         else
            touch WRFFIRE_SUCCESS
         fi
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
...
#
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}
         export FILE=wrfchemi_d${DOMAIN_CHR}_${L_FILE_DATE}
         rm -rf ${FILE}
         cp ${FILE_PATH}/${FILE} ${FILE}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      export L_DATE=$(${BUILD_DIR}/da_advance_time.exe ${L_DATE} 1 2>/dev/null)
   done
...
# SELECT MOZART DATA FILE
  if [[ ${YYYY} -eq 2008 && ${MM} -eq 06 ]]; then export MOZBC_DATA=/h0001.nc; fi
  if [[ ${YYYY} -eq 2008 && ${MM} -eq 07 ]]; then export MOZBC_DATA=/h0002.nc; fi
...
ch_chem_spc='E_CO','E_NO','E_NO2','E_BIGALK','E_BIGENE','E_C2H4','E_C2H5OH','E_C2H6','E_C3H6','E_C3H8','E_CH2O','E_CH3CHO','E_CH3COCH3','E_CH3OH','E_MEK','E_SO2','E_TOLUENE','E_NH3','E_ISOP','E_C10H16','E_sulf','E_CO_A','E_CO_BB','E_CO02','E_CO03','E_XNO','E_XNO2','E_PM25I','E_PM25J','E_PM_10','E_ECI','E_ECJ','E_ORGI','E_ORGJ','E_SO4I','E_SO4J','E_NO3I','E_NO3J','E_NH4I','E_NH4J','E_PM_25','E_OC','E_BC',
...
# convert ASCII to obs_seq file
   ...
   if [[ -e modis_aod_ascii_${YYYY}${MM}${DD}${HH} ]]; then
      ...
      if [[ -e modis_obs_seq.out ]]; then
         mv modis_obs_seq.out ${MODIS_FILE}
         touch MODIS_AOD_SUCCESS
      else
         touch NO_MODIS_AOD_DATA_${DATE}
      fi
   else
      echo APM: ERROR modis_aod_ascii_${YYYY}${MM}${DD}${HH} does not exit
      touch MODIS_AOD_FAILED_${DATE}
      exit
   fi
fi
#
...
#
      ...
      while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
         export DOMAIN_CHR=0${DOMAIN_IDX}  
         cp ${DART_FILTER_DIR}/wrfout_d${DOMAIN_CHR}_${START_FILE_DATE}_filt.${CMEM} wrfinput_d${DOMAIN_CHR}_${KMEM}
         cp ${UPDATE_BC_DIR}/wrfbdy_d${DOMAIN_CHR}_${START_FILE_DATE}_filt.${CMEM} wrfbdy_d${DOMAIN_CHR}_${KMEM}
         (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
      done 
      let MEM=${MEM}+1
   done
...
# Calculate ensemble mean
   ...
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      ncea -n ${NUM_MEMBERS},4,1 wrfinput_d${DOMAIN_CHR}_0001 wrfinput_d${DOMAIN_CHR}_mean
      ncea -n ${NUM_MEMBERS},4,1 wrfbdy_d${DOMAIN_CHR}_0001 wrfbdy_d${DOMAIN_CHR}_mean
      rm -rf wrfinput_d${DOMAIN_CHR}_*0*
      rm -rf wrfbdy_d${DOMAIN_CHR}_*0*
      (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
   done 
fi
#
...
# Get WRF-Chem emissions files
   ...
   while [[ ${DOMAIN_IDX} -le ${MAX_DOMAINS} ]] do
      export DOMAIN_CHR=0${DOMAIN_IDX}  
      cp ${EXO_COLDENS_DIR}/exo_coldens_d${DOMAIN_CHR} ./.
      cp ${SEASONS_WES_DIR}/wrf_season_wes_usgs_d${DOMAIN_CHR}.nc ./.
      cp ${WRFCHEM_CHEM_EMISS_DIR}/wrfbiochemi_d${DOMAIN_CHR}_${START_FILE_DATE}.${CLOSE_MEM_ID} wrfbiochemi_d${DOMAIN_CHR}_${START_FILE_DATE}
      (( DOMAIN_IDX = ${DOMAIN_IDX} + 1 ))
   done 
#
   export L_DATE=${START_DATE}
