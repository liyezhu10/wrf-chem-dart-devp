...
#IB export FIRST_FILTER_DATE=2014071406
...
#export RUN_DIR=${FRAPPE_DIR}/real_PANDA_COnXX_INDEP.a
...
#
   ...
   cat << EOF >job.ksh
#!/bin/ksh
#SBATCH --job-name=${JOBRND}      # Specify job name
##SBATCH --partition=shared     # Specify partition name
#SBATCH --partition=prepost
#SBATCH --ntasks=1             # Specify max. number of tasks to be invoked
#SBATCH --mem-per-cpu=1280     # Specify real memory required per CPU in MegaBytes
#SBATCH --time=01:00:00        # Set a limit on the total run time
#SBATCH --account=mh0735       # Charge resources on this project account
#SBATCH --output=${JOBRND}.o%j    # File name for standard output
#SBATCH --error=${JOBRND}.e%j     # File name for standard error output
...
#SBATCH --ntasks-per-node=48   # Specify number of (MPI) tasks on each node
#SBATCH --time=01:00:00        # Set a limit on the total run time
##SBATCH --mail-type=FAIL       # Notify user by email in case of job failure
##SBATCH --mail-user=idir.bouarar@mpimet.mpg.de #Set your e-mail address
#SBATCH --account=mh0735       # Charge resources on this project account
#SBATCH --output=LOG.o%    # File name for standard output
#SBATCH --error=LOG.e%j     # File name for standard error output
...
#IB   cp /mnt/lustre01/work/mh0735/m300315/WRF_chem/mozbc/mozbc ./.
...
# CREATE INPUT FILES COARSE DOMAIN
   rm -rf mozbc.ic.inp
#IB  ln -sf ${PERT_CHEM_INPUT_DIR}/MOZART/${MOZART_DATA} .
...
#IB   ncks -A ${FRAPPE_DUST_DIR}/EROD_d${CR_DOMAIN} ${WRFINPEN}
...
# COMBINE WRFCHEM WITH WRF FR DOMAIN PARENT FILES
   ...
   ncks -A ${REAL_DIR}/${WRFINPEN} ${WRFINPEN}
#IB   ncks -A ${FRAPPE_DUST_DIR}/EROD_d${FR_DOMAIN} ${WRFINPEN}
...
#IB      cp ${WRFCHEM_DIR_GABI}/test/em_real/wrf.exe ./.
