...
#
# To generate the chemistry perturbations check NL_SW_GENERATE comments
#
#########################################################################
#
# CYCLE DATE-TIME:
export CYCLE_STR_DATE=2016052500
export CYCLE_STR_DATE=2016052512
export CYCLE_END_DATE=${CYCLE_STR_DATE}
export CYCLE_END_DATE=2016052512
...
export FR_DOMAIN=02
#
# Start changes for new applications
export NNXP_CR=140
export NNYP_CR=140
export NNZP_CR=37
export NNXP_FR=231
...
#
# CODE VERSIONS:
# IB change to your version names
...
export RUN_DIR=${FRAPPE_DIR}/real_KORUS_RETR_VARLOC
export TRUNK_DIR=${WORK_DIR}/TRUNK
export TRUNK_DIR_GABI=${FRAPPE_DIR}/FRAPPE_TRUNK
...
export WPS_GEOG_DIR=/glade/p/work/wrfhelp/WPS_GEOG
...
export KORUS_DATA_DIR=${FRAPPE_DIR}/KORUS_REAL_TIME_DATA
export MOZBC_DATA_DIR=${FRAPPE_DIR}/KORUS_REAL_TIME_DATA/cam_chem_forecasts
export KORUS_STATIC_FILES=${KORUS_DATA_DIR}/static_files
export KORUS_WRFCHEMI_DIR=${KORUS_DATA_DIR}/anthro_emissions
export KORUS_WRFFIRECHEMI_DIR=${KORUS_DATA_DIR}/fire_emissions
export KORUS_WRFBIOCHEMI_DIR=${KORUS_DATA_DIR}/bio_emissions
export KORUS_COLDENS_DIR=${KORUS_DATA_DIR}/wes_coldens
export KORUS_PREPBUFR_DIR=${KORUS_DATA_DIR}/met_obs_prep_data
export KORUS_MOPITT_CO_DIR=${KORUS_DATA_DIR}/mopitt_co_hdf_data
export KORUS_IASI_CO_DIR=${KORUS_DATA_DIR}/iasi_co_hdf_ata
export KORUS_IASI_O3_DIR=${KORUS_DATA_DIR}/iasi_o3_hdf_data
...
if [[ ${NL_MIN_LON} -lt 0. ]]; then ((NNL_MIN_LON=${NL_MIN_LON}+360.)); fi 
export NNL_MAX_LON=${NL_MAX_LON}
if [[ ${NL_MAX_LON} -lt 0. ]]; then ((NNL_MAX_LON=${NL_MAX_LON}+360.)); fi 
...
#export NL_GEOG_DATA_RES="'"default"'","'"default"'"
#export NL_GEOG_DATA_RES="'"modis_15s+modis_30s+30s"'","'"modis_15s+modis_30s+30s"'"
...
export NL_NUM_METGRID_LEVELS=48
...
export NL_DIFF_6TH_FACTOR=0.05,0.05
export NL_KM_OPT=4
...
# UNTAR THE PARENT FORECAST FILES
   FILES=''
   if [[ -e ${KORUS_GFS_DIR}/${DATE} ]]; then
      if [[ -e ${KORUS_GFS_DIR}/${DATE}/${GRIB_PART1}${DATE}${GRIB_PART2} ]]; then
         cd ${KORUS_GFS_DIR}/${DATE}
         tar -xf ${KORUS_GFS_DIR}/${DATE}/${GRIB_PART1}${DATE}${GRIB_PART2}
         cd ${RUN_DIR}/${DATE}/ungrib
      else
...
###
###   if [[ -e ${KORUS_GFS_DIR} ]]; then
###      if [[ -e ${KORUS_GFS_DIR}/${GRIB_PART1}${DATE}_0000.grib2 ]]; then
###         cd ${RUN_DIR}/${DATE}/ungrib
###      else
###         echo 'APM: ERROR - No GRIB files in directory'
###         exit
...
###         export CCHH=${HH}
         ...
         while [[ ${LBC_ITR} -le ${LBC_END} ]]; do
            ...
            export FILE=${KORUS_GFS_DIR}/${DATE}/${GRIB_PART1}${START_YEAR}${START_MONTH}${START_DAY}_${CCHH}_${CFTM}.grb2
###            export FILE=${KORUS_GFS_DIR}/${GRIB_PART1}${START_YEAR}${START_MONTH}${START_DAY}${CCHH}_${CFTM}.grib2
            FILES="${FILES} ${FILE}"
            (( LBC_ITR=${LBC_ITR}+${LBC_FREQ} ))
         done
      else
         export FILE=${KORUS_GFS_DIR}/${DATE}/GFS_Global_0p5deg_20080612_1800.grib2
         FILES="${FILES} ${FILE}"
      fi
   fi
#
...
# TAR THE PARENT FORECAST FILES
   if [[ -e ${KORUS_GFS_DIR}/${DATE}/${GRIB_PART1}${DATE}${GRIB_PART2} ]]; then
      rm -rf ${KORUS_GFS_DIR}/${DATE}/${GRIB_PART1}*.grb2
   else
      cd ${KORUS_GFS_DIR}
...
###         cp ${KORUS_PREPBUFR_DIR}/${DATE}/prepbufr.gdas.${DATE}.wo40.be ob.bufr
...
# JOB SCRIPT 
         ...
         rm -rf *.out
         touch job.ksh
         export JOBRND=wrfda_cr_${T_RANDOM}
         cat << EOF >job.ksh
...
if [[ \$RC = 0 ]]; then
   touch SUCCESS_${JOBRND}
else
   touch FAILED_${JOBRND} 
   exit
fi
EOF
#
...
# JOB SCRIPT 
         ...
         rm -rf *.out
         touch job.ksh
         export JOBRND=wrfda_fr_${T_RANDOM}
         cat << EOF >job.ksh
#!/bin/ksh -aeux
#BSUB -P ${PROJ_NUMBER_NSC}
#BSUB -x                                    # exclusive use of node (not_shared)
...
# JOB SCRIPT 
         ...
         rm -rf *.out
         touch job.ksh
         export JOBRND=pert_bc_${T_RANDOM}
         cat << EOF >job.ksh
#!/bin/ksh -aeux
#BSUB -P ${PROJ_NUMBER_NSC}
#BSUB -x
...
# LINK NEEDED FILES
   ...
   rm -rf ${FILE}
   ln -sf ${KORUS_COLDENS_DIR}/${FILE} ${FILE}
...
# LINK NEEDED FILES
      ...
      rm -rf shr*.nc
      rm -rf TAS*.nc
      cp ${KORUS_WRFBIOCHEMI_DIR}/MEGAN-DATA/*.nc ./.
      export FILE=megan_bio_emiss
      rm -rf ${FILE}
      cp ${KORUS_WRFBIOCHEMI_DIR}/MEGAN-BIO/${FILE} ${FILE}
#
# CREATE INPUT FILE
...
# LINK NEEDED FILES
   ...
   ln -sf ${REAL_DIR}/${FILE_FR}_${FILE_DATE} ${FILE_FR}   
   rm -rf GLOBAL*.txt
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/GLOBAL*.txt ./.
   export FILE=fire_emis
   rm -rf ${FILE}
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/src/${FILE} ${FILE}
   rm -rf grass_from_img.nc
   rm -rf shrub_from_img.nc
   rm -rf tempfor_from_img.nc
   rm -rf tropfor_from_img.nc
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/grass_from_img.nc
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/shrub_from_img.nc
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/tempfor_from_img.nc
   ln -sf ${KORUS_WRFFIRECHEMI_DIR}/tropfor_from_img.nc
#
# CREATE INPUT FILE
...
&control
domains = 2,
fire_filename(1) = 'GLOBAL_FINNv15_2016_MOZ4_11022016.txt',
...
#
   while [[ ${L_DATE} -le ${LE_DATE} ]]; do
      ...
      export L_HH=$(echo $L_DATE | cut -c9-10)
#
      export FILE_PATH=${KORUS_WRFCHEMI_DIR}
...
# PERTURB CHEM ICBC
   ...
   cp ${PERT_CHEM_INPUT_DIR}/random_correlated_perts_mirror.py ./random.py
   ...
   cp ${PERT_CHEM_INPUT_DIR}/set0_KORUS ./set0
   cp ${PERT_CHEM_INPUT_DIR}/set00_KORUS ./set00
#
...
#  if [[ ${YYYY} -eq 2014 ]]; then export MOZBC_DATA=h0005.nc; fi
  export MOZBC_DATA=/cesm122_fsdssoa_f09_2016a.cam.h2.${YYYY}-${MM}-${DD}-00000.nc
#
...
#   ncks -A ${KORUS_DUST_DIR}/EROD_d${CR_DOMAIN} ${WRFINPEN}
...
#   ncks -A ${REAL_DIR}/${WRFINPEN} ${WRFINPEN}
##   ncks -A ${KORUS_DUST_DIR}/EROD_d${FR_DOMAIN} ${WRFINPEN}
#
# LOOP THROUGH ALL MEMBERS IN THE ENSEMBLE
   let MEM=1
   while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
...
# COPY PERTURBATION CODE
   if [[ -e perturb_chem_emiss_CORR_RT_CONST.exe ]]; then rm -rf perturb_chem_emiss_CORR_RT_CONST.exe; fi
...
ch_chem_spc='E_ACET','E_BENZENE','E_BIGALK','E_BIGENE','E_C10H16','E_C2H2','E_C2H4','E_C2H5OH','E_C2H6','E_C3H6','E_C3H8','E_CH2O','E_CH3CHO','E_CH3COCH3','E_CH3OH','E_CLI','E_CLJ','E_CO','E_CO_A','E_CO_BB','E_ECI','E_ECJ','E_GLY','E_HCOOH','E_HONO','E_ISOP','E_MACR','E_MEK','E_MGLY','E_MVK','E_NAI','E_NAJ','E_NH3','E_NH4I','E_NH4J','E_NO','E_NO2','E_NO3I','E_NO3J','E_ORGI','E_ORGI_A','E_ORGI_BB','E_ORGJ','E_ORGJ_A','E_ORGJ_BB','E_PM25I','E_PM25J','E_PM_10','E_SO2','E_SO4I','E_SO4J','E_SULF','E_TOLUENE','E_XYLENE',
...
#&perturb_chem_emiss_spec_nml
#ch_chem_spc='E_ACET','E_BENZENE','E_BIGALK','E_BIGENE','E_C10H16','E_C2H2','E_C2H4','E_C2H5OH','E_C2H6','E_C3H6','E_C3H8','E_CH2O','E_CH3CHO','E_CH3COCH3'#,'E_CH3OH','E_CLI','E_CLJ','E_CO','E_CO_A','E_CO_BB','E_ECI','E_ECJ','E_GLY','E_HCOOH','E_HONO','E_ISOP','E_MACR','E_MEK','E_MGLY','E_MVK','E_NAI','E_NAJ'#,'E_NH3','E_NH4I','E_NH4J','E_NO','E_NO2','E_NO3I','E_NO3J','E_ORGI','E_ORGI_A','E_ORGI_BB','E_ORGJ','E_ORGJ_A','E_ORGJ_BB','E_PM25I','E_PM25J','E_PM_10',#'E_SO2','E_SO4I','E_SO4J','E_SULF','E_TOLUENE','E_XYLENE',
#ch_fire_spc='ebu_in_co','ebu_in_no','ebu_in_so2','ebu_in_bigalk','ebu_in_bigene','ebu_in_c2h4','ebu_in_c2h5oh','ebu_in_c2h6','ebu_in_c3h8','ebu_in_c3h6','#ebu_in_ch2o','ebu_in_ch3cho','ebu_in_ch3coch3','ebu_in_ch3oh','ebu_in_mek','ebu_in_toluene','ebu_in_nh3','ebu_in_no2','ebu_in_open','ebu_in_c10h16','ebu_i#n_ch3cooh','ebu_in_cres','ebu_in_glyald','ebu_in_mgly','ebu_in_gly','ebu_in_acetol','ebu_in_isop','ebu_in_macr','ebu_in_mvk','ebu_in_oc','ebu_in_bc',
#ch_bio_spc='MSEBIO_ISOP',
#/
#EOF
##
...
# SET MOPITT PARAMETERS
   export MOPITT_FILE_PRE=MOP02R-
   export MOPITT_FILE_EXT=-L2V10.2.3.beta.hdf   
...
# SET MOPITT INPUT DATA DIR
   if [[ ${BIN_END} -ne 3 ]]; then
      export MOP_INFILE="'"${KORUS_MOPITT_CO_DIR}/${MOPITT_FILE_PRE}${ASIM_MX_YYYY}${ASIM_MX_MM}${ASIM_MX_DD}${MOPITT_FILE_EXT}"'"
   else
      export FLG=1
      export BIN_END=24
      export MOP_INFILE="'"${KORUS_MOPITT_CO_DIR}/${MOPITT_FILE_PRE}${ASIM_MN_YYYY}${ASIM_MN_MM}${ASIM_MN_DD}${MOPITT_FILE_EXT}"'"
   fi
#
# COPY EXECUTABLE
...
# Normal cases (06Z, 12Z, 18Z)
      ...
      while [[ ${A_DATE} -le ${ASIM_MAX_DATE} ]]; do 
         ...
         while [[ ${ICNT} -le ${NCNT} ]]; do
            ...
            export FILE=`ls ${KORUS_IASI_CO_DIR}/${A_YY}${A_MM}/${A_DD}/${FILE_PRE}_${A_YY}${A_MM}${A_DD}${A_HH}*Z_${ND_YY}${ND_MM}${ND_DD}${ND_HH}*Z_*`
            echo ${FILE}
            if [[ -e ${FILE} ]]; then 
...
if ${RUN_IASI_O3_OBS}; then
   if [[ ! -d ${RUN_DIR}/${DATE}/iasi_o3_obs ]]; then
      mkdir ${RUN_DIR}/${DATE}/iasi_o3_obs
...
if ${RUN_MODIS_AOD_OBS}; then
   if [[ ! -d ${RUN_DIR}/${DATE}/modis_aod_obs ]]; then
      mkdir ${RUN_DIR}/${DATE}/modis_aod_obs
...
# DART time data (no leading zeros)
   ...
   while [[ ${L_DATE} -le ${E_DATE} ]]; do
      ...
      export L_DD=$(echo $L_DATE | cut -c7-8)
      export L_HH=$(echo $L_DATE | cut -c9-10)
####      cp ${KORUS_PREPBUFR_DIR}/${L_YYYY}${L_MM}${L_DD}${L_HH}/prepbufr.gdas.${L_YYYY}${L_MM}${L_DD}${L_HH}.wo40.be prepqm${L_YY}${L_MM}${L_DD}${L_HH}
      cp ${KORUS_PREPBUFR_DIR}/${L_YYYY}${L_MM}${L_DD}/${L_YYYY}${L_MM}${L_DD}.nr/prepbufr.gdas.${L_YYYY}${L_MM}${L_DD}.t${L_HH}z.nr prepqm${L_YY}${L_MM}${L_DD}${L_HH}
      export L_DATE=$(${BUILD_DIR}/da_advance_time.exe ${L_DATE} +6 2>/dev/null)
   done
#
# GET DART input.nml
...
# MET OBS
#IB   if [[ -e ${PREPBUFR_MET_OBS_DIR}/obs_seq_${DATE}.out && ${RUN_MET_OBS} ]]; then 
...
if [[ \$RC = 0 ]]; then
   touch WRFCHEM_SUCCESS_${JOBRND}
else
   touch WRFCHEM_FAILED_${JOBRND} 
   exit
fi
EOF
#
...
if [[ \$RC = 0 ]]; then
   touch WRF2DART_SUCCESS_${JOBRND}
else
   touch WRF2DART_FAILED_${JOBRND} 
   exit
fi
EOF
#
...
if [[ \$RC = 0 ]]; then
   touch DART2WRF_SUCCESS_${JOBRND}
else
   touch DART2WRF_FAILED_${JOBRND}
   exit
fi
EOF
#
