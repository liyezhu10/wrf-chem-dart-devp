...
#
# NOTE: To generate the chemistry perturbations check NL_SW_GENERUN_IASIATE comments
...
export EXPERIMENT_DATA_DIR=${ACD_DIR}/FRAPPE_REAL_TIME_DATA
...
export SINGLE_CORES=1
export SINGLE_PROCS=1
...
#
   ...
   ${HYBRID_SCRIPTS_DIR}/job_script.ksh ${JOBRND} ${PROJ_NUMBER} ${GENERAL_TIME_LIMIT} ${GENERAL_JOB_CLASS} ${GENERAL_NODES} ${GENERAL_CORES} ${GENERAL_PROCS} geogrid.exe
...
#PBS -N ${JOBRND}
#PBS -A ${PROJ_NUMBER}
#PBS -l walltime=${SINGLE_TIME_LIMIT}
#PBS -q ${SINGLE_JOB_CLASS}
#PBS -j oe
#PBS -l select=${SINGLE_NODES}:ncpus=${SINGLE_CORES}:mpiprocs=${SINGLE_PROCS}
export MPI_SHEPERD=true 
...
#PBS -j oe
#PBS -l select=${FILTER_NODES}:ncpus=${FILTER_CORES}:mpiprocs=${FILTER_PROCS}:mem=109GB
