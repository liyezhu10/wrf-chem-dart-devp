...
# DEPENDENT INPUT DATA DIRECTORIES:
   ...
   export WRF_CHEM_WORK_DIR=${DART_DIR}/models/wrf_chem/work
...
#
   cp ${WRF_CHEM_WORK_DIR}/advance_time ./.
   cp ${WRF_CHEM_WORK_DIR}/input.nml ./.
...
# CALCULATE GREGORIAN TIMES FOR START AND END OF ASSIMILATION WINDOW
   set -A GREG_DATA `echo $DATE 0 -g | ${WRF_CHEM_WORK_DIR}/advance_time`
   export DAY_GREG=${GREG_DATA[0]}
   export SEC_GREG=${GREG_DATA[1]}
   set -A GREG_DATA `echo $NEXT_DATE 0 -g | ${WRF_CHEM_WORK_DIR}/advance_time`
   ...
   set -A temp `echo $ASIM_MIN_DATE 0 -g | ${WRF_CHEM_WORK_DIR}/advance_time`
   export ASIM_MIN_DAY_GREG=${temp[0]}
   export ASIM_MIN_SEC_GREG=${temp[1]}
   set -A temp `echo $ASIM_MAX_DATE 0 -g | ${WRF_CHEM_WORK_DIR}/advance_time` 
...
# &model_nml
   ...
   export WRFFIRECHEMI_DARTVARS="ebu_in_co,ebu_in_no,ebu_in_oc,ebu_in_bc,ebuin_c2h4,ebu_in_ch2o,ebu_in_ch3oh"
...
# LOOP THROUGH ALL MEMBERS IN THE ENSEMBLE
      ...
      while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
         ...
         rm -rf pert_wrf_bc
         cp ${WRF_CHEM_WORK_DIR}/pert_wrf_bc ./.
         rm -rf input.nml
...
#
# GET EXECUTABLES
      cp ${WRF_CHEM_WORK_DIR}/obs_sequence_tool ./.
      export NUM_FILES=0
#
...
#
# GET DART UTILITIES
      cp ${WRT_CHEM_WORK_DIR}/wrf_dart_obs_preprocess ./.
...
IMPACT
  ...
  chem_vars met_vars 0.0
  NOT_chem_obs chem_vars 0.0
  CO_obs NOT_CO_vars 0.0
  O3_obs NOT_O3_vars 0.0
  NOx_obs NOT_NOx_vars 0.0
  SO2_obd NOT_SO2_vars 0.0
  PM10_obs NOT_PM10_vars 0.0 
  PM25_obs NOT_PM25_vars 0.0 
  AOD_obs NOT_AOD_vars 0.0 
  CO_obs CO_vars 1.0
  O3_obs O3_vars 1.0
  NOx_obs NOx_vars 1.0
  SO2_obd SO2_vars 1.0
...
# Copy the obs_impact_tool executable
      cp ${WRF_CHEM_WORK_DIR}/obs_impact_tool ./.
#
...
#
# Get DART and static files
      cp ${WRF_CHEM_WORK_DIR}/filter      ./.
      cp ${WRF_CHEM_WORK_DIR}/advance_time ./.
      cp ${WRF_CHEM_WORK_DIR}/input.nml ./.
...
# Copy DART file that controls the observation/state variable update localization
      cp ${LOCALIZATION__DIR}/control_impact_runtime.table ./.
#
# Create namelist
      ${DART_DIR}/models/wrf_chem/namelist_scripts/DART/dart_create_input.nml.ksh
#
...
#
# Construction obs/state variable localization table
...
# Copy the adjusted emissions fields from the wrfinput files to the emissions input files
         ncks -x -v ${WRFCHEMI_DARTVARS} wrfout_d${CR_DOMAIN}_temp.${CMEM} wrfout_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM}
         ncks -O -v ${WRFCHEMI_DARTVARS} wrfout_d${CR_DOMAIN}_temp.${CMEM} wrfchemi_d${CR_DOMAIN}_${LL_FILE_DATE}.${CMEM}
         ncks -O -v ${WRFFIRECHEMI_DARTVAR} wrfout_d${CR_DOMAIN}_temp.${CMEM} wrffirechemi_d${CR_DOMAIN}_${LL_FILE_DATE}.${CMEM}
...
# Get WRF-Chem parameter files
         cp ${WRF_CHEM_WORK_DIR}/advance_time ./.
         cp ${WRF_CHEM_WORK_DIR}/work/input.nml ./.
         cp ${WRFCHEM_DIR}/test/em_real/wrf.exe ./.
         cp ${WRFCHEM_DIR}/test/em_real/aerosol.formatted ./.
         cp ${WRFCHEM_DIR}/test/em_real/aerosol_lat.formatted ./.
         cp ${WRFCHEM_DIR}/test/em_real/aerosol_lon.formatted ./.
