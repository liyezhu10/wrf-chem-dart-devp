#!/bin/ksh -aux
. /glade/u/apps/ch/opt/Lmod/default/lmod/lmod/init/sh
#
#########################################################################
#
...
#
# ROOT DIRECTORIES:
export SCRATCH_DIR=/glade/scratch/fossell/DART/chem_tutorial
export WORK_DIR=/glade/work/fossell/DART
export ACD_DIR=${SCRATCH_DIR}
#
...
export EXPERIMENT_DATA_DIR=${ACD_DIR}/PANDA_REAL_TIME_DATA
...
# DO INTERPOLATION
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      export TIME_INTERP_DIR1=/glade/work/fossell/DART/wrf-chem.r13172/models/wrf_chem
...
./megan_bio_emiss < megan_bio_emiss.inp > index.html 2>&1 
export RC=\$?
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# JOB SCRIPT
   ...
   touch job.ksh
   RANDOM=$$
   export JOBRND=${RANDOM}_icbc_pert
   cat << EOF > job.ksh
#!/bin/ksh -aeux
#PBS -N ${JOBRND}
#PBS -A ${PROJ_NUMBER}
...
# RUN PERTURBATION CODE    
   ...
   touch job.ksh
   RANDOM=$$
   export JOBRND=${RANDOM}_cr_emis_pert
   cat << EOF > job.ksh
#!/bin/ksh -aeux
#PBS -N ${JOBRND}
#PBS -A ${PROJ_NUMBER}
...
#   touch job.ksh
#   RANDOM=$$
#   export JOBRND=${RANDOM}_fr_emis_pert
#   cat << EOF > job.ksh
##!/bin/ksh -aeux
##PBS -N ${JOBRND}
##PBS -A ${PROJ_NUMBER}
