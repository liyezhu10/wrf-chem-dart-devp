...
#
# CODE VERSIONS:
export WPS_VER=WPSV3.9.1_dmpar
...
export RUN_DIR=${EXPERIMENT_DIR}/real_PANDA_RETR_VARLOC_RELEASE_TEST
...
export NL_CONV_STATE_VARIABLES="'U',     'QTY_U_WIND_COMPONENT',     'TYPE_U',  'UPDATE','999',
          ...
          'BC1','QTY_CB1',                     'TYPE_EXTCOF','UPDATE','999',
          'BC2','QTY_CB2',                     'TYPE_EXTCOF','UPDATE','999',
          'OC1','QTY_OC1',                     'TYPE_EXTCOF','UPDATE','999',
          'OC2','QTY_OC2',                     'TYPE_EXTCOF','UPDATE','999',
          ...
          'PM10','QTY_PM10',                   'TYPE_EXTCOF','UPDATE','999',
          'PM2_5_DRY','QTY_PM25' ,             'TYPE_EXTCOF','UPDATE','999',
          'P10','QTY_PM10',                    'TYPE_EXTCOF','UPDATE','999',
          'P25','QTY_PM25',                    'TYPE_EXTCOF','UPDATE','999',
          'SEAS_1','QTY_SSLT01',               'TYPE_EXTCOF','UPDATE','999',
          'SEAS_2','QTY_SSLT02',               'TYPE_EXTCOF','UPDATE','999',
          'SEAS_3','QTY_SSLT03',               'TYPE_EXTCOF','UPDATE','999',
          'SEAS_4','QTY_SSLT04',               'TYPE_EXTCOF','UPDATE','999'"
...
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=${GENERAL_CORES}:mpiprocs=${GENERAL_PROCS}
mpiexec_mpt ./geogrid.exe  > index.html 2>&1 
export RC=\$?     
...
#PBS -q ${GENERAL_JOB_CLASS}
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=1:mpiprocs=1
mpiexec_mpt ./ungrib.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=${GENERAL_CORES}:mpiprocs=${GENERAL_PROCS}
mpiexec_mpt ./metgrid.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=${GENERAL_CORES}:mpiprocs=${GENERAL_PROCS}
mpiexec_mpt ./real.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#PBS -j oe
#PBS -l select=${WRFDA_NODES}:ncpus=${WRFDA_CORES}:mpiprocs=${WRFDA_PROCS}
mpiexec_mpt ./perf_wrf_bc.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#PBS -N ${JOBRND}
#PBS -A ${PROJ_NUMBER}
#PBS -l walltime=${BIO_TIME_LIMIT}
#PBS -q ${BIO_JOB_CLASS}
#PBS -j oe
#PBS -l select=${BIO_NODES}:ncpus=${BIO_CORES}:mpiprocs=${BIO_PROCS}
...
#
# generate emissions pertubation 
      export NL_SW_GENERATE=false
      if [[ ${DATE} -eq ${INITIAL_DATE} && ${PERT_CHEM_GENER} == true ]]; then export NL_SW_GENERATE=true; fi
#
...
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=${GENERAL_CORES}:mpiprocs=${GENERAL_PROCS}
./perturb_chem_emiss_CORR_RT_CONST.exe > index.html 2>&1
export RC=\$?
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#
# GET EXECUTABLE
   cp ${DART_DIR}/observations/obs_converters/IASI_CO/work/iasi_ascii_to_obs ./.
...
#
# GET EXECUTABLE
   cp ${DART_DIR}/observations/obs_converters/IASI_O3/work/iasi_ascii_to_obs ./.
   ./iasi_ascii_to_obs > index.html 2>&1  
#
# COPY OUTPUT TO ARCHIVE LOCATION
   export IASI_FILE=iasi_obs_seq${D_DATE}
...
#
 #KRF no more dart_to_wrf
 if [[ ${manhattan} = "false" ]]; then
# Run WRF_TO_DART
   let MEM=1
   while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
      export CMEM=e${MEM}
...
#PBS -j oe
#PBS -l select=${GENERAL_NODES}:ncpus=${GENERAL_CORES}:mpiprocs=${GENERAL_PROCS}
mpiexec_mpt ./wrf_to_dart > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# KRF: Loop through members, link, copy background files, create input/output lists
   let MEM=1
   while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
      export CMEM=e${MEM}
      export KMEM=${MEM}
      ...
      # KRF Link background wrfout file to wrfinput for filter to use.
      # Copy the same background wrfout file to the filter output name so
      # fiter can overwrite it and retain propoer metadata.
      ln -sf ${BACKGND_FCST_DIR}/run_${CMEM}/wrfout_d${CR_DOMAIN}_${FILE_DATE} wrfinput_d${CR_DOMAIN}_${CMEM}
      cp ${BACKGND_FCST_DIR}/run_${CMEM}/wrfout_d${CR_DOMAIN}_${FILE_DATE} wrfout_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM}
      ...
      echo wrfout_d${CR_DOMAIN}_${FILE_DATE}_filt.${CMEM} >> output_list.txt
...
# APM: ---
#KRF test copying from ../wrfchem_fire and ../wrfchem_chemi since wrf-to-dart isn't run anymore
 if [[ ${LL_DATE} -le ${FIRST_EMISS_INV_DATE} || ${ADD_EMISS} = ".false." ]]; then
    cp ${WRFCHEM_CHEM_EMISS_DIR}/wrfchemi_d${CR_DOMAIN}_${LL_FILE_DATE}.e001 wrfchemi_d${CR_DOMAIN}
    cp ${WRFCHEM_CHEM_EMISS_DIR}/wrffirechemi_d${CR_DOMAIN}_${LL_FILE_DATE}.e001 wrffirechemi_d${CR_DOMAIN}
else
    cp ${BACKGND_FCST_DIR}/run_e001/wrfchemi_d${CR_DOMAIN}_${LL_FILE_DATE} wrfchemi_d${CR_DOMAIN}
    cp ${BACKGND_FCST_DIR}/run_e001/wrffirechemi_d${CR_DOMAIN}_${LL_FILE_DATE} wrffirechemi_d${CR_DOMAIN}  
fi
...
# KRF What else to check for Manhattan?
...
#PBS -j oe
#PBS -l select=${FILTER_NODES}:ncpus=${FILTER_CORES}:mpiprocs=${FILTER_PROCS}
mpiexec_mpt ./dart_to_filter > index.html 2>&1 
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
   touch SUCCESS
...
#PBS -j oe
#PBS -l select=${WRFCHEM_NODES}:ncpus=${WRFCHEM_CORES}:mpiprocs=${WRFCHEM_PROCS}
mpiexec_mpt ./wrf.exe > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
