...
export DATE_P=2008072012
export DATE_P=2008072018
...
#
   ...
   export NL_START_YEAR=$(echo $L_START_DATE | cut -c1-4)
   export NL_START_MONTH=$(echo $L_START_DATE | cut -c5-6)
   export NL_START_DAY=$(echo $L_START_DATE | cut -c7-8)
   export NL_START_HOUR=$(echo $L_START_DATE | cut -c9-10)
   export NL_END_YEAR=$(echo $L_END_DATE | cut -c1-4)
   export NL_END_MONTH=$(echo $L_END_DATE | cut -c5-6)
   export NL_END_DAY=$(echo $L_END_DATE | cut -c7-8)
   export NL_END_HOUR=$(echo $L_END_DATE | cut -c9-10)
   export NL_START_DATE="'"${L_START_YEAR}-${L_START_MONTH}-${L_START_DAY}_${L_START_HOUR}:00:00"'"
   export NL_END_DATE="'"${L_END_YEAR}-${L_END_MONTH}-${L_END_DAY}_${L_END_HOUR}:00:00"'"
   ${HYBRID_SCRIPTS_DIR}/da_create_wps_namelist.ksh
#
...
#
   ln -fs ${GEOGRID_DIR}/geo_em.d${DOMAIN}.nc ./.
   ln -fs ../ungrib/FILE:* ./.
...
# LINK IN THE METGRID FILES
   ...
   while [[ ${P_DATE} -le ${P_END_DATE} ]] ; do
      ...
      ln -sf ${RUN_DIR}/${DATE}/metgrid/met_em.d${DOMAIN}.${P_FILE_DATE} ./.
...
#
      bsub -K < job.ksh 
      mv wrfinput_d${DOMAIN} wrfinput_d${DOMAIN}.$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} 0 -W 2>/dev/null)
      mv wrfbdy_d${DOMAIN} wrfbdy_d${DOMAIN}.$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} 0 -W 2>/dev/null)
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done   
fi
#
...
# SET WRFDA PARAMETERS
      ...
      export DA_INPUT_FILE=../real/wrfinput_d${DOMAIN}
...
#
         ...
         cp wrfvar_output wrfinput_d${DOMAIN}.${ANALYSIS_DATE}.${CMEM}
         let MEM=${MEM}+1
      done
      export P_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} ${LBC_FREQ} 2>/dev/null) 
   done
...
# LOOP THROUGH ALL MEMBERS IN THE ENSEMBLE
   ...
   while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
      ...
      if [[ -f wrfbdy_this ]]; then
         ...
         export DA_BDY_FILE=${DA_BDY_PATH}/wrfbdy_d${DOMAIN}.${ANALYSIS_DATE}
...
# LOOP THROUGH ALL BDY TENDENCY TIMES FOR THIS MEMBER.
      ...
      while [[ ${L_DATE} -lt ${L_END_DATE} ]]; do
         ...
         ln -fs ${DA_INPUT_PATH}/wrfinput_d${DOMAIN}.${ANALYSIS_DATE}.${CMEM} wrfinput_this 
         ln -fs ${DA_INPUT_PATH}/wrfinput_d${DOMAIN}.${NEXT_ANALYSIS_DATE}.${CMEM} wrfinput_next 
#
# JOB SCRIPT 
         if [[ -f job.ksh ]]; then rm -rf job.ksh; fi
         rm -rf *.err
...
#
      ...
      export ANALYSIS_DATE=$(${BUILD_DIR}/da_advance_time.exe ${DATE} 0 -W 2>/dev/null)
      mv wrfbdy_this wrfbdy_d${DOMAIN}.${ANALYSIS_DATE}.${CMEM}
      let MEM=${MEM}+1
   done
fi
#
...
# TEST WHETHER OUTPUT EXISTS
   export FILE=exo_coldens_d${DOMAIN}
   if [[ ! -e ${FILE} ]]; then
...
#
# TEST WHETHER OUTPUT EXISTS
   export FILE=wrf_season_wes_usgs_d${DOMAIN}.nc
   if [[ ! -e ${FILE} ]]; then
      echo WESELY FAILED
      exit
   else
...
# LINK NEEDED FILES
      ...
      export FILE=wrfbiochemi_d${DOMAIN}
...
#
# TEST WHETHER OUTPUT EXISTS
      export FILE=wrffirechemi_d${DOMAIN}_${L_FILE_DATE}
      if [[ ! -e ${FILE} ]]; then
         echo WRFFIRE FAILED
         exit
      else
...
#
   while [[ ${L_DATE} -le ${LE_DATE} ]]; do
      ...
      export L_HH=$(echo $L_DATE | cut -c9-10)
#
      cp ${ACD_DIR}/AVE_TEST_DATA/chem_static_100km/${L_YYYY}${L_MM}${L_DD}/wrfchemi_d${DOMAIN}_${L_YYYY}-${L_MM}-${L_DD}_${L_HH}:00:00 ./.
      export P_DATE=${L_DATE}
      export L_DATE=$(${BUILD_DIR}/da_advance_time.exe ${P_DATE} 1 2>/dev/null)
   done
fi
...
met_file_separator= '.'
EOF
   cp ${MET_EM_DIR}/${DATE}/met_em.d${DOMAIN}.*:00:00.nc ./.
...
# COMBINE WRFCHEM WITH WRF
      export WRFINPEN=wrfinput_d${DOMAIN}.${YYYY}-${MM}-${DD}_${HH}:00:00.${CMEM}
      export WRFBDYEN=wrfbdy_d${DOMAIN}.${YYYY}-${MM}-${DD}_${HH}:00:00.${CMEM}
      if [[ -e wrfinput_d${DOMAIN} ]]; then rm -rf wrfinput_d${DOMAIN}; fi
      if [[ -e ${WRFINPEN} ]]; then rm -rf ${WRFINPEN}; fi
      if [[ -e wrfbdy_d${DOMAIN} ]]; then rm -rf wrfbdy_d${DOMAIN}; fi
      if [[ -e ${WRFBDYEN} ]]; then rm -rf ${WRFBDYEN}; fi
      cp ${WRFCHEM_MET_IC_DIR}/${WRFINPEN} wrfinput.met
      cp ${WRFCHEM_MET_BC_DIR}/${WRFBDYEN} wrfbdy.met
      ncks -x -v MU,PSFC,Q2,T2,TH2,U10,V10,P,QVAPOR,T,U,V ${WRFCHEM_MET_IC_DIR}/${WRFINPEN} wrfinput_d${DOMAIN}
      ncks -v MU,PSFC,Q2,T2,TH2,U10,V10,P,QVAPOR,T,U,V wrfinput.met ${WRFINPEN}
      ncks -A wrfinput_d${DOMAIN} ${WRFINPEN}
#
      ncks -x -v MU_BTXE,MU_BTXS,MU_BTYE,MU_BTYS,MU_BXE,MU_BXS,MU_BYE,MU_BYS,QVAPOR_BTXE,QVAPOR_BTXS,QVAPOR_BTYE,QVAPOR_BTYS,QVAPOR_BXE,QVAPOR_BXS,QVAPOR_BYE,QVAPOR_BYS,T_BTXE,T_BTXS,T_BTYE,T_BTYS,T_BXE,T_BXS,T_BYE,T_BYS,U_BTXE,U_BTXS,U_BTYE,U_BTYS,U_BXE,U_BXS,U_BYE,U_BYS,V_BTXE,V_BTXS,V_BTYE,V_BTYS,V_BXE,V_BXS,V_BYE,V_BYS,PH_BTXE,PH_BTXS,PH_BTYE,PH_BTYS,PH_BXE,PH_BXS,PH_BYE,PH_BYS ${WRFCHEM_MET_BC_DIR}/${WRFBDYEN} wrfbdy_d${DOMAIN}
      ...
      ncks -A wrfbdy_d${DOMAIN} ${WRFBDYEN}
      rm -rf wrfinput.met
      rm -rf wrfbdy.met
      let MEM=MEM+1
   done
...
# GET EMISSON FILES FOR THIS MEMBER
         export WRFCHEMI=wrfchemi_d${DOMAIN}_${L_YYYY}-${L_MM}-${L_DD}_${L_HH}:00:00
         export WRFFIRECHEMI=wrffirechemi_d${DOMAIN}_${L_YYYY}-${L_MM}-${L_DD}_${L_HH}:00:00
         export WRFBIOCHEMI=wrfbiochemi_d${DOMAIN}_${L_YYYY}-${L_MM}-${L_DD}_${L_HH}:00:00
#
...
# GET WRFINPUT TEMPLATE
   cp ${WRFCHEM_MET_IC_DIR}/wrfinput_d${DOMAIN}.${FILE_DATE}.e001 wrfinput_d${DOMAIN}
#
# GET DART UTILITIES
...
#
      ...
      cp ${EXO_COLDENS_DIR}/exo_coldens_d${DOMAIN} ./.
      cp ${SEASONS_WES_DIR}/wrf_season_wes_usgs_d${DOMAIN}.nc ./.
#
# Get WRF-Chem emissions files

      cp ${WRFCHEM_CHEM_EMISS_DIR}/wrfbiochemi_d${DOMAIN}_${START_FILE_DATE}.${CMEM} wrfbiochemi_d${DOMAIN}_${START_FILE_DATE}
      ...
      while [[ ${L_DATE} -le ${END_DATE} ]]; do
         ...
         cp ${WRFCHEM_CHEM_EMISS_DIR}/wrffirechemi_d${DOMAIN}_${L_FILE_DATE}.${CMEM} wrffirechemi_d${DOMAIN}_${L_FILE_DATE}
         cp ${WRFCHEM_CHEM_EMISS_DIR}/wrfchemi_d${DOMAIN}_${L_FILE_DATE}.${CMEM} wrfchemi_d${DOMAIN}_${L_FILE_DATE}
         export L_DATE=$(${BUILD_DIR}/da_advance_time.exe ${L_DATE} +1 2>/dev/null)
      done
#
# Get WR-Chem input and bdy files
      cp ${WRFCHEM_CHEM_ICBC_DIR}/wrfinput_d${DOMAIN}.${START_FILE_DATE}.${CMEM} wrfinput_d${DOMAIN}
      cp ${WRFCHEM_CHEM_ICBC_DIR}/wrfbdy_d${DOMAIN}.*.${CMEM} wrfbdy_d${DOMAIN}
#
# Create WRF-Chem namelist.input 
      ${HYBRID_SCRIPTS_DIR}/da_create_wrfchem_namelist.ksh
#
...
# Wait for WRFCHEM to complete for each member
   ${HYBRID_SCRIPTS_DIR}/da_run_hold.ksh ${JOBRND}
fi
#
#########################################################################
#
...
# &wrf_to_dart_nml
      ...
      cp ${BACKGND_FCST_DIR}/run_${CMEM}/wrfout_d${DOMAIN}_${FILE_DATE} wrfinput_d${DOMAIN}
#
# Create job script 
      rm -rf job.ksh
      touch job.ksh
...
#
export RC=\$?     
rm -rf WRF2DART_SUCCESS     
rm -rf WRF2DART_FAILED          
if [[ \$RC = 0 ]]; then
   touch WRF2DART_SUCCESS
else
   touch WRF2DART_FAILED 
   exit
fi
EOF
#
...
#
   cd ${RUN_DIR}/${DATE}/dart_filter
   ${DART_DIR}/models/wrf_chem/namelist_scripts/DART/dart_create_input.nml.ksh
   cp ${BACKGND_FCST_DIR}/run_e001/wrfout_d${DOMAIN}_${FILE_DATE} wrfinput_d${DOMAIN}
#
...
#
export RC=\$?     
rm -rf DART2WRF_SUCCESS
rm -rf DART2WRF_FAILED
if [[ \$RC = 0 ]]; then
   touch DART2WRF_SUCCESS
else
   touch DART2WRF_FAILED
   exit
fi
EOF
#
# Submit convert file script for each and wait until job completes
      bsub < job.ksh 
      cp wrfinput_d${DOMAIN} ../wrfout_d${DOMAIN}.${FILE_DATE}_filt.${CMEM} 
      let MEM=${MEM}+1
   done
#
...
#
   ...
   while [[ ${MEM} -le ${NUM_MEMBERS} ]]; do
      ...
      if [[ ${MEM} -lt 10 ]]; then export KMEM=000${MEM}; export CMEM=e00${MEM}; fi
#
      export OPS_FORC_FILE=${WRFCHEM_CHEM_ICBC_DIR}/wrfinput_d${DOMAIN}.${FILE_DATE}.${CMEM}
      export BDYCDN_IN=${WRFCHEM_CHEM_ICBC_DIR}/wrfbdy_d${DOMAIN}.${FILE_DATE}.${CMEM}
      cp ${BDYCDN_IN} wrfbdy_d${DOMAIN}_${FILE_DATE}_prior.${CMEM}
      export DA_OUTPUT_FILE=${DART_FILTER_DIR}/wrfout_d${DOMAIN}.${FILE_DATE}_filt.${CMEM} 
      export BDYCDN_OUT=wrfbdy_d${DOMAIN}.${FILE_DATE}_filt.${CMEM}    
...
#
# Get WR-Chem input and bdy files
      cp ${DART_FILTER_DIR}/wrfout_d${DOMAIN}.${START_FILE_DATE}_filt.${CMEM} wrfinput_d${DOMAIN}
      cp ${UPDATE_BC_DIR}/wrfbdy_d${DOMAIN}.${START_FILE_DATE}_filt.${CMEM} wrfbdy_d${DOMAIN}
#
# Create WRF-Chem namelist.input 
      ${HYBRID_SCRIPTS_DIR}/da_create_wrfchem_namelist.ksh
#
...
#
# NDOWN ENSEMBLE MEAN TO FINE REOLUTION GRID
#
# RUN COARSE RESOLUTION ENSEMBLE FORECASTS
#
# RUN FINE RESOLUTION DETERMINISTIC FORECAST
#
#########################################################################
