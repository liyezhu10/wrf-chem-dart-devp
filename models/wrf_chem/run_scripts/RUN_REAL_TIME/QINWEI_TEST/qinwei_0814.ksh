...
export WRF_VER=WRF_no_chem
export WRFCHEM_VER=WRF_chem
export DART_VER=WRFChem_DART
#
# ROOT DIRECTORIES:
export SCRATCH_DIR=/home/zhangqw/wrf_v3.6.1
export WORK_DIR=/home/zhangqw/wrf_v3.6.1
export INPUT_DATA_DIR=/home/zhangqw/wrf_v3.6.1
#
...
export DART_DIR=${TRUNK_DIR}/${DART_VER}/wrf-chem.r12769
...
export EXPERIMENT_DATA_DIR=${INPUT_DATA_DIR}/PANDA_REAL_TIME_DATA_TARDIR
...
export EXPERIMENT_GFS_DIR=${EXPERIMENT_DATA_DIR}/gfs_forecasts_TARDIR
...
export WRFCHEM_TIME_LIMIT=03:00:00
export WRFCHEM_NODES=1
export WRFCHEM_TASKS=12
#
# RUN DIRECTORIES
...
#export NL_GEOG_DATA_RES=\'usgs_30s+default\',\'usgs_30s+default\' for V3.9
export NL_DX=${DX_CR}
export NL_DY=${DX_CR}
...
export NL_AUXINPUT2_INTERVAL_M=10800,10800
...
export NL_J_PARENT_START=${JSTR_CR},${JSTR_FR}
export NL_PARENT_GRID_RATIO=1,3
export NL_PARENT_TIME_STEP_RATIO=1,3
...
export NL_BIOEMDT=6,6
export NL_PHOTDT=6,6
export NL_CHEMDT=6,6
...
export NL_ADV_MOD_COMMAND="'mpirun -np 8 ./wrf.exe'"
...
# JOB SCRIPT
      ...
      cat << EOF > job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./real.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# JOB SCRIPT 
         ...
         cat << EOF > job.ksh
#!/bin/ksh -aeux
mpirun  -np 8 ./da_wrfvar.exe  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# JOB SCRIPT 
         ...
         cat << EOF > job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./pert_wrf_bc  > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# Create job script for this member and run it 
      ...
      cat << EOF >job.ksh
#!/bin/ksh -aeux
mpirun -np 8  ./wrf.exe > index_${KMEM} 2>&1 
export RC=\$?
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
#
      ...
      cat << EOF >job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./wrf_to_dart > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# Create job script for this member and run it 
   ...
   cat << EOF >job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./filter > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
...
# Create job script 
      ...
      cat << EOF >job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./dart_to_wrf > index.html 2>&1 
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
   touch SUCCESS
...
# Wait for dart_to_wrf to complete for each member
   cd ${RUN_DIR}/${DATE}/dart_filter
 #  ${HYBRID_SCRIPTS_DIR}/da_run_#hold_cu.ksh ${TRANDOM}
#
...
# Create job script for this member and run it 
      ...
      cat << EOF >job.ksh
#!/bin/ksh -aeux
mpirun -np 8 ./wrf.exe > index.html 2>&1 
export RC=\$?     
if [[ -f SUCCESS ]]; then rm -rf SUCCESS; fi     
if [[ -f FAILED ]]; then rm -rf FAILED; fi          
if [[ \$RC = 0 ]]; then
