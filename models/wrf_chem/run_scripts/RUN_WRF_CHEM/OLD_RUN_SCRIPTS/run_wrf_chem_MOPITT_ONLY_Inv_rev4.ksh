...
export END_DATE=2008060300
export INITIAL_DATE=2008060112
export INITIAL_FILE_DATE=2008-06-01_12:00:00
export FIRST_FILTER_DATE=2008060118
...
export OBS_DIR=obs
#if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export OBS_DIR=obs_MOPITT_Inv; fi
if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export OBS_DIR=obs_MOPITT_Cx_Std_SVD; fi
#if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export OBS_DIR=obs_MOPITT; fi
#if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export OBS_DIR=obs_MOPCOMB; fi
...
export RUN_DIR=/glade/scratch/mizzi/DART_TEST_AVE/MOPITT_Inv
export HSI_SAVE_DIR=/MIZZI/DART_TEST_AVE/MOPITT_Inv
...
#
# make run director for file conversons
...
# Get the obs_seq.out file for current cycle
         ...
         if [[ ${CHEM_TYPE} == IASI_O3 ]]; then export FILE=obs_seq_${L_DATE}.out; fi
#         if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export FILE=obs_seq_COMB_${L_DATE}.out; fi
         if [[ ${CHEM_TYPE} == MOPITT_CO ]]; then export FILE=obs_seq.proc; fi
         if ${USE_HSI}; then
