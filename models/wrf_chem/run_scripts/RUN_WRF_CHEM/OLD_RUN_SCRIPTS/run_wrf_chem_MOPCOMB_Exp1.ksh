...
export START_DATE=2008061212
export END_DATE=2008061212
...
export OBS_SEQ_FLNAME=obs_seq_comb_filt_
...
export RUN_DIR=/glade/scratch/mizzi/DART_TEST_AVE/MOPCOMB_Exp_1
export HSI_SAVE_DIR=/MIZZI/DART_TEST_AVE/MOPCOMB_Exp_1
...
# Check that all filter_ic_new.xxxx files exist
   ...
   while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
      ...
      if [[ ! -e convert_file_${KMEM}/assim_model_state_ic_${KMEM} ]]; then
         echo 'APM: '${RUN_DIR}/${L_DATE}/initial/convert_file_${KMEM}/assim_model_state_ic_${KMEM}' does not exit.'
         exit
      fi 
      let IMEM=${IMEM}+1
   done
...
#
# APM:Check that dart_wrf_vector exist
         let IMEM=1
         while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
            export KMEM=${IMEM}
            export CMEM=${IMEM}
            ...
            if [[ ! -e ${RUN_DIR}/${L_DATE}/dart_filter/filter_ic_old.${KMEM} ]]; then
               echo APM:${RUN_DIR}/${L_DATE}/dart_filter/filter_ic_old.${KMEM} does not exit.
               exit
            fi 
            let IMEM=${IMEM}+1
         done
...
# APM:Check that all wrfout files exist
         ...
         while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
            ...
            if [[ ! -e ${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_ANL} ]]; then
               echo 'APM: '${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_ANL}' does not exit.'
               exit
            fi
            if [[ ! -e ${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_FOR} ]]; then
               echo 'APM: '${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_FOR}' does not exit.'
               exit
            fi
            if [[ ! -e ${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_APM} ]]; then
               echo 'APM: '${CENTRALDIR}/advance_temp_${KMEM}/${WRFOUT_FILE_APM}' does not exit.'
               exit
            fi
            let IMEM=${IMEM}+1
         done
