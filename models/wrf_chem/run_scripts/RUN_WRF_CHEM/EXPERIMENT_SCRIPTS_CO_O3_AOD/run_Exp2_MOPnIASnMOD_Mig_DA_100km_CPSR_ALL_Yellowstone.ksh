...
export FIRST_FILTER_DATE=${W_FIRST_FILTER_DATE}
export COLD_START_LIMIT=2
export DOMAIN=01
export NUM_MEMBERS=20
#
export CYCLE_PERIOD=${W_CYCLE_PERIOD}
export FCST_PERIOD=${W_FCST_PERIOD}
...
export INIT_YY=`echo $INITIAL_DATE | cut -c1-4`
export INIT_MM=`echo $INITIAL_DATE | cut -c5-6`
export INIT_DD=`echo $INITIAL_DATE | cut -c7-8`
export INIT_HH=`echo $INITIAL_DATE | cut -c9-10`
...
export OBS_SEQ_DIR=obs_MOPnIASnMOD_CO_O3_AOD_CPSR_SCALE_SUPR_SINGLE_CLUSTER
export OBS_SEQ_DIR=obs_MOPnIASnMOD_CO_O3_AOD_CPSR_SCALE_SUPR
...
export RUN_INITIAL=${W_RUN_INITIAL}
...
export RUN_WARM=${W_RUN_WARM}
export WARM_FILTER=${W_WARM_FILTER}
export WARM_WRFCHEM=${W_WARM_WRFCHEM}
export WARM_ARCHIVE=${W_WARM_ARCHIVE}
#
if [[ ${WARM_FILTER} == true && ! ${RUN_FILTER} == true ]]; then 
   echo ERROR: WARM_FILTER and RUN_FILTER options inconsistent
   exit
elif [[ ${WARM_WRFCHEM} == true && ! ${RUN_WRFCHEM} == true ]]; then 
   echo ERROR: WARM_WRFCHEM and RUN_WRFCHEM options inconsistent
   exit
...
export TASKS_PER_NODE_FILTER=16
...
export RUN_DIR=${SCRATCH_DIR}/DART_TEST_AVE/MOPnIASnMOD_Exp_2_MgDA_20M_100km_COnXXnAOD_CPSR_Indep
export RUN_DIR=${SCRATCH_DIR}/DART_TEST_AVE/MOPnIASnMOD_Exp_2_MgDA_20M_100km_COnXXnAOD_CPSR_Indep_All
export RUN_DIR=${SCRATCH_DIR}/DART_TEST_AVE/MOPnIASnMOD_Exp_2_MgDA_20M_100km_COnXXnAOD_CPSR_Joint_All
mkdir -p ${RUN_DIR}
#
...
# Copy wrfinput to $CENTRALDIR
   ...
   cp ${DATA_DIR}/${EMISSIONS_DIR}/${L_YY}${L_MM}${L_DD}/wrfchemi_d${DOMAIN}_${L_FILE_DATE}.e001 wrfchemi_d${DOMAIN}
   cp ${DATA_DIR}/${EMISSIONS_DIR}/${L_YY}${L_MM}${L_DD}/wrffirechemi_d${DOMAIN}_${L_FILE_DATE}.e001 wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,E_CO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}
   ncatted -O -a coordinates,E_NO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_co,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_no,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_oc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_bc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_c2h4,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_ch2o,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
   ncatted -O -a coordinates,ebu_in_ch3oh,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}
#
...
########################################################################
#
# Set DART input.nml and WRFCHEM namelist.input parameters
#
########################################################################
...
# &assim_tools_nml
   ...
   export NL_SPECIAL_LOCALIZATION_OBS_TYPES="'MOPITT_CO_RETRIEVAL','IASI_CO_RETRIEVAL','IASI_O3_RETRIEVAL','MODIS_AOD_RETRIEVAL'"
   export NL_SAMPLING_ERROR_CORRECTION=.true.
   export NL_SPECIAL_LOCALIZATION_CUTOFFS=0.05,0.05,0.05,0.05
...
# Get WRFINPUT and WRFBDY files and convert to DART format for call to advance_model at initial time
   ...
   while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
      ...
      cp ${DATA_DIR}/${EMISSIONS_DIR}/${L_YY}${L_MM}${L_DD}/wrfchemi_d${DOMAIN}_${L_FILE_DATE}.e${CMEM} wrfchemi_d${DOMAIN}
      cp ${DATA_DIR}/${EMISSIONS_DIR}/${L_YY}${L_MM}${L_DD}/wrffirechemi_d${DOMAIN}_${L_FILE_DATE}.e${CMEM} wrffirechemi_d${DOMAIN}
#
...
# make run directory for file conversons
      ...
      cp ../wrfchemi_d${DOMAIN} ./.
      cp ../wrffirechemi_d${DOMAIN} ./.
#
...
#
# Copy the advance_time and input.nml to $CENTRALDIR/WRFCHEM_RUN
...
# Copy the wrfchem static chemistry data to $CENTRALDIR/WRFCHEM_RUN for this member
      ...
      cp ${CENTRALDIR}/input.nml ./.
      cp ${CENTRALDIR}/advance_time ./.
      cp ${DATA_DIR}/${WPB_RC_DIR}/${L_YY}${L_MM}${L_DD}${L_HH}/wrfinput_d${DOMAIN}_${L_FILE_DATE}.e${CMEM} wrfinput_d${DOMAIN}_${L_FILE_DATE}
...
# Copy dart executables
         cp ${CENTRALDIR}/input.nml ./.
         cp ${CENTRALDIR}/wrfchemi_d${DOMAIN} ./.
         cp ${CENTRALDIR}/wrffirechemi_d${DOMAIN} ./.
         cp ${CENTRALDIR}/advance_time ./.
         cp ${CENTRALDIR}/wrf_to_dart ./.
...
# APM: copy emission files for emission estimation
            ...
            while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
               ...
               while [[ ${LL_DATE} -le ${LL_END_DATE} ]]; do
                  ...
                  export LL_FILE_DATE=${LL_YY}-${LL_MM}-${LL_DD}_${LL_HH}:00:00
                  if [[ ${L_DATE} -eq ${FIRST_FILTER_DATE} ]]; then
                     cp ${DATA_DIR}/${EMISSIONS_DIR}/${LL_YY}${LL_MM}${LL_DD}/wrfchemi_d${DOMAIN}_${LL_FILE_DATE}.${CMEM} wrfchemi_d${DOMAIN}_${KMEM}
                     cp ${DATA_DIR}/${EMISSIONS_DIR}/${LL_YY}${LL_MM}${LL_DD}/wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}.${CMEM} wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,E_CO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,E_NO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_co,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_no,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_oc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_bc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_c2h4,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_ch2o,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_ch3oh,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                  else
                     cp ${RUN_DIR}/${PAST_DATE}/wrfchem_forecast/wrfchemi_d${DOMAIN}_${LL_FILE_DATE}_${KMEM} wrfchemi_d${DOMAIN}_${KMEM}
                     cp ${RUN_DIR}/${PAST_DATE}/wrfchem_forecast/wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}_${KMEM} wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,E_CO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,E_NO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_co,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
                     ncatted -O -a coordinates,ebu_in_no,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${KMEM}
...
# make run directory for file conversions
   ...
   cp ../wrfchemi_d${DOMAIN}_\${KMEM} wrfchemi_d${DOMAIN}
   cp ../wrffirechemi_d${DOMAIN}_\${KMEM} wrffirechemi_d${DOMAIN}
#
# Run wrf_to_dart
   ./wrf_to_dart > index_wrf_to_dart 2>&1 
...
#BSUB -P ${PROJ_NUMBER_NSC}
#BSUB -x                                 
#BSUB -n ${NUM_TASKS_FILTER}             
#BSUB -R "span[ptile=${TASKS_PER_NODE_FILTER}]" 
#BSUB -J ${JOBRND}                       
#BSUB -o ${JOBRND}.jout                  
#BSUB -e ${JOBRND}.jerr                  
...
# Generate job script to run advance_model.ksh for each member
         ...
         if ${RUN_SPECIAL_FORECAST}; then
            L_NUM_MEMBERS=${NUM_SPECIAL_FORECAST}
         fi
         while [[ ${IMEM} -le ${L_NUM_MEMBERS} ]]; do
            ...
            if [[ ${MEM} -lt 100 ]]; then export KMEM=00${MEM}; export CMEM=0${MEM}; fi
            if [[ ${MEM} -lt 10 ]]; then export KMEM=000${MEM}; export CMEM=00${MEM}; fi
#
# Copy chem static chemistry data to $CENTRALDIR/WRFCHEM_RUN for this member
            cd ${CENTRALDIR}/WRFCHEM_RUN
            if ${RUN_SPECIAL_FORECAST}; then
               rm -rf MEMBER_${KMEM}
            fi
            ...
            cp ${CENTRALDIR}/input.nml ./.
            cp ${CENTRALDIR}/advance_time ./.
            cp ${CENTRALDIR}/WRF/wrfinput_d${DOMAIN}_${NEXT_DAY_GREG}_${NEXT_SEC_GREG}_${KMEM} wrfinput_d${DOMAIN}_${L_FILE_DATE}
            cp ${DATA_DIR}/${EMISSIONS_DIR}/${L_YY}${L_MM}${L_DD}/wrfbiochemi_d${DOMAIN}_${L_FILE_DATE}.e${CMEM} wrfbiochemi_d${DOMAIN}_${L_FILE_DATE}
#
# Copy the wrfchem time dependent chemistry data to $CENTRALDIR/WRFCHEM_RUN for this member
            export LL_DATE=${L_DATE}
...
# Add coordinate attribute
               ncatted -O -a coordinates,E_CO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,E_NO,c,c,"XLONG, XLAT" wrfchemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_co,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_no,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_oc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_bc,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_c2h4,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_ch2o,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
               ncatted -O -a coordinates,ebu_in_ch3oh,c,c,"XLONG, XLAT" wrffirechemi_d${DOMAIN}_${LL_FILE_DATE}
#
# One WRFCHEM file as template
#               if [[ ${LL_DATE} -eq ${L_DATE} ]]; then
#                  cp wrfchemi_d${DOMAIN}_${L_FILE_DATE} ${CENTRALDIR}/wrfchemi_d${DOMAIN}
#                  cp wrffirechemi_d${DOMAIN}_${L_FILE_DATE} ${CENTRALDIR}/wrffirechemi_d${DOMAIN}
#               fi
               export LL_DATE=`echo ${LL_DATE} +1h | ./advance_time` 
            done
#
...
# Save WRFCHEM forecasts to archive directory
         ...
         while [[ ${IMEM} -le ${NUM_MEMBERS} ]]; do
            ...
            while [[ ${LL_DATE} -le ${LL_END_DATE} ]]; do
               ...
               export LL_HH=`echo ${LL_DATE} | cut -c9-10`
               export LL_FILE_DATE=${LL_YY}-${LL_MM}-${LL_DD}_${LL_HH}:00:00
               export WRFCHEMI_FILE_ANL=wrfchemi_d${DOMAIN}_${LL_FILE_DATE} 
               export WRFFIRECHEMI_FILE_ANL=wrffirechemi_d${DOMAIN}_${LL_FILE_DATE} 
               cp ${CENTRALDIR}/advance_temp_${KMEM}/${WRFCHEMI_FILE_ANL} ${WRFCHEMI_FILE_ANL}_${KMEM}
               cp ${CENTRALDIR}/advance_temp_${KMEM}/${WRFFIRECHEMI_FILE_ANL} ${WRFFIRECHEMI_FILE_ANL}_${KMEM}
               export LL_DATE=`echo ${LL_DATE} +6h | ./advance_time` 
            done
            let IMEM=${IMEM}+1
         done
         cp ${CENTRALDIR}/advance_temp_0001/namelist.input ./.
