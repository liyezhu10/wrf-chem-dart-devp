...
inside_mcell : &
           if( .not. dcell_outside_mcell ) then
             ...
             elseif( new_grid ) then
               ...
               dcell_area = area_map( i, j, mlon_ndx, mlat_ndx )
...
!  write(*,*)  'emis dataset size = ',size(wrk_data)
!  write(*,*)  'emis dataset min,max values = ',minval(wrk_data(:,:)),maxval(wrk_data(:,:))
!  write(*,*)  'emis dataset missing value count = ',count(wrk_data(:,:) == missing_value )
!  write(*,*)  '% valid data = ',100.* real( count(wrk_data(:,:) /= missing_value ) ) /real(size(wrk_data))
!  write(*,*)  'emis dataset surf integral       = ',sum(wrk_data(:,:))
!  write(*,*)  'emis dataset min value           = ',minval(wrk_data(:,:))
!  write(*,*)  'emis dataset min indices         = ',minloc(wrk_data(:,:))
!  write(*,*)  'emis dataset max value           = ',maxval(wrk_data(:,:))
