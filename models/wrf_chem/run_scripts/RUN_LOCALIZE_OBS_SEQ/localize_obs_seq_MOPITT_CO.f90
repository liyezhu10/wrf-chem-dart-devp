...
!            integer,parameter                         :: icycle_str=98
!            integer,parameter                         :: icycle_end=120
            integer,parameter                         :: icycle_end=36
!            integer,parameter                         :: icycle_end=2
            integer,parameter                         :: mcycle=icycle_end-icycle_str+1   
            integer,parameter                         :: mopitt_nlev=10
            integer,parameter                         :: nobs_kind=1
            integer,parameter                         :: mopitt_nstd_pr=10
...
!
            real,dimension(mopitt_nlev)               :: mopitt_prs_mid=(/100000.,85000.,75000.,65000., &
                                                         55000.,45000.,35000.,25000.,15000.,7500./)
            real,dimension(mopitt_nlev)               :: mopitt_prs_std=(/100000.,90000.,80000.,70000., &
...
!
            real,dimension(mopitt_nstd_pr)            :: mopitt_std_pr = (/1000.,900.,800.,700.,600.,500. &
                                                        ,400.,300.,200.,100./)
            real*8,dimension(mopitt_nstd_pr)          :: mopitt_avgker
!
...
! assign data paths
               ...
               path_obs_seq=trim(path_acd)//'/obs_MOPITT_CO_RETR_CPSR_SCALE_SUPR'
               path_obs_seq_loc=trim(path_acd)//'/obs_MOPITT_CO_RETR_CPSR_SCALE_BLOC_SUPR'
!
               file='obs_seq_mopitt_'//trim(sp_cycle(icycle))
!               file='obs_seq_mopitt_'//trim(cycle(icycle))
...
!               obs_kind(24) = 'MOPITT_CO_RETRIEVAL'
               obs_kind(1) = 'MOPITT_CO_RETRIEVAL'
!
! assign obs_kind_ids
!               obs_kind_id(1)=1
!               obs_kind_id(2)=2
...
!                                print *, x,y
                              case('loc3d')
                                 ...
                                 do k=kstr,mopitt_nlev
                                    prs=mopitt_prs_mid(k)
                                    if(k.eq.kstr) prs=mopitt_psfc
                                    avg=avg+prs*abs(mopitt_avgker(k))
                                    twt=twt+abs(mopitt_avgker(k))
                                 enddo
!=====================================
! Switch for pressure level or model levels
!=====================================
                                 avg=avg/twt
                                 level_sw=1
!                                 level_sw=0
                                 if(level_sw.eq.0) then
                                    write(iunit_out,'(3(g25.15,1x),i2)'),x,y,avg,z_id
!                                    print *, 'prs, prs_loc ',z,avg                             
!                                    print *,x,y,avg,z_id
                                 else
                                    k_lvl=0
                                    do k=kstr,mopitt_nlev-1
                                       if(avg.ge.mopitt_prs_mid(k)) then
                                          k_lvl=k
                                          exit
                                       else if(avg.lt.mopitt_prs_mid(mopitt_nlev)) then
                                          k_lvl=mopitt_nlev
                                          exit
                                       else if(avg.lt.mopitt_prs_mid(k) .and. avg.ge.mopitt_prs_mid(k+1)) then
                                          k_lvl=k
                                          exit
                                       endif
                                    enddo
                                    if(k_lvl.eq.0) then
                                       print *, 'APM: ERROR - level index is zero ',k_lvl
                                       call abort
                                    endif
                                    avg=real(k_lvl)
                                    write(iunit_out,'(3(g25.15,1x),i2)'),x,y,avg,z_id
