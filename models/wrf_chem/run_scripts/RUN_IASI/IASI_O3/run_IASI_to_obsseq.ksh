...
# DEPENDENT DIRECTORIES
  ...
  export OBS_IASI_DIR=${DATA_DIR}/obs_IASI
#
...
# RUN_MDART_TO_SDART (CONVERT MANY OBS_SEQ FILES TO A SINGLE OBS_SEQ FILE AND FILTER)
     if ${RUN_MDART_TO_SDART}; then
        ...
        export IASI_PRES_FILE=iasi_obs_seq_${YYYY}${MM}${DD}
        export IASI_NEXT_FILE=iasi_obs_seq_${IASI_NEXT_YYYY}${IASI_NEXT_MM}${IASI_NEXT_DD}
        if ${RUN_PREP_ASCII_TO_DART}; then cp ${ASIM_DIR}/prep_ascii_to_dart/${YYYY}${MM}/obs_seq${D_DATE} ./.; fi
        if ${RUN_IASI_ASCII_TO_DART}; then cp ${ASIM_DIR}/IASI_ascii_to_dart/${YYYY}${MM}/${IASI_PAST_FILE} ./.; fi
        if ${RUN_IASI_ASCII_TO_DART}; then cp ${ASIM_DIR}/IASI_ascii_to_dart/${YYYY}${MM}/${IASI_PRES_FILE} ./.; fi
        if ${RUN_IASI_ASCII_TO_DART}; then cp ${ASIM_DIR}/IASI_ascii_to_dart/${YYYY}${MM}/${IASI_NEXT_FILE} ./.; fi
#
...
# SETUP OBS_SEQUENCE_TOOL INPUT.NML
        ...
        if [[ -e obs_seq_${L_DATE}.out ]]; then
           ...
           touch ${DATA_DIR}/obs_IASI/${L_DATE}/NO_OBS_SEQ.OUT_DATA
        fi
        cd ${ASIM_DIR}
     fi 
#
