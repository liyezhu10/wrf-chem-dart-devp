...
use               model_mod, only : static_init_model, pres_to_zk
...
use obs_sequence_mod, only : static_init_obs_sequence, obs_type, obs_sequence_type,          &
                             get_obs_def, set_obs_def, get_first_obs, get_next_obs,          &
                             get_copy_meta_data, set_copy_meta_data, get_obs_values,         &
                             get_qc_meta_data, set_qc_meta_data, assignment(=),              &
                             init_obs, destroy_obs, init_obs_sequence, destroy_obs_sequence, &
                             get_num_copies, get_num_qc, get_qc, read_obs_seq,               &
                             read_obs_seq_header, insert_obs_in_seq, write_obs_seq

use      obs_def_mod, only : obs_def_type, get_obs_def_time, get_obs_kind,                   &
                             get_obs_def_location, set_obs_def_location,                     &
                             get_obs_def_error_variance

use     obs_kind_mod, only : get_obs_kind_name, get_obs_kind_index

use     location_mod, only : location_type, get_location, set_location,                      &
                             write_location, operator(/=), operator(==),                     &
                             LocationDims, VERTISSURFACE
...
integer           :: locnum           = 100
real(r8)          :: locrad           = 0.005   
integer           :: obslocnum        = 1       !901     ! min(log(P)) = 4; max(log(P)) = 13;
real(r8)          :: obsdloc          = 0.01    ! log(P) (natual log of Pa)
integer           :: distlocnum       = 180    ! locnum for distance between obs and var
real(r8)          :: distdloc         = 0.002
integer           :: wrf_ndom         = 1
integer           :: dom_start        = 1
...
! integer          :: indobs_e         = 1
 integer          :: indvar_s         = 2
 integer          :: indvar_e         = 2
! index for verfication box
integer           :: vi_beg           = 205
integer           :: vi_end           = 375
integer           :: vj_beg           = 80
integer           :: vj_end           = 250
! threshold for preciptation
real(r8)          :: precip_thresh    = 0.25_r8

real              :: R_P              = 10000.0_r8 ! obs error variance of Psfc
...
type localization_data
    ...
    real(r8), dimension(:,:,:,:), pointer   :: abscorr
...
real(r8)         :: obs_rainc, obs_rainnc, obs_precip, var_rainc, var_rainnc, var_precip

logical          :: is_lev0
logical :: pre_I_format, last_ob_flag
...
!!-------------------------------------------------------
!
!call read_obs_seq_header(obs_name_input,                                         &
!          obs_seq_in%num_copies, obs_seq_in%num_qc, obs_seq_in%num_obs, obs_seq_in%max_num_obs,  &
!          obs_seq_file_id, obs_seq_read_format, pre_I_format,                    &
!          close_the_file = .true.)
!
!call init_obs(      obs_in, obs_seq_in%num_copies, obs_seq_in%num_qc)
!call init_obs( next_obs_in, obs_seq_in%num_copies, obs_seq_in%num_qc)
!call init_obs(     obs_out, obs_seq_in%num_copies, obs_seq_in%num_qc)
!call init_obs(prev_obs_out, obs_seq_in%num_copies, obs_seq_in%num_qc)
!call init_obs_sequence(seq_in, 0,0,0)
!call init_obs_sequence(seq_out,0,0,0)
!
!allocate( copy_values(obs_seq_in%num_copies), qc_values(obs_seq_in%num_qc))
!
!call init_obs_sequence(seq_out, obs_seq_in%num_copies, obs_seq_in%num_qc, obs_seq_in%num_obs)
!call read_obs_seq(obs_name_input, 0, 0, 0, seq_in)
!
!do i=1, obs_seq_in%num_copies
!      string1   = trim(get_copy_meta_data(seq_in,i))
!      meta_data = adjustl(string1)
!      call set_copy_meta_data(seq_out, i, trim(meta_data))
!enddo
!do i=1, obs_seq_in%num_qc
!      string1   = trim(get_qc_meta_data(seq_in,i))
!      meta_data = adjustl(string1)
!      call set_qc_meta_data(seq_out, i, trim(meta_data))
!enddo
!
!obs_ngood = 0
!
!if ( .not. get_first_obs(seq_in, obs_in) )           &
!   call error_handler(E_ERR,'read obs_seq.final',    &
!           'No first observation in sequence.',      &
!           source,revision,revdate)
!
!obs_out     = obs_in  ! copy over everything
!next_obs_in = obs_in  ! record starting position
!
!   !--------------------------------------------------------------------
!   ObservationLoop : do obs_nread = 1,obs_seq_in%num_obs
!   !--------------------------------------------------------------------
!
!!      if ( verbose .and. (mod(obs_nread,10000) == 0) ) &
!!         write(*,*)'Processing obs ',obs_nread,' of ',obs_seq_in%num_obs
!
!      call get_obs_def(obs_in,        obs_def)
!      flavor   = get_obs_kind(        obs_def)
!      obs_loc  = get_obs_def_location(obs_def)
!      rloc     = get_location(obs_loc)
!      Robs     = get_obs_def_error_variance(obs_def)
!
!      if ((obs_nread == 1)) then
!         call get_qc(        obs_in,   qc_values)
!         call get_obs_values(obs_in, copy_values)
...
!            write(*,*)i,qc_values(i)
!         enddo
!         write(*,*)'observation error variance is: ', Robs
!
...
!      endif
!
!      if ( trim(get_obs_kind_name(flavor)) == "MOPITT_CO_RETRIEVAL" ) then
!         obs_ngood = obs_ngood + 1
!
!         if ( obs_ngood == 1 ) then
...
!         endif
!         prev_obs_out = obs_out
!
!         if ( (mod(obs_nread,1000) == 0) ) then
!
!             write(*,*)'NGOOD = ', obs_ngood
!             write(*,*)'Observation "kind" is ',flavor, &
!                                    get_obs_kind_name(flavor)
!    
!             call get_qc(        obs_in,   qc_values)
!             call get_obs_values(obs_in, copy_values)
...
!      endif
!
!      call get_next_obs(seq_in, obs_in, next_obs_in, last_ob_flag)
!
!      if (.not. last_ob_flag) obs_in = next_obs_in
!
!   !--------------------------------------------------------------------
!   enddo ObservationLoop
!   !--------------------------------------------------------------------
!
!! NOTE: the correct num_obs for seq_out is obs_ngood
!
...
!
!deallocate( copy_values, qc_values )
!
!print *, 'ngood = ', obs_ngood
!if ( obs_ngood == 0 ) then
!    print *, 'no MOPITT observations. STOP...'
!    stop
!endif

!-------------------------------------------------------
! READ True_State.nc and Prior_Diag.nc 
!-------------------------------------------------------
...
WRFDomains : do id = 1, wrf_ndom 
   ...
   wrf%dom(id)%description(1)  = 'T'
   wrf%dom(id)%description(2)  = 'co'
...
!   ! NOTE: 'RAINC' and 'RAINNC' only for truth.nc
!   wrf%dom(id)%description(21) = 'RAINC'
!   wrf%dom(id)%description(22) = 'RAINNC'
   ...
   ! read WRF dimensions
   !-------------------------------------------------------
   ! dimension of copies (2(ens mean, ens spread)+ens_size+2(inf mean, inf spread))
   wrf%dom(id)%ncopy = 1+ens_size + 4

   var_name_tmp = 'bottom_top_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id),       &
                  'read_wrf_dimensions','inq_dimid bottom_top')
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%bt),  &
   ...
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%bts), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   var_name_tmp = 'south_north_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id), &
                     'read_wrf_dimensions','inq_dimid south_north')
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%sn), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   var_name_tmp = 'south_north_stag_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id), &
                     'read_wrf_dimensions','inq_dimid south_north_stag') ! reuse var_id, no harm
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%sns), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   var_name_tmp = 'west_east_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id), &
                     'read_wrf_dimensions','inq_dimid west_east')
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%we), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   var_name_tmp = 'west_east_stag_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id), &
                     'read_wrf_dimensions','inq_dimid west_east_stag')  ! reuse var_id, no harm
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%wes), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   var_name_tmp = 'soil_layers_stag_d0'//idchar
   call nc_check( nf90_inq_dimid(ncidprior, trim(var_name_tmp), var_id), &
                     'read_wrf_dimensions','inq_dimid soil_layers_stag')  ! reuse var_id, no harm
   call nc_check( nf90_inquire_dimension(ncidprior, var_id, name, wrf%dom(id)%sls), &
                     'read_wrf_dimensions','inquire_dimension '//trim(name))

   call nc_check( nf90_inq_varid(ncidprior, "DX", var_id), &
                     'read_wrf_dimensions','inq_varid DX')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%dx), &
                     'read_wrf_dimensions','get_var DX')

   call nc_check( nf90_inq_varid(ncidprior, "DY", var_id), &
                     'read_wrf_dimensions','inq_varid DY')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%dy), &
...
!   wrf%dom(id)%re_m = earth_radius
   ...
   call nc_check( nf90_inq_varid(ncidprior, "MAP_PROJ", var_id), &
                     'read_wrf_map_para','inq_varid MAP_PROJ')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%map_proj), &
                     'read_wrf_map_para','get_var MAP_PROJ')

   call nc_check( nf90_inq_varid(ncidprior, "CEN_LAT", var_id), &
                     'read_wrf_map_para','inq_varid CEN_LAT')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%cen_lat), &
                     'read_wrf_map_para','get_var CEN_LAT')

   call nc_check( nf90_inq_varid(ncidprior, "CEN_LON", var_id), &
                     'read_wrf_map_para','inq_varid CEN_LON')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%cen_lon), &
                     'read_wrf_map_para','get_var CEN_LON')
!   if ( wrf%dom(id)%cen_lon < 0.0 )         & 
!        wrf%dom(id)%cen_lon = wrf%dom(id)%cen_lon + 360.0

   call nc_check( nf90_inq_varid(ncidprior, "TRUELAT1", var_id), &
                     'read_wrf_map_para','inq_varid TRUELAT1')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, truelat1), &
                     'read_wrf_map_para','get_var TRUELAT1')

   call nc_check( nf90_inq_varid(ncidprior, "TRUELAT2", var_id), &
                     'read_wrf_map_para','inq_varid TRUELAT2')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, truelat2), &
                     'read_wrf_map_para','get_var TRUELAT2')

   call nc_check( nf90_inq_varid(ncidprior, "STAND_LON", var_id), &
                     'read_wrf_map_para','inq_varid STAND_LON')  ! reuse var_id, no harm
   call nc_check( nf90_get_var(ncidprior, var_id, stdlon), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%dn),  &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%dnw), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%znu), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%znw), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%mub), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%hgt), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%latitude), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%latitude_u), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%latitude_v), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%longitude), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%longitude_u), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%longitude_v), &
   ...
   call nc_check( nf90_get_var(ncidprior, var_id, wrf%dom(id)%phb), &
   ...
   ! var_size(3,:) - dimension of bt ( = 1, means 2D variable, others 3D
   ! variable)
   ...
   do ind = 1,wrf%dom(id)%number_of_state_variables
      ...
      ! Once this is done for variable of True_State.nc, there is no need to do
      ! this for
...
!      if ( ind == 21 .or. ind == 22 ) then
!         print *, 'ind = ', ind
!         print *, wrf%dom(id)%variables(30,:,1,1,ind)
!         pause
!      endif
   enddo
...
! close netcdf files 
!-------------------------------------------------------
call nc_check(nf90_close(ncidtruth),'wrf_emp_localization','close True_State.nc')
call nc_check(nf90_close(ncidprior),'wrf_emp_localization','close Prior_Diag.nc')
...
! maxdist is the distance threshold used for limite grid points
maxdist = real(distlocnum-1) * distdloc
...
LoopDomain: do id = dom_start, wrf_ndom 
   ...
   LoopObsTypes: do idobs = 1, 1    !indobs_s, indobs_e
   print *, 'Now processing MOPITT observations '

      ndims = 1 * (obslev_num * varlev_num) 
      allocate(localization%num(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumx(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumx2(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy2(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%numerator(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%denominator(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha1(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha2(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%abscorr(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%correlation(distlocnum,obslocnum,ndims,wrf%dom(id)%number_of_state_variables))
      localization%num(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)          = 0
      localization%sumx(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumy(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumx2(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%sumy2(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%numerator(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)    = 0.0
      localization%denominator(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)  = 0.0
      localization%alpha1(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%alpha2(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%abscorr(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%correlation(1:distlocnum,1:obslocnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)  = 0.0

      LoopVarTypes: do inddim = indvar_s, indvar_e  !ind, ind    !indvar_s, indvar_e
      print *, 'State variable is ', trim(wrf%dom(id)%description(inddim))
      ...
      !--------------------------------------------------------------------
         ...
         if ( trim(get_obs_kind_name(flavor)) == "MOPITT_CO_RETRIEVAL" ) then
            ...
            call get_qc(        obs_in,   qc_values)
            if ( qc_values(2) < 1.0_r8 ) then

                call get_obs_values(obs_in, copy_values)
                ytrue      = copy_values(2)
                yfmean     = copy_values(3)
                yprior_var = copy_values(5)**2
                ypost_var  = copy_values(6)**2
                allocate(yprior(ens_size))
                yprior(1:ens_size) = copy_values(6+1:6+ens_size*2:2)

                obslon   = rloc(1)*DEG2RAD
                obslat   = rloc(2)*DEG2RAD
                obspres  = rloc(3)
...
!pause
                ...
                if ( obspres > 95000.0_r8) then
                    obslevk = 1

print *, 'y info = ', ytrue, yfmean, yprior_var, Robs, ypost_var
print *, 'yprior = ', yprior(1:ens_size)
print *, 'rloc   = ', rloc
...
!print *, 'var lev = ', varlevk, kk, ndims
                  do jj = nbdy_nouse+1, wrf%dom(id)%sn-nbdy_nouse 
                     do ii = nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
                        ...
                        ! Lili: 2013-07-18, loop through whole domain, but only
                        ! do computation for the grid points within a certain
                        ! distance threshold
...
!print *, 'index diag ', ilocind
                           ...
                           xfmean     = wrf%dom(id)%variables(ii,jj,kk,2,inddim)
                           xprior_var = wrf%dom(id)%variables(ii,jj,kk,3,inddim)**2
                           allocate(xprior(ens_size))
                           xprior(1:ens_size) = wrf%dom(id)%variables(ii,jj,kk,3+1:3+ens_size,inddim)

print *, 'x info = ', xtrue, xfmean, xprior_var
print *, 'xprior = ', xprior(1:ens_size)
print *, 'rloc   = ', varlon/DEG2RAD, varlat/DEG2RAD

print *, 'diat = ', dist, ilocind
...
print *, 'emp diag ', covxy, corr
print *, 'emp diag ', reg_coef, ccc, ddd
pause
...
!if ( ii == 100 .and. jj == wrf%dom(id)%sn-nbdy_nouse ) then
!print *, 'dist diag ', dist
!print *, 'index diag ', ilocind
!print *, 'emp diag ', covxy, corr
!print *, 'emp diag ', reg_coef, ccc, ddd
!print *, 'emp diag ', sublocx, sublocy, sublocnumer, sublocdenom
!pause
!endif

                           if ( abs(sublocdenom) > 0.0000000001_r8 ) then
                              sublocalpha = sublocnumer/sublocdenom
                              ndims = 1
   localization%num(ilocind,iobslocind,ndims,inddim)=localization%num(ilocind,iobslocind,ndims,inddim)+1
   localization%sumx(ilocind,iobslocind,ndims,inddim)=localization%sumx(ilocind,iobslocind,ndims,inddim)+sublocx
   localization%sumy(ilocind,iobslocind,ndims,inddim)=localization%sumy(ilocind,iobslocind,ndims,inddim)+sublocy
   localization%sumx2(ilocind,iobslocind,ndims,inddim)=localization%sumx2(ilocind,iobslocind,ndims,inddim)+sublocx2
   localization%sumy2(ilocind,iobslocind,ndims,inddim)=localization%sumy2(ilocind,iobslocind,ndims,inddim)+sublocy2
   localization%numerator(ilocind,iobslocind,ndims,inddim)=localization%numerator(ilocind,iobslocind,ndims,inddim)+sublocnumer
   localization%denominator(ilocind,iobslocind,ndims,inddim)=localization%denominator(ilocind,iobslocind,ndims,inddim)+sublocdenom
   localization%abscorr(ilocind,iobslocind,ndims,inddim)=localization%abscorr(ilocind,iobslocind,ndims,inddim)+abs(corr)
   localization%correlation(ilocind,iobslocind,ndims,inddim)=localization%correlation(ilocind,iobslocind,ndims,inddim)+corr
                           endif

                           deallocate(xprior)

                        endif ! within 'maxdist'
                     enddo    ! ii   
                  enddo       ! jj
               enddo          ! kk
...
!       thus put the writting out of OMP loop
      do inddim = indvar_s, indvar_e  !ind, ind    !indvar_s, indvar_e
         writelenloc = locnum
         write(idchar, '(i1.1)') id
         ...
         do obslevk = 1, obslev_num
            do varlevk = 1, varlev_num
               ...
               if ( obslev_ind(obslevk) < 10 ) then
                  write(levchar1,'(i1)') obslev_ind(obslevk)
               else
                  write(levchar1,'(i2)') obslev_ind(obslevk)
               endif
               if ( varlev_ind(obslevk,varlevk) < 10 ) then
                  write(levchar2,'(i1)') varlev_ind(obslevk,varlevk)
               else
                  write(levchar2,'(i2)') varlev_ind(obslevk,varlevk)
               endif

               ! write Obs_Precip, Var_All (OPVA)
               ndims = 1
               var_name_tmp = 'MOPITT_CO_RETRIEVAL_'//         &
               ...
               numtmp(1:writelenloc) = localization%num(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) numtmp 
               write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%abscorr(1:writelenloc,1,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,1,ndims,inddim)
               deallocate(numtmp)
               close(iunit)
      ...
      deallocate(localization%alpha1, localization%alpha2, localization%abscorr, localization%correlation)
