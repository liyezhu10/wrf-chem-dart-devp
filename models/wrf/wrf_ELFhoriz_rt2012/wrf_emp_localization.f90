...
integer           :: locnum           = 180
real(r8)          :: locrad           = 0.002  ! ~10km for 30degree north 
...
integer           :: wrf_ndom         = 1
integer           :: dom_start        = 1
integer           :: nbdy_nouse       = 20
integer           :: num_state_vars   = 22
integer           :: ens_size         = 50
...
!      endif
   ...
   !-------------------------------------------------------
   ! NOTE: ind ends '-2', because 'RAINC' and 'RAINNC' exist for truth.nc ONLY
   do ind = 1,wrf%dom(id)%number_of_state_variables - 2
...
!   obs%dom(id)%obstype_metadata(7)   = 'RADIOSONDE_SURFACE_ALTIMETER'
   obs%dom(id)%obstype_metadata(1)   = 'SURFACE_PRESSURE' 
...
!   obs%dom(id)%obstype_metadata(17)  = 'ACARS_DEWPOINT'
   ...
   do i = 1, obs%dom(id)%type_size
      if ( trim(obs%dom(id)%obstype_metadata(i)) == "SURFACE_PRESSURE" ) then
...
LoopDomain: do id = dom_start, wrf_ndom 
   ...
   print *, 'Now processing obs type ', trim(obs%dom(id)%obstype_metadata(idobs))
      ...
      ! copies: 1 - RAINC > 0 & RAINNC > 0; 2 - RAINC > 0 RAINNC = 0; 3 - RAINC = 0 | RAINNC > 0;
      !         4 - RAINC+RAINNC > 0; 5 - RAINC+RAINNC = 0
      ndims = 5 * (obslev_num * varlev_num) 
...
!      do k = lev_s, lev_e
      do obslevk = 1, obslev_num
         ...
         do j = vj_beg, vj_end       !nbdy_nouse+1, wrf%dom(id)%sn-nbdy_nouse
            do i = vi_beg, vi_end    !nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
               ...
               obs_rainc  = wrf%dom(id)%variables(i,j,1,1,21)
               obs_rainnc = wrf%dom(id)%variables(i,j,1,1,22)
               obs_precip = obs_rainc + obs_rainnc
...
!print *, 'var lev = ', varlevk, kk, ndims
                  do jj = vj_beg, vj_end      !nbdy_nouse+1, wrf%dom(id)%sn-nbdy_nouse 
                     do ii = vi_beg, vi_end   !nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
...
!print *, 'index diag ', ilocind
                           ...
                           var_rainc  = wrf%dom(id)%variables(ii,jj,1,1,21)
                           var_rainnc = wrf%dom(id)%variables(ii,jj,1,1,22)
                           var_precip = var_rainc + var_rainnc
...
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
                              ! record in distance
                              if ( obs_precip >= precip_thresh ) then
                                 ndims = 1
   ...
   localization%correlation(ilocind,iobslocind,ndims,inddim)=localization%correlation(ilocind,iobslocind,ndims,inddim)+corr
                                 if ( var_precip >= precip_thresh ) then
                                    ndims = 2
   localization%num(ilocind,iobslocind,ndims,inddim)=localization%num(ilocind,iobslocind,ndims,inddim)+1
   localization%sumx(ilocind,iobslocind,ndims,inddim)=localization%sumx(ilocind,iobslocind,ndims,inddim)+sublocx
   localization%sumy(ilocind,iobslocind,ndims,inddim)=localization%sumy(ilocind,iobslocind,ndims,inddim)+sublocy
      ...
      do inddim = indvar_s, indvar_e  !ind, ind    !indvar_s, indvar_e
         ...
         do obslevk = 1, obslev_num
            do varlevk = 1, varlev_num
               ...
               ! write Obs_Precip, Var_Precip (OPVP)
               ndims = 2
               var_name_tmp = trim(obs%dom(id)%obstype_metadata(idobs))//'_'//        &
                              trim(wrf%dom(id)%description(inddim))//'_d0'//idchar    &
                              //'_OPVP'                                               &
               ...
               ! write Obs_Noprecip, Var_All (ONVA)
               ndims = 3
               var_name_tmp = trim(obs%dom(id)%obstype_metadata(idobs))//'_'//      &
                              trim(wrf%dom(id)%description(inddim))//'_d0'//idchar  &
                              //'_ONVA'                                             &
               ...
               ! write Obs_Noprecip, Var_Noprecip (ONVN)
               ndims = 4
               var_name_tmp = trim(obs%dom(id)%obstype_metadata(idobs))//'_'//      &
                              trim(wrf%dom(id)%description(inddim))//'_d0'//idchar  &
                              //'_ONVN'                                             &
