...
integer           :: distlocnum       = 160    ! locnum for distance between obs and var
real(r8)          :: distdloc         = 0.02
integer           :: obsregion        = 5      ! index for 'tropic', 'mid latitude', 'pole'
integer           :: wrf_ndom         = 1
...
real(r8)         :: ref_P, surf_P, obs_P, var_P
integer          :: obslat_s, obslat_e, varlat_s, varlat_e, iregion

logical          :: is_lev0
character(len=2) :: idchar,idchar1, idchar2
...
var_ind(1,1) = 1
!!var_ind(1,1) = 8   ! maxi veritcal weight for TPW at model level 8
!var_ind(1,2) = 7
!var_ind(1,3) = 12
!var_ind(1,4) = 25
!var_ind(2,1) = 1
...
LoopObsTypes: do idobs = indobs_s, indobs_e
   ...
   ! U is on slat grid (lat has 192 dimensions, but slat has 191 dimensions)
   obslat_s = 1
   if ( idobs == 3 ) then
      obslat_e = cam%nslat
   else
...
!               do kk = lev_s, lev_e
               do varlevk = 1, varlev
                  ...
                  do jj = varlat_s, varlat_e 
                     do ii = 1, cam%nlon

                     ! compute the pressure of variable
                     ref_P  = cam%P0(1)
                     surf_P = cam%variables(ii,jj,1,2,1)
...
!pause
                     ...
                     localization%denominator(ilocind,iobslocind,iregion,inddim) = localization%denominator(ilocind,iobslocind,iregion,inddim) &
                                                                      + sublocdenom
!                     localization%alpha1(ilocind,iobslocind,iregion,inddim) = localization%alpha1(ilocind,iobslocind,iregion,inddim) + sublocalpha
!                     localization%alpha2(ilocind,iobslocind,iregion,inddim) = localization%alpha2(ilocind,iobslocind,iregion,inddim) + sublocalpha**2
      ...
      do obslevk = 1, obslev
         do varlevk = 1, varlev
            ...
            var_name_tmp = trim(obs%obstype_metadata(idobs))//'_'//                     &
                           trim(cam%description(inddim))//'_obslev'//trim(idchar1)//    &
                           '_varlev'//trim(idchar2)//'_region'//idchar
            iunit = 10 + inddim
            open(iunit, FILE=var_name_tmp, FORM='FORMATTED')

            allocate(numtmp(writelenloc))
            numtmp(1:writelenloc) = localization%num(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) numtmp
            write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,1,iregion,inddim)
...
!            write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,ndims,inddim)
            write(iunit, FMT=locreal_format) localization%abscorr(1:writelenloc,1,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,1,iregion,inddim)
            deallocate(numtmp)
   ...
   deallocate(localization%abscorr, localization%correlation)
