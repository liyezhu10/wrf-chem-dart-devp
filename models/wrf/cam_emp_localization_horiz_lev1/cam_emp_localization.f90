...
integer           :: lev_s            = 1
integer           :: lev_e            = 1
integer           :: indobs_s         = 2  ! 1
integer           :: indobs_e         = 2  ! 4
integer           :: indvar_s         = 3  ! 1
integer           :: indvar_e         = 3  ! 5
...
integer          :: i, j, k, id, idobs, ind, inddim, indens,           &
                    ii, jj, kk, indk1, indk2, var_id, ndims, dimids(10), junit
...
!   LoopVarTypes: do inddim = 1, cam%number_of_state_variables
   LoopVarTypes: do inddim = indvar_s, indvar_e
      ...
      do junit = 1, locnum/10
         if ( junit < 10 ) then
            write(idchar,'(i1)') junit
         elseif ( junit < 100 ) then
            write(idchar,'(i2)') junit
         elseif( junit < 1000 ) then
            write(idchar, '(i3)') junit
         else
            print *, 'ERROR: write subloc index to idchar'
         endif

         var_name_tmp = trim(obs%obstype_metadata(idobs))//'_'//         &
                        trim(cam%description(inddim))//'_subloc'//idchar
...
print *, 'obs loc = ', i,j,k,obslon,obslat
print *, 'var loc = ', ii,jj,kk,varlon,varlat
print *, 'dist = ', dist
...
junit = ceiling(ilocind/10.0)
!print *, ilocind, junit
write(junit,*) sublocx, sublocy, sublocdenom
!pause
!if ( ilocind > 130 .and. ilocind <=140 ) then
!   print *, sublocx, sublocy, sublocdenom
!endif
