...
integer           :: distlocnum       = 901     ! locnum for distance between obs and var abs(log(P_obs)-log(P_var))
...
!      ndims = 2 * (obslev_num * varlev_num)    ! 2 for whole domain and ocean 
      ndims = 2 * wrf%dom(id)%bts
      allocate(localization%num(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumx(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumx2(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy2(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%numerator(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%denominator(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha1(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha2(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%correlation(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      allocate(localization%abscorr(distlocnum,obslocnum,wrf%dom(id)%number_of_state_variables))
      localization%num(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)          = 0
      localization%sumx(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumy(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumx2(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%sumy2(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%numerator(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)    = 0.0
      localization%denominator(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)  = 0.0
      localization%alpha1(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%alpha2(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%abscorr(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%correlation(1:distlocnum,1:obslocnum,1:wrf%dom(id)%number_of_state_variables)  = 0.0



!!!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,obslevk,varlevk,inddim,ndims,indk1,indk2,iunit,writelenloc,var_name_tmp,obslon,obslat,varlon,varlat,varhgt,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,wrf_var_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)

      ind = obs%dom(id)%obs_to_statevar(idobs)
      LoopVarTypes: do inddim = ind, ind !indvar_s, indvar_e
...
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
                        ! record in distance
                        localization%num(ilocind,iobslocind,inddim) = localization%num(ilocind,iobslocind,inddim) + 1

                        localization%sumx(ilocind,iobslocind,inddim) = localization%sumx(ilocind,iobslocind,inddim) + sublocx
                        localization%sumy(ilocind,iobslocind,inddim) = localization%sumy(ilocind,iobslocind,inddim) + sublocy

                        localization%sumx2(ilocind,iobslocind,inddim) = localization%sumx2(ilocind,iobslocind,inddim) + sublocx2
                        localization%sumy2(ilocind,iobslocind,inddim) = localization%sumy2(ilocind,iobslocind,inddim) + sublocy2

                        localization%numerator(ilocind,iobslocind,inddim) = localization%numerator(ilocind,iobslocind,inddim)        &
                                                                   + sublocnumer
                        localization%denominator(ilocind,iobslocind,inddim) = localization%denominator(ilocind,iobslocind,inddim)    &
                                                                     + sublocdenom
!                        localization%alpha1(ilocind,iobslocind,inddim) = localization%alpha1(ilocind,iobslocind,inddim) + sublocalpha
!                        localization%alpha2(ilocind,iobslocind,inddim) = localization%alpha2(ilocind,iobslocind,inddim) + sublocalpha**2
                        localization%abscorr(ilocind,iobslocind,inddim) = localization%abscorr(ilocind,iobslocind,inddim) + abs(corr)
                        localization%correlation(ilocind,iobslocind,inddim) = localization%correlation(ilocind,iobslocind,inddim) + corr
...
!               write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%abscorr(1:writelenloc,ndims,inddim)
...
integer    :: i
real(r8)   :: ptop,eta_levels(50)

ptop = 2000.0      ! unit: Pa, pressure at model top
data eta_levels/1.000_r8,  0.9947_r8, 0.9895_r8, 0.9843_r8, 0.979_r8,     &
                0.9739_r8, 0.9684_r8, 0.9626_r8, 0.9564_r8, 0.9498_r8,    &
                0.9426_r8, 0.9348_r8, 0.9262_r8, 0.9167_r8, 0.9062_r8,    &
                0.8946_r8, 0.8816_r8, 0.8671_r8, 0.8509_r8, 0.833_r8,     &
                0.813_r8,  0.7909_r8, 0.7667_r8, 0.7402_r8, 0.7116_r8,    &
                0.6809_r8, 0.6483_r8, 0.6141_r8, 0.5785_r8, 0.5419_r8,    &
                0.5047_r8, 0.4672_r8, 0.4299_r8, 0.3931_r8, 0.357_r8,     &
                0.322_r8,  0.2883_r8, 0.256_r8,  0.2253_r8, 0.1963_r8,    &
                0.169_r8,  0.1433_r8, 0.119_r8,  0.0967_r8, 0.0757_r8,    &
                0.0571_r8, 0.0407_r8, 0.0257_r8, 0.0122_r8, 0.000_r8 /
