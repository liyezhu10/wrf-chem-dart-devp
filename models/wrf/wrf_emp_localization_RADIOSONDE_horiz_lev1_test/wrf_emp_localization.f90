...
! 2012-07-07
! this code cannot run with OMP, run with one obs type and one state var type
! write statistics of pairs for every subloc
...
integer          :: ngroup, iens1, iens2, ens_size_group
...
real(r8)         :: xprior_group_var,xfmean_group,yprior_group_var,yfmean_group,ypost_group_var,           &
                    reg_coef_group,ccc_group,ddd_group,covxy_group, sublocy_group, sublocdenom_group

logical          :: is_lev0
character(len=1) :: idchar
character(len=3) :: idlocchar
...
real(r8), allocatable :: xprior_group(:), yprior_group(:)
...
LoopDomain: do id = dom_start, wrf_ndom 
   ...
   print *, 'Now processing obs type ', trim(obs%dom(id)%obstype_metadata(idobs))
      ...
      allocate(localization%sumx(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumx2(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%sumy2(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%numerator(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%denominator(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha1(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%alpha2(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      allocate(localization%correlation(locnum,ndims,wrf%dom(id)%number_of_state_variables))
      localization%num(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)          = 0
      localization%sumx(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumy(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%sumx2(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%sumy2(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)      = 0.0
      localization%numerator(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)    = 0.0
      localization%denominator(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)  = 0.0
      localization%alpha1(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%alpha2(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)       = 0.0
      localization%correlation(1:locnum,1:ndims,1:wrf%dom(id)%number_of_state_variables)  = 0.0



!!!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,obslevk,varlevk,inddim,ndims,indk1,indk2,iunit,junit,writelenloc,var_name_tmp,obslon,obslat,varlon,varlat,varhgt,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,wrf_var_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)
...
!      LoopVarTypes: do inddim = 6, wrf%dom(id)%number_of_state_variables 
      ...
      ! open unit for each subloc
      do junit = 1, locnum/10+1
!      do junit = 1, locnum/10 
print *, 'open junit = ', junit
         if ( junit == 1 ) then
            write(idlocchar,'(i1)') junit-1
         elseif ( junit - 1 > 0 .and. junit-1 < 10 ) then
            write(idlocchar,'(i1)') junit-1
         elseif ( junit - 1 >= 10 .and. junit-1 < 100 ) then
            write(idlocchar,'(i2)') junit-1
         elseif ( junit - 1 >= 100 .and. junit-1 < 1000 ) then
            write(idlocchar, '(i3)') junit-1
         else
            print *, 'ERROR: write subloc index to idlocchar'
         endif

         var_name_tmp = trim(obs%dom(id)%obstype_metadata(idobs))//'_'//         &
                        trim(wrf%dom(id)%description(inddim))//'_subloc'//idlocchar

print *, 'var_name_tmp = ', var_name_tmp

         open(junit, FILE=var_name_tmp)
      enddo
...
!print *, 'obs_to_statevar = ', ind

               ! get obs information ready
               ytrue      = wrf%dom(id)%variables(i,j,k,1,ind)
               yfmean     = wrf%dom(id)%variables(i,j,k,2,ind)
               yprior_var = wrf%dom(id)%variables(i,j,k,3,ind)**2
               Robs       = obs%dom(id)%obsvar(idobs)
               ypost_var    = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
               allocate(yprior(ens_size))
               yprior(1:ens_size) = wrf%dom(id)%variables(i,j,k,3+1:3+ens_size,ind)
...
!print *, 'y loc = ', wrf%dom(id)%longitude(i,j), wrf%dom(id)%latitude(i,j), obslon, obslat
!pause
...
!print *, 'x loc = ', wrf%dom(id)%longitude(ii,jj), wrf%dom(id)%latitude(ii,jj), varlon, varlat
!pause
!endif
...
! write to subloc files
if ( ilocind == 1 ) then
   junit = 1
else
   junit = ceiling(ilocind/10.0)+1
endif
!if ( junit <= 100 ) then
write(junit,'(8e15.7)')  0.0, Robs, yprior_var, covxy, ytrue-yfmean, sublocx, sublocy, sublocdenom

! loop through ngroup for group filter test
ngroup = 2
do indk1 = 1, ngroup
   ens_size_group = ens_size/ngroup
   allocate(yprior_group(ens_size_group))
   allocate(xprior_group(ens_size_group))
   iens1 = (indk1-1)*ens_size_group+1
   iens2 = indk1*ens_size_group
   yprior_group(1:ens_size_group) = yprior(iens1:iens2)
   xprior_group(1:ens_size_group) = xprior(iens1:iens2)

   yfmean_group = sum(yprior_group)/ens_size_group
   xfmean_group = sum(xprior_group)/ens_size_group

   yprior_group_var = comp_var(ens_size_group,yfmean_group,yprior_group)
   ypost_group_var  = 1.0_r8/(1.0_r8/yprior_group_var+1.0_r8/Robs)
   xprior_group_var = comp_var(ens_size_group,xfmean_group,xprior_group)
   
   covxy_group    = comp_cov(ens_size_group,xfmean_group,xprior_group,yfmean_group,yprior_group)
   reg_coef_group = covxy_group/yprior_group_var
   ccc_group      = reg_coef_group*(ypost_group_var/yprior_group_var-1.0_r8)*yfmean_group
   ddd_group      = reg_coef_group*ypost_group_var/Robs

   sublocy_group     = ccc_group + ddd_group * ytrue
   sublocdenom_group = (ccc_group+ddd_group*ytrue)**2 + ddd_group**2 * Robs

   write(junit,'(8e15.7)')  real(indk1), Robs, yprior_group_var, covxy_group, ytrue-yfmean_group,       &
                            xtrue-xfmean_group, sublocy_group, sublocdenom_group

   deallocate(yprior_group,xprior_group)
enddo
!endif
...
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
                        ! record in distance
                        localization%num(ilocind,ndims,inddim) = localization%num(ilocind,ndims,inddim) + 1
                        localization%sumx(ilocind,ndims,inddim) = localization%sumx(ilocind,ndims,inddim) + sublocx
                        localization%sumy(ilocind,ndims,inddim) = localization%sumy(ilocind,ndims,inddim) + sublocy

                        localization%sumx2(ilocind,ndims,inddim) = localization%sumx2(ilocind,ndims,inddim) + sublocx2
                        localization%sumy2(ilocind,ndims,inddim) = localization%sumy2(ilocind,ndims,inddim) + sublocy2

                        localization%numerator(ilocind,ndims,inddim) = localization%numerator(ilocind,ndims,inddim)        &
                                                                   + sublocnumer
                        localization%denominator(ilocind,ndims,inddim) = localization%denominator(ilocind,ndims,inddim)    &
                                                                     + sublocdenom
                        localization%alpha1(ilocind,ndims,inddim) = localization%alpha1(ilocind,ndims,inddim) + sublocalpha
                        localization%alpha2(ilocind,ndims,inddim) = localization%alpha2(ilocind,ndims,inddim) + sublocalpha**2
                        localization%correlation(ilocind,ndims,inddim) = localization%correlation(ilocind,ndims,inddim) + abs(corr)
   
                        ! record terrain less than 100 m
                        if ( varhgt <= 100.0 ) then
                           ! record in distance
                           localization%num(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%num(ilocind,obslev_num*varlev_num+ndims,inddim) + 1

                           localization%sumx(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%sumx(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocx
                           localization%sumy(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%sumy(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocy

                           localization%sumx2(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%sumx2(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocx2
                           localization%sumy2(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%sumy2(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocy2

                           localization%numerator(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%numerator(ilocind,obslev_num*varlev_num+ndims,inddim)     &
                                                                      + sublocnumer
                           localization%denominator(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%denominator(ilocind,obslev_num*varlev_num+ndims,inddim) &
                                                                        + sublocdenom
                           localization%alpha1(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%alpha1(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocalpha
                           localization%alpha2(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%alpha2(ilocind,obslev_num*varlev_num+ndims,inddim) + sublocalpha**2
                           localization%correlation(ilocind,obslev_num*varlev_num+ndims,inddim) = localization%correlation(ilocind,obslev_num*varlev_num+ndims,inddim) + abs(corr)
...
!               write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%alpha1(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,ndims,inddim)
!               deallocate(numtmp)
!
!               ! write ocean
!               allocate(numtmp(writelenloc))
!               numtmp(1:writelenloc) = localization%num(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) numtmp
!               write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%alpha1(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,obslev_num*varlev_num+ndims,inddim)
!               deallocate(numtmp)
!
!               close(iunit)
!            enddo
...
function comp_var(numens, xmean, x)
integer,   intent(in)  :: numens
real(r8),  intent(in)  :: xmean
real(r8),  intent(in)  :: x(:)
real(r8)               :: comp_var
...
comp_var = 0.0
do i = 1, numens
   comp_var = comp_var + ( x(i) - xmean )**2
enddo
comp_var = comp_var / (real(numens - 1))

end function comp_var
