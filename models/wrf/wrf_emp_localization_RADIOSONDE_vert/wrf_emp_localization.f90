...
!pause
                        ...
                        if ( indk1 > 0 ) then
                           localization%num(ilocind,k,inddim) =                                &
                               localization%num(ilocind,k,inddim) + 1
                           ...
                           localization%beta1(ilocind,k,inddim) =                              &
                               localization%beta1(ilocind,k,inddim) + sumbeta1
                           localization%beta2(ilocind,k,inddim) =                              &
                               localization%beta2(ilocind,k,inddim) + sumbeta2

                           do ig = 1, group_size
                           localization%sumx(ilocind,ig,k,inddim) =                            &
                               localization%sumx(ilocind,ig,k,inddim) + sublocx(ig)
                           localization%sumy(ilocind,ig,k,inddim) =                            &
                               localization%sumy(ilocind,ig,k,inddim) + sublocy(ig)

                           localization%sumx2(ilocind,ig,k,inddim) =                           &
                               localization%sumx2(ilocind,ig,k,inddim) + sublocx2(ig)
                           localization%sumy2(ilocind,ig,k,inddim) =                           &
                               localization%sumy2(ilocind,ig,k,inddim) + sublocy2(ig)

                           localization%numerator(ilocind,ig,k,inddim) =                       &
                               localization%numerator(ilocind,ig,k,inddim) + sublocnumer(ig)
                           localization%denominator(ilocind,ig,k,inddim) =                     &
                               localization%denominator(ilocind,ig,k,inddim) + sublocdenom(ig)

                           localization%alpha1(ilocind,ig,k,inddim) =                          &
                               localization%alpha1(ilocind,ig,k,inddim) + sublocalpha(ig)
                           localization%alpha2(ilocind,ig,k,inddim) =                          &
                               localization%alpha2(ilocind,ig,k,inddim) + sublocalpha(ig)**2

                           localization%correlation(ilocind,ig,k,inddim) =                     &
                               localization%correlation(ilocind,ig,k,inddim) + abs(corr(ig))
                           enddo
