...
! 2012-08-03: update the code for global grou filter 
...
! 2010-08-03, add variables to store global group filter information
...
!var_ind(1,1) = 1
var_ind(1,1) = 18   ! maxi veritcal weight for TPW at model level 8
...
! allocate and initialize localization variables
   ...
   allocate(localization%beta1(locnum,ndims,cam%number_of_state_variables))
   allocate(localization%beta2(locnum,ndims,cam%number_of_state_variables))
...
!   allocate(localization%alpha2(locnum,ndims,cam%number_of_state_variables))
   allocate(localization%correlation(locnum,group_size,ndims,cam%number_of_state_variables))
   localization%num         = 0
...
!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,obslevk,varlevk,inddim,ndims,indk1,indk2,ig,igs,ige,igg,iunit,writelenloc,ref_P,surf_P,var_name_tmp,obslon,obslat,varlon,varlat,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,cam_var_1d,qv_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d,sumbeta1,sumbeta2)
...
!                              ytrue      = cam%variables(i,j,k,1,idobs)
                              yfmean     = cam%variables(i,j,k,igs,idobs)
                              yprior_var = cam%variables(i,j,k,igs+1,idobs)**2
!                              Robs       = obs%obsvar(idobs)
                              ypost_var    = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
...
!                           xtrue      = cam%variables(ii,jj,kk,1,inddim)
                           xfmean     = cam%variables(ii,jj,kk,igs,inddim)
                           xprior_var = cam%variables(ii,jj,kk,igs+1,inddim)**2
...
!             treat PS as integral variable
   ...
   deallocate(localization%num, localization%beta1, localization%beta2)
