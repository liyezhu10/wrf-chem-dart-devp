...
integer           :: locnum           = 180
real(r8)          :: locrad           = 0.02  ! ~10km for 30 degreee north 
integer           :: obslocnum        = 1       !901     ! min(log(P)) = 4; max(log(P)) = 13;
real(r8)          :: obsdloc          = 0.01    ! log(P) (natual log of Pa)
integer           :: distlocnum       = 40     ! locnum for distance between obs and var abs(log(P_obs)-log(P_var))
real(r8)          :: distdloc         = 0.01
integer           :: wrf_ndom         = 1
integer           :: dom_start        = 1
integer           :: nbdy_nouse       = 20    !10
...
integer          :: i, j, k, id, idobs, ind, inddim, indens,               &
                    ...
                    obslevk, varlevk, nlev
integer          :: ncid, ncidtruth, ncidprior, ncidobs, iunit
integer          :: proj_code
integer          :: iobslocind, ilocind, sublocnum, writelenloc
...
real(r8)         :: surf_P, obs_P, var_P, dist_logP
...
!               obshgt = wrf%dom(id)%mhgt(i,j,1,1)
               ! compute obs pressure
               surf_P = wrf%dom(id)%variables(i,j,1,1,7)
               allocate(wrf_var_1d(nlev)) 
               call get_pressure(surf_P,nlev,wrf_var_1d)
               obs_P = wrf_var_1d(k)
!print *, wrf_var_1d
!pause

               ! loop target variable grid points
               do kk = 1, wrf%dom(id)%bt

                     ! get target variable information ready
                     xtrue      = wrf%dom(id)%variables(i,j,kk,1,inddim)
                     xfmean     = wrf%dom(id)%variables(i,j,kk,2,inddim)
                     xprior_var = wrf%dom(id)%variables(i,j,kk,3,inddim)**2
                     allocate(xprior(ens_size))
                     xprior(1:ens_size) = wrf%dom(id)%variables(i,j,kk,3+1:3+ens_size,inddim)
...
!                     endif
!print *, obs_P, var_P, dist_logP, ilocind
!pause
...
!       thus put the writting out of OMP loop
       do inddim = indvar_s, indvar_e   ! ind, ind
          writelenloc = distlocnum
          write(idchar, '(i1.1)') id
 
          if ( lev_s < 10 ) then
             write(levchar1,'(i1)') lev_s
          else
             write(levchar1,'(i2)') lev_s 
          endif
          ...
          var_name_tmp = trim(obs%dom(id)%obstype_metadata(idobs))//'_'//         &
                         ...
                         //'_OP'//'_obslev'//trim(levchar1)
          iunit = 10 + inddim
          open(iunit, FILE=var_name_tmp, FORM='FORMATTED')
          do k = 1, obslocnum
               ! write whole domain
               allocate(numtmp(writelenloc))
               numtmp(1:writelenloc) = localization%num(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) numtmp 
               write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%abscorr(1:writelenloc,k,ndims,inddim)
               write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,k,ndims,inddim)
               deallocate(numtmp)
          enddo
          close(iunit)
...
subroutine get_pressure(Psfc,nlev,pressure)
! compute the pressure in model levels by eta_levels
real(r8),   intent(in)  :: Psfc
integer,    intent(in)  :: nlev
real(r8),   intent(out) :: pressure(:)

integer    :: i
real(r8)   :: ptop,eta_levels(40)

ptop = 5000.0      ! unit: Pa, pressure at model top
data eta_levels/1.00000_r8, 0.99307_r8, 0.98348_r8, 0.97105_r8, 0.95551_r8,  &
                0.93651_r8, 0.91363_r8, 0.88644_r8, 0.85460_r8, 0.81855_r8,  &
                0.77877_r8, 0.73579_r8, 0.69016_r8, 0.64246_r8, 0.59329_r8,  &
                0.54573_r8, 0.50104_r8, 0.45908_r8, 0.41972_r8, 0.38281_r8,  &
                0.34824_r8, 0.31589_r8, 0.28563_r8, 0.25735_r8, 0.23096_r8,  &
                0.20635_r8, 0.18343_r8, 0.16209_r8, 0.14226_r8, 0.12384_r8,  &
                0.10677_r8, 0.09095_r8, 0.07633_r8, 0.06282_r8, 0.05036_r8,  &
                0.03889_r8, 0.02835_r8, 0.01868_r8, 0.00983_r8, 0.00000_r8 /

do i = 1, nlev
   pressure(i) = ptop + eta_levels(i)*(Psfc-ptop)
enddo
