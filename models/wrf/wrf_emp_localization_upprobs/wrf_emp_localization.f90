...
real(r8)          :: locrad           = 0.0002  ! 200 meter instead of radiance of 0.0002 (~1km) 
integer           :: wrf_ndom         = 1
...
!   character(len=129),dimension(:),pointer :: units, stagger, coordinates
   ...
   real(r8), dimension(:,:,:,:), pointer :: var_inf_corr
end type wrf_data
...
type obs_data
    ...
    real(r8), dimension(:,:,:,:), pointer :: obs_inf_corr
...
real(r8)         :: yobs, ytrue, yfmean, oerr, Robs, yprior_var, ypost_var, yvar, yinno_norm,              &
                    xfmean, xtrue, xt_xfmean, covxy, deltaxmean, corr, reg_coef, ccc, ddd, dist
real(r8)         :: obsdiag1, obsdiag2, obsdiag3, obsdiag4
real(r8)         :: sublocnomi, sublocdenomi, sublocalpha
logical          :: is_lev0
...
do id = 1, wrf_ndom 
   ...
   ! The 'FAKE_SFC' OBS are faked sfc obs at model levels (e.g., level 15)
   ! thus, no fake altimeter obs
   obs%dom(id)%obstype_metadata(7)  = 'FAKE_SFC_U_WIND_COMPONENT'
   obs%dom(id)%obstype_metadata(8)  = 'FAKE_SFC_V_WIND_COMPONENT'
   obs%dom(id)%obstype_metadata(9)  = 'FAKE_SFC_TEMPERATURE'
   obs%dom(id)%obstype_metadata(10) = 'FAKE_SFC_DEWPOINT'
   obs%dom(id)%obstype_metadata(11) = 'FAKE_SFC_SPECIFIC_HUMIDITY'

!    obs%dom(id)%obstype_metadata(7)  = 'RADIOSONDE_SURFACE_ALTIMETER'
!    obs%dom(id)%obstype_metadata(8)  = 'RADIOSONDE_U_WIND_COMPONENT'
!    obs%dom(id)%obstype_metadata(9)  = 'RADIOSONDE_V_WIND_COMPONENT'
!    obs%dom(id)%obstype_metadata(10) = 'RADIOSONDE_TEMPERATURE'
...
!                                    obs%dom(id)%type_size)
   ...
   allocate(wrf_var_4d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,wrf%dom(id)%ncopy))
   allocate(wrf_var_4d_2(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,wrf%dom(id)%ncopy))
   allocate(obs_var_3d(wrf%dom(id)%we,wrf%dom(id)%sn,obs%dom(id)%obscopysize))
   allocate(obs_var_4d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,obs%dom(id)%obscopysize))
...
! print *, 'ind = ', ind
      ...
      elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'SFC_SPECIFIC_HUMIDITY' ) then
       ...
       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_U_WIND_COMPONENT' ) then
         obs%dom(id)%obs_size(3,ind) = 1
         ! U index --- 8
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy) =           &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy,8)
         call draw_perfect_obs_3d(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,           &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)
! Lili test
!print *, 'draw obs diag ', obs%dom(id)%obs_val(50,50,15,1,7), obs%dom(id)%obs_val(50,50,15,2,7)
!print *, 'draw obs diag ', obs%dom(id)%obs_val(50,50,15,3,7), obs%dom(id)%obs_val(50,50,15,64,7)
!pause
       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_V_WIND_COMPONENT' ) then
         obs%dom(id)%obs_size(3,ind) = 1
         ! V index --- 9
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy) =           &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy,9)
         call draw_perfect_obs_3d(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,           &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)

       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_TEMPERATURE' ) then
! Lili NOTE: T in model variable is T-T0 (T0 is Tbase)
!   without T0, the computation of alpha is not impacted, 
!   since (ytrue - yfmean), var(y), cov(x,y) are not impacted without adding T0.
!   HOWEVER, the computation of dewpoint and specific humidity have been impacted (later 'elseif')
         obs%dom(id)%obs_size(3,ind) = 1
         ! V index --- 11 
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy) =           &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%ncopy,11)
         call draw_perfect_obs_3d(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,           &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)

       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_DEWPOINT' ) then
         obs%dom(id)%obs_size(3,ind) = 1
         ! PH index ---- 12
         allocate(xtrue3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xfmean3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xfspd3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xprior3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,ens_size))
         call get_model_variable(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,    &
                                 wrf%dom(id)%var_size(3,12),ens_size,             &
                                 wrf%dom(id)%description(12),                     &
                                 wrf%dom(id)%variables(:,:,:,:,12),               &
                                 xtrue3d,xfmean3d,xfspd3d,xprior3d)
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1) =              &
              xtrue3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,2) =              &
              xfmean3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,3) =              &
              xfspd3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,4:ens_size+3) =   &
              xprior3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:ens_size)
         ! P will be used, so add PHB to PH, but PHB needs be interpolated from full-level to half-level
         do k = 1, wrf%dom(id)%bt
            do j = 1, wrf%dom(id)%sn
               do i = 1, wrf%dom(id)%we
                  wrf_var_3d(i,j,k) = (wrf%dom(id)%phb(i,j,k) + wrf%dom(id)%phb(i,j,k+1))/2.0_r8
               enddo
            enddo
         enddo
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1) =             &
              wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1) + wrf_var_3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,2) =             &
              wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,2) + wrf_var_3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         do k = 4, ens_size+3
            wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,k) =          &
              wrf_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,k) + wrf_var_3d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2)
         enddo
         ! T index --- 11, only use 1st copy 
         wrf_var_3d_2(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2) =             &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1,11)
         ! T needs adding on T0
         wrf_var_3d_2(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2) =             &
              wrf_var_3d_2(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2) + 300.0_r8
         ! Q index --- 13 
         wrf_var_4d_2(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:wrf%dom(id)%ncopy) =         &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:wrf%dom(id)%ncopy,13)
         call draw_perfect_obs_3d_dewpoint(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,     &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,wrf_var_4d_2,wrf_var_3d_2,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)

         deallocate(xtrue3d, xfmean3d, xfspd3d, xprior3d)

       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_SPECIFIC_HUMIDITY' ) then
         obs%dom(id)%obs_size(3,ind) = 1
         ! PH index ---- 12
         allocate(xtrue3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xfmean3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         ...
         wrf_var_4d_2(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:wrf%dom(id)%ncopy) =         &
              wrf%dom(id)%variables(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:wrf%dom(id)%ncopy,13)
         call draw_perfect_obs_3d_q(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,     &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,wrf_var_4d_2,wrf_var_3d_2,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)
...
!         obs%dom(id)%obs_size(3,ind) = wrf%dom(id)%bt
   ...
   deallocate(wrf_var_2d, wrf_var_3d, wrf_var_3d_2, wrf_var_4d, wrf_var_4d_2)
   deallocate(obs_var_3d, obs_var_4d)

enddo

print *, 'finish collecting obs'
...
!-------------------------------------------------------
! Lili test: compute the inflation correction for spread
!-------------------------------------------------------
do id = 1, wrf_ndom

   allocate(obs%dom(id)%obs_inf_corr(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,obs%dom(id)%type_size))
   allocate(wrf%dom(id)%var_inf_corr(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,wrf%dom(id)%number_of_wrf_variables))

   ! initialize these inflation correction values
   obs%dom(id)%obs_inf_corr(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:obs%dom(id)%type_size) = 1.0_r8
   wrf%dom(id)%var_inf_corr(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,1:wrf%dom(id)%bt,1:wrf%dom(id)%number_of_wrf_variables) = 1.0_r8
   ...
   do idobs = 7, 7
         ...
         ! compute alpha for subset of distance = 0 (diagonal term: correction for spread)
         do j = start_j, end_j
!            do i = nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
            do i = start_i, end_i 
               if ( obs%dom(id)%obs_val(i,j,levk1,1,idobs) /= missing_r8 ) then
                  !!! get one obs information ready
                  ytrue        = obs%dom(id)%obs_val(i,j,levk1,1,idobs)
                  yfmean       = obs%dom(id)%obs_val(i,j,levk1,2,idobs)
                  yprior_var   = obs%dom(id)%obs_val(i,j,levk1,3,idobs) * obs%dom(id)%obs_val(i,j,levk1,3,idobs)
                  Robs         = obs%dom(id)%obs_val(i,j,levk1,obs%dom(id)%obscopysize,idobs)
                  ypost_var    = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
!                  allocate(yprior(ens_size))
!                  yprior(1:ens_size) = obs%dom(id)%obs_val(i,j,levk1,4:ens_size+3,idobs)

                  xt_xfmean = ytrue - yfmean
                  reg_coef  = yprior_var/(yprior_var+Robs)
                  ccc       = xt_xfmean * reg_coef * xt_xfmean
                  ddd       = (reg_coef**2) * (xt_xfmean**2 + Robs)

                  sublocnomi   = sublocnomi + ccc
                  sublocdenomi = sublocdenomi + ddd
...
!                  deallocate(yprior)
            ...
            enddo   ! i
         enddo      ! j
         sublocalpha = sublocnomi / sublocdenomi

! Lili test
print *, 'sublocalpha = ', sublocalpha

         ! compute the inflation correction for each grid point given one variable kind
         do j = start_j, end_j 
            do i = start_i, end_i 
               if ( obs%dom(id)%obs_val(i,j,levk1,1,idobs) /= missing_r8 ) then
                  ...
                  obs%dom(id)%obs_inf_corr(i,j,levk1,idobs) = sublocalpha / ((1-sublocalpha)*yprior_var/Robs + 1.0_r8)

!               else
!                  obs%dom(id)%obs_inf_corr(i,j,1,idobs) = 1.0_r8
               endif
            enddo   ! i
         enddo      ! j

         ! put the inflation correction for model variable too
         ind = -1
         if ( trim(obs%dom(id)%obstype_metadata(idobs)) == 'FAKE_SFC_U_WIND_COMPONENT' ) then
            ind = 8
         elseif ( trim(obs%dom(id)%obstype_metadata(idobs)) == 'FAKE_SFC_V_WIND_COMPONENT' ) then
            ind = 9
         elseif ( trim(obs%dom(id)%obstype_metadata(idobs)) == 'FAKE_SFC_TEMPERATURE' ) then
            ind = 11
         elseif ( trim(obs%dom(id)%obstype_metadata(idobs)) == 'FAKE_SFC_SPECIFIC_HUMIDITY' ) then
            ind = 13
         endif
! Lili test
print *, 'ind = ', ind
         if ( ind > 0 ) then
            wrf%dom(id)%var_inf_corr(start_i:end_i,start_j:end_j,levk1,ind) =           &
                obs%dom(id)%obs_inf_corr(start_i:end_i,start_j:end_j,levk1,idobs)
         endif
print *, 'infcorr = ', obs%dom(id)%obs_inf_corr(100,70,levk1,idobs),          &
                          wrf%dom(id)%var_inf_corr(100,70,levk1,ind)
! Lili test: check whether alpha = 1.0 at distance 0
         sublocnomi   = 0.0_r8
         sublocdenomi = 0.0_r8
         ! compute alpha for subset of distance = 0 (diagonal term: correction for spread)
         do j = start_j, end_j
!            do i = nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
            do i = start_i, end_i 
               if ( obs%dom(id)%obs_val(i,j,levk1,1,idobs) /= missing_r8 ) then
                  ...
                  ypost_var    = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
!                  allocate(yprior(ens_size))
!                  yprior(1:ens_size) = obs%dom(id)%obs_val(i,j,1,4:ens_size+3,idobs)

                  xt_xfmean = ytrue - yfmean
                  reg_coef  = obs%dom(id)%obs_inf_corr(i,j,levk1,idobs)*yprior_var/(obs%dom(id)%obs_inf_corr(i,j,levk1,idobs)*yprior_var+Robs)
                  ccc       = xt_xfmean * reg_coef * xt_xfmean
                  ddd       = (reg_coef**2) * (xt_xfmean**2 + Robs)
...
! Lili test 
print *, 'sublocalpha inf corr = ', sublocalpha
...
!-------------------------------------------------------
LoopDomain: do id = 1, wrf_ndom 

   ! LoopObsTypes: do idobs = 1, obs%numobstype
   LoopObsTypes: do idobs = 7, 7 
   ! Lili test
   print *, 'Now processing obs type ', trim(obs%dom(id)%obstype_metadata(idobs))

         ndims = 2
         if ( emp_loc_kind == 1 ) then
            allocate(localization%num(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%nominator(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%denominator(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%alpha1(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%alpha2(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%correlation(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            localization%num(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)          = 0
            localization%nominator(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)    = 0.0
            localization%denominator(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)  = 0.0
            localization%alpha1(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)       = 0.0
            localization%alpha2(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)       = 0.0
            localization%correlation(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)  = 0.0
         elseif ( emp_loc_kind == 2 ) then
            allocate(localization%num(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%nominator(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%denominator(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%alpha1(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%alpha2(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            allocate(localization%correlation(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            localization%num(1:2*locnumvert+1, 1:ndims,1:wrf%dom(id)%number_of_wrf_variables)         = 0
            localization%nominator(1:2*locnumvert+1, 1:ndims,1:wrf%dom(id)%number_of_wrf_variables)   = 0.0
            localization%denominator(1:2*locnumvert+1, 1:ndims,1:wrf%dom(id)%number_of_wrf_variables) = 0.0
            localization%alpha1(1:2*locnumvert+1,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)       = 0.0
            localization%alpha2(1:2*locnumvert+1,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)       = 0.0
            localization%correlation(1:2*locnumvert+1,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)  = 0.0
         elseif ( emp_loc_kind == 3 ) then
...
!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,inddim,ndims,indk1,indk2,iunit,writelenloc,var_name_tmp,obslon,obslat,varlon,varlat,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xt_xfmean,covxy,deltaxmean,ilocind,sublocnum,sublocnomi,sublocalpha,sublocdenomi,wrf_var_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)

! Lili test
!      LoopVarTypes: do inddim = 8, wrf%dom(id)%number_of_wrf_variables 
      LoopVarTypes: do inddim = 8, 8
print *, 'processing variable ', trim(wrf%dom(id)%description(inddim))

         !!! get model state variable ready
         allocate(xtrue3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xfmean3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xfspd3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt))
         allocate(xprior3d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,ens_size))
!        allocate(wrf_var_1d(wrf%dom(id)%bt))
         call get_model_variable(mlevk1,mlevk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,    &
                                 wrf%dom(id)%var_size(3,inddim),ens_size,             &
                                 wrf%dom(id)%description(inddim),                     &
                                 wrf%dom(id)%variables(:,:,:,:,inddim),               &
                                 xtrue3d,xfmean3d,xfspd3d,xprior3d)

! Lili test
print *, 'starting obs loop over grid points'
...
!            do i = nbdy_nouse+1, wrf%dom(id)%we-nbdy_nouse
            do i = start_i, end_i
               if ( obs%dom(id)%obs_val(i,j,levk1,1,idobs) /= missing_r8 ) then
                  !!! get one obs information ready
! *** LILI NOTE ***
! obs only for one level only, here use mlevk1 only
                  ytrue        = obs%dom(id)%obs_val(i,j,levk1,1,idobs)
                  yfmean       = obs%dom(id)%obs_val(i,j,levk1,2,idobs)
                  yprior_var   = obs%dom(id)%obs_val(i,j,levk1,3,idobs) * obs%dom(id)%obs_val(i,j,levk1,3,idobs)
                  Robs         = obs%dom(id)%obs_val(i,j,levk1,obs%dom(id)%obscopysize,idobs)
                  ypost_var    = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
! Lili test
!print *, 'obs diag ', ytrue, yfmean, yprior_var, Robs
                  if ( idobs == 1 ) then
                     obshgt = wrf%dom(id)%mhgt(i,j,1,1)
                  elseif ( (idobs == 2) .or. (idobs == 3) ) then
                     obshgt = wrf%dom(id)%mhgt(i,j,1,2)
                  elseif ( (idobs ==4) .or. (idobs == 5) .or. (idobs == 6) ) then
                     obshgt = wrf%dom(id)%mhgt(i,j,1,3)
                  elseif ( (idobs == 7) .or. (idobs == 8) ) then
                     obshgt = wrf%dom(id)%mhgt(i,j,1,2)
                  elseif ( (idobs ==9) .or. (idobs == 10) .or. (idobs == 11) ) then
                     obshgt = wrf%dom(id)%mhgt(i,j,1,3)
                  else
                     print *, 'ERROR: obs height is not defined!'
                  endif
                  allocate(yprior(ens_size))
                  yprior(1:ens_size) = obs%dom(id)%obs_val(i,j,levk1,4:ens_size+3,idobs)
...
!                    dist   = wrf_var_1d(k) - obshgt
                     ...
                     xt_xfmean = xtrue3d(ii,jj,kk) - xfmean3d(ii,jj,kk)
                     covxy = comp_cov(ens_size, xfmean3d(ii,jj,kk), xprior3d(ii,jj,kk,:), yfmean, yprior)
                     ! compute deltaxmean = cov(x,y) * (yobs - yfmean) / (var(y) + Robs)
                     !                    = covxy * yinno_norm
                     corr = covxy / (sqrt(yprior_var) * xfspd3d(ii,jj,kk))

! Jeff's algorithm
!                     reg_coef = covxy / yprior_var
!                     ccc = reg_coef * (ypost_var / yprior_var - 1.0) * yfmean
!                     ddd = reg_coef * ypost_var / Robs
!
!                     sublocnomi = xt_xfmean * (ccc + ddd * ytrue)
!                     sublocdenomi = ccc**2 + 2.0 * ccc * ddd * ytrue + ddd**2 * (Robs + ytrue**2)

                     ccc     = sqrt(obs%dom(id)%obs_inf_corr(i,j,levk1,idobs)) *               &
                                  sqrt(wrf%dom(id)%var_inf_corr(ii,jj,levk1,inddim)) * covxy
                     ddd     = obs%dom(id)%obs_inf_corr(i,j,levk1,idobs) * yprior_var + Robs
                     reg_coef     = ccc / ddd
                     sublocnomi   = xt_xfmean * reg_coef * (ytrue - yfmean)
                     sublocdenomi = (reg_coef**2) * ( (ytrue - yfmean)**2 + Robs)

! Lili test
!print *, 'emp diag ', sublocnomi, sublocdenomi
!pause
                     if ( abs(sublocdenomi) > 0.0000000001_r8 ) then
                        sublocalpha = sublocnomi/sublocdenomi
! Lili test
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
                        ! record in distance
                        localization%num(ilocind,1,inddim) = localization%num(ilocind,1,inddim) + 1
                        localization%nominator(ilocind,1,inddim) = localization%nominator(ilocind,1,inddim) + sublocnomi
                        localization%denominator(ilocind,1,inddim) = localization%denominator(ilocind,1,inddim) + sublocdenomi
...
!                        localization%num(k,2,inddim) = localization%num(k,2,inddim) + 1
!                        localization%nominator(k,2,inddim) = localization%nominator(k,2,inddim) + sublocnomi
!                        localization%denominator(k,2,inddim) = localization%denominator(k,2,inddim) + sublocdenomi
!                        localization%alpha1(k,2,inddim) = localization%alpha1(k,2,inddim) + sublocalpha
!                        localization%alpha2(k,2,inddim) = localization%alpha2(k,2,inddim) + sublocalpha**2
!                        localization%correlation(k,2,inddim) = localization%correlation(k,2,inddim) + abs(corr)
   
                        ! record terrain less than 100 m
                        if ( obshgt <= 100.0 ) then
                           ! record in distance
                           localization%num(ilocind,2,inddim) = localization%num(ilocind,2,inddim) + 1
                           localization%nominator(ilocind,2,inddim) = localization%nominator(ilocind,2,inddim) + sublocnomi
                           localization%denominator(ilocind,2,inddim) = localization%denominator(ilocind,2,inddim) + sublocdenomi
                           localization%alpha1(ilocind,2,inddim) = localization%alpha1(ilocind,2,inddim) + sublocalpha
                           localization%alpha2(ilocind,2,inddim) = localization%alpha2(ilocind,2,inddim) + sublocalpha**2
                           localization%correlation(ilocind,2,inddim) = localization%correlation(ilocind,2,inddim) + abs(corr)

!                           ! record in model level
!                           localization%num(k,4,inddim) = localization%num(k,4,inddim) + 1
!                           localization%nominator(k,4,inddim) = localization%nominator(k,4,inddim) + sublocnomi
!                           localization%denominator(k,4,inddim) = localization%denominator(k,4,inddim) + sublocdenomi
!                           localization%alpha1(k,4,inddim) = localization%alpha1(k,4,inddim) + sublocalpha
!                           localization%alpha2(k,4,inddim) = localization%alpha2(k,4,inddim) + sublocalpha**2
!                           localization%correlation(k,4,inddim) = localization%correlation(k,4,inddim) + abs(corr)
                        endif
                     endif
...
! Lili test
!      do inddim = 8, wrf%dom(id)%number_of_wrf_variables
       do inddim = 8, 8
         writelenloc = locnum
         write(idchar, '(i1.1)') id
         ...
         do ind = 1, ndims
            ...
            write(iunit, FMT=locreal_format) localization%nominator(1:writelenloc,ind,inddim)
...
subroutine draw_perfect_obs_3d(k1,k2,we,sn,bt,nbdyoff,ens_size,description,variable,obs_var_4d)
! some state variables are at mass points
! some are not, these need interpolation

integer,            intent(in)  :: k1, k2, we, sn, bt, nbdyoff, ens_size
character(len=129), intent(in)  :: description
real(r8),           intent(in)  :: variable(:,:,:,:)
real(r8),           intent(out) :: obs_var_4d(:,:,:,:)
...
obs_var_4d(1:we,1:sn,k1:k2,1:obscopysize) = missing_r8

if ( trim(description) == "FAKE_SFC_U_WIND_COMPONENT" )  then

   do k = k1, k2
      do j = nbdyoff+1, sn-nbdyoff
         do i = nbdyoff+1, we-nbdyoff

            obs_var_4d(i,j,k,1) = (variable(i,j,k,1) + variable(i+1,j,k,1)) / 2.0_r8
            obs_var_4d(i,j,k,2) = (variable(i,j,k,2) + variable(i+1,j,k,2)) / 2.0_r8
            do ind = 1, ens_size
               obs_var_4d(i,j,k,3+ind) = (variable(i,j,k,3+ind) + variable(i+1,j,k,3+ind)) / 2.0_r8
            enddo
            ! compute ensemble spread
            varspd = 0.0_r8
            do ind = 1, ens_size
              varspd = varspd + (obs_var_4d(i,j,k,3+ind)-obs_var_4d(i,j,k,2))**2
            enddo
            varspd = varspd / (ens_size - 1)
            obs_var_4d(i,j,k,3) = sqrt(varspd)
            ! compute obs error variance
            pres = 1000.00_r8              ! this pressure is not really used in land_wind_error
            oerr = land_wind_error(pres)
            obs_var_4d(i,j,k,obscopysize) = oerr * oerr

         enddo
      enddo
   enddo

elseif ( trim(description) == "FAKE_SFC_V_WIND_COMPONENT" )  then

   do k = k1, k2
      do j = nbdyoff+1, sn-nbdyoff 
         do i = nbdyoff+1, we-nbdyoff 

            obs_var_4d(i,j,k,1)  = (variable(i,j,k,1) + variable(i,j+1,k,1)) / 2.0_r8
            obs_var_4d(i,j,k,2) = (variable(i,j,k,2) + variable(i,j+1,k,2)) / 2.0_r8
            do ind = 1, ens_size
               obs_var_4d(i,j,k,3+ind) = (variable(i,j,k,3+ind) + variable(i,j+1,k,3+ind)) / 2.0_r8
            enddo
            ! compute ensemble spread
            varspd = 0.0_r8
            do ind = 1, ens_size
...
elseif ( trim(description) == "FAKE_SFC_TEMPERATURE" ) then

    obs_var_4d(nbdyoff+1:we-nbdyoff,nbdyoff+1:sn-nbdyoff,k1:k2,1:3+ens_size)  =      &
        variable(nbdyoff+1:we-nbdyoff,nbdyoff+1:sn-nbdyoff,k1:k2,1:3+ens_size)
    ! compute obs error variance
    pres = 1000.00_r8              ! this pressure is not really used in land_temp_error
    oerr = land_temp_error(pres)
    obs_var_4d(nbdyoff+1:we-nbdyoff,nbdyoff+1:sn-nbdyoff,k1:k2,obscopysize) = oerr * oerr
...
return
end subroutine draw_perfect_obs_3d 



subroutine draw_perfect_obs_3d_dewpoint(k1,k2,we,sn,bt,nbdyoff,ens_size,description,pres_var_4d,qv_var_4d,t_var_3d,obs_var_4d)

integer,            intent(in)  :: k1, k2, we, sn, bt, nbdyoff, ens_size
character(len=129), intent(in)  :: description
real(r8),           intent(in)  :: pres_var_4d(:,:,:,:)   ! pressure
real(r8),           intent(in)  :: qv_var_4d(:,:,:,:)     ! qvapor
real(r8),           intent(in)  :: t_var_3d(:,:,:)        ! temperature
real(r8),           intent(out) :: obs_var_4d(:,:,:,:)

integer    :: i, j, k, ind, obscopysize
real(r8)   :: var1, var2, var3, var4, varspd
real(r8)   :: pres, qv, td, tair, rh, qerr, oerr
real(r8), allocatable :: temp_var_1d(:)
...
if ( trim(description) == "FAKE_SFC_DEWPOINT" )  then

    do k = k1, k2
       do j = nbdyoff+1, sn-nbdyoff
          do i = nbdyoff+1, we-nbdyoff
             ! compute the truth
             pres = pres_var_4d(i,j,k,1)
             qv   = qv_var_4d(i,j,k,1) 
             if ( qv >= 0.0_r8 .and. qv < 1.0_r8) then
                td = compute_dewpoint(pres, qv)
                obs_var_4d(i,j,k,1) = td
             endif
             ...
             ! compute ensemble members
             do ind = 1, ens_size
                pres = pres_var_4d(i,j,k,ind+3)
                qv   = qv_var_4d(i,j,k,ind+3)

                if ( qv >= 0.0_r8 .and. qv < 1.0_r8) then
                   td = compute_dewpoint(pres, qv)
                   obs_var_4d(i,j,k,ind+3) = td
                endif
             enddo

             ! compute ensemble mean
             temp_var_1d(1:ens_size) = obs_var_4d(i,j,k,4:ens_size+3)
             obs_var_4d(i,j,k,2) = sum(temp_var_1d) / ens_size

             ! compute ensemble spread
             var1 = 0.0_r8
             do ind = 1, ens_size
                var1 = var1 + (temp_var_1d(ind)-obs_var_4d(i,j,k,2))**2
             enddo
             var1 = var1 / (ens_size - 1)
             obs_var_4d(i,j,k,3) = sqrt(var1)
    ...
    print *, 'ERROR: draw_perfect_obs_3d_dewpoint'
    print *, 'ERROR: wrong obs type', trim(description)
...
return
end subroutine draw_perfect_obs_3d_dewpoint



subroutine draw_perfect_obs_3d_q(k1,k2,we,sn,bt,nbdyoff,ens_size,description,pres_var_4d,qv_var_4d,t_var_3d,obs_var_4d)
...
integer    :: i, j, k, ind, obscopysize
real(r8)   :: var1, var2, var3, var4, varspd
real(r8)   :: pres, qv, td, tair, rh, qsat, qerr, oerr
real(r8), allocatable :: temp_var_1d(:)
...
if ( trim(description) == "FAKE_SFC_SPECIFIC_HUMIDITY" )  then
    ...
    print *, 'ERROR: draw_perfect_obs_3d_q'
    print *, 'ERROR: wrong obs type', trim(description)
...
return
end subroutine draw_perfect_obs_3d_q
