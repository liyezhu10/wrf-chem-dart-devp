...
integer           :: locnum_vert      = 600
real(r8)          :: locrad_vert      = 0.01
integer           :: wrf_ndom         = 1
...
! integer           :: lev_s            = 7
! integer           :: lev_e            = 7
! integer           :: obslev           = 4      ! change obs_ind below
 integer           :: obslev_s         = 1
 integer           :: obslev_e         = 1
! integer           :: varlev           = 3      ! change var_ind below
 integer           :: varlev_s         = 1
 integer           :: varlev_e         = 1
integer           :: indobs_s         = 2
integer           :: indobs_e         = 2
...
type localization_data
    ...
    real(r8), dimension(:), pointer       :: locind_vert
...
! 1. 1D array
   ...
   allocate(cam_var_1d(1:cam%nlev))
   cam_var_1d = cam%hyam
   do i = 1, cam%nlev
   ...
   ! reshape data from top-bottom to bottom-top
   allocate(cam_var_1d(1:cam%nilev))
   cam_var_1d = cam%hyai
   do i = 1, cam%nilev
...
! Initialize locind which contains the radiance for each subset
   ...
   enddo
   allocate(localization%locind_vert(locnum_vert))
   do ind = 1, locnum_vert
      localization%locind_vert(ind) = locrad_vert *real(ind)
   enddo

! allocate and initialize localization variables
   ndims = locnum_vert 
   allocate(localization%num(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%sumx(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%sumy(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%sumx2(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%sumy2(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%numerator(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%denominator(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%alpha1(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%alpha2(locnum,locnum_vert,cam%number_of_state_variables))
   allocate(localization%correlation(locnum,locnum_vert,cam%number_of_state_variables))
   localization%num(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)          = 0
   localization%sumx(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)         = 0.0
   localization%sumy(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)         = 0.0
   localization%sumx2(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)        = 0.0
   localization%sumy2(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)        = 0.0
   localization%numerator(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)    = 0.0
   localization%denominator(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)  = 0.0
   localization%alpha1(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)       = 0.0
   localization%alpha2(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)       = 0.0
   localization%correlation(1:locnum,1:locnum_vert,1:cam%number_of_state_variables)  = 0.0


!!!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,obslevk,varlevk,inddim,ndims,indk1,indk2,iunit,writelenloc,var_name_tmp,obslon,obslat,varlon,varlat,obsi,obsj,obsk,obspres,varpres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,dist_vert,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,ilocind_vert,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,cam_var_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)
...
!pause
               ...
               ! get obs pressure (use ensemble mean of PS)
               obspres = cam%hyam(k) * cam%P0(1) + cam%hybm(k) * cam%variables(i,j,1,2,1)
!print *, 'obspres = ', obspres

               ! loop target variable grid points
               do kk = varlev_s, varlev_e
!               do varlevk = 1, varlev
...
!pause
                     ...
                     ! get variable pressure (use ensemble mean of PS)
!print *, 'PS = ', cam%variables(ii,jj,1,2,1)
                     varpres = cam%hyam(kk) * cam%P0(1) + cam%hybm(kk) * cam%variables(ii,jj,1,2,1)
!print *, 'varpres = ', varpres
...
!pause
                     ...
                     ! get the vertical distance (log10(P))
!                     if ( abs(obspres-varpres) <= 10.0 ) then
!                        dist_vert = 0
!                     else
!                        dist_vert = log10(abs(obspres-varpres))
!                     endif
                     dist_vert = abs(log10(obspres)-log10(varpres))
!print *, 'dist_vert = ', dist_vert
!print *, 'locind_vert = ', localization%locind_vert(1:5)
!pause
                     call find_index_in_locind(ilocind_vert, dist_vert, locnum_vert, localization%locind_vert)
!print *, 'ilocind_vert = ', ilocind_vert
!pause
...
!pause
                     ...
                     ! record in distance
                     localization%num(ilocind,ilocind_vert,inddim) = localization%num(ilocind,ilocind_vert,inddim) + 1

                     localization%sumx(ilocind,ilocind_vert,inddim) = localization%sumx(ilocind,ilocind_vert,inddim) + sublocx
                     localization%sumy(ilocind,ilocind_vert,inddim) = localization%sumy(ilocind,ilocind_vert,inddim) + sublocy

                     localization%sumx2(ilocind,ilocind_vert,inddim) = localization%sumx2(ilocind,ilocind_vert,inddim) + sublocx2
                     localization%sumy2(ilocind,ilocind_vert,inddim) = localization%sumy2(ilocind,ilocind_vert,inddim) + sublocy2

                     localization%numerator(ilocind,ilocind_vert,inddim) = localization%numerator(ilocind,ilocind_vert,inddim)     &
                                                                    + sublocnumer
                     localization%denominator(ilocind,ilocind_vert,inddim) = localization%denominator(ilocind,ilocind_vert,inddim) &
                                                                      + sublocdenom
                     localization%alpha1(ilocind,ilocind_vert,inddim) = localization%alpha1(ilocind,ilocind_vert,inddim) + sublocalpha
                     localization%alpha2(ilocind,ilocind_vert,inddim) = localization%alpha2(ilocind,ilocind_vert,inddim) + sublocalpha**2
                     localization%correlation(ilocind,ilocind_vert,inddim) = localization%correlation(ilocind,ilocind_vert,inddim) + abs(corr)
                     endif
...
!            endif
            var_name_tmp = trim(obs%obstype_metadata(idobs))//'_'//         &
                           trim(cam%description(inddim))//'_3d'
            iunit = 10 + inddim
            open(iunit, FILE=var_name_tmp, FORM='FORMATTED')

            allocate(numtmp(writelenloc))
            do i = 1, locnum_vert
               numtmp(1:writelenloc) = localization%num(1:writelenloc,i,inddim)
               write(iunit, FMT=locreal_format) numtmp
            enddo
            deallocate(numtmp)
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%alpha1(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,i,inddim)
            enddo
            do i = 1, locnum_vert
               write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,i,inddim)
            enddo
...
subroutine find_index_in_locind_vert(indfind, dist, locnum, locind)
! deal with locind has only positive subset
...
return
end subroutine find_index_in_locind_vert
