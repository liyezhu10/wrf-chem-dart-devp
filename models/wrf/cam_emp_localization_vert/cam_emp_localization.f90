...
integer           :: locnum           = 30      ! vertical levels of CAM (NOTE: all variables are on lev instead of ilev)
...
integer           :: distlocnum       = 30      !901     ! locnum for distance between obs and var abs(log(P_obs)-log(P_var))
real(r8)          :: distdloc         = 0.01
integer           :: obsregion        = 5       ! obs is divided to 'tropics','mid-latitude', and 'pole'
integer           :: wrf_ndom         = 1
...
real(r8)          :: R_P              = 10000.0_r8 ! obs error variance of Psfc
...
real(r8)          :: pressure_top     = 20000.0_r8  ! top pressure for 'TPW'
...
real(r8)         :: ref_P, surf_P, obs_P, var_P, dist_logP
integer          :: obslat_s, obslat_e, iregion

logical          :: is_lev0
character(len=3) :: idchar, idchar1
...
! 1. 1D array
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, cam%hyam),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, cam%hybm),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, cam%hyai),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, cam%hybi),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, cam%P0),  &
...
LoopObsTypes: do idobs = indobs_s, indobs_e
   ...
   ! U is on slat grid (lat has 192 dimensinos, but slat has 191 dimensions)
   obslat_s = 1
   if ( idobs == 3 ) then
      obslat_e = cam%nslat
   else
...
!print *, 'obslat beg end = ', obslat_s, obslat_e
!pause
...
!   do ind = 1, locnum
!      localization%locind(ind) = ind
!   enddo

! allocate and initialize localization variables
!   ndims = lev_e - lev_s + 1
!   ndims = cam%nlev
   allocate(localization%num(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%sumx(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%sumy(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%sumx2(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%sumy2(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%numerator(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%denominator(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%alpha1(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%alpha2(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%abscorr(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   allocate(localization%correlation(distlocnum,obslocnum,obsregion,cam%number_of_state_variables))
   localization%num(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)          = 0
   localization%sumx(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)         = 0.0
   localization%sumy(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)         = 0.0
   localization%sumx2(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)        = 0.0
   localization%sumy2(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)        = 0.0
   localization%numerator(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)    = 0.0
   localization%denominator(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)  = 0.0
   localization%alpha1(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)       = 0.0
   localization%alpha2(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)       = 0.0
   localization%abscorr(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)      = 0.0
   localization%correlation(1:distlocnum,1:obslocnum,1:obsregion,1:cam%number_of_state_variables)  = 0.0


!!!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,inddim,ndims,iregion,indk1,indk2,iunit,writelenloc,ref_P,surf_P,obs_P,var_P,dist_logP,var_name_tmp,obslon,obslat,varlon,varlat,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,iobslocind,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,cam_var_1d,qv_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)

!   LoopVarTypes: do inddim = 1, cam%number_of_state_variables
   LoopVarTypes: do inddim = indvar_s, indvar_e    !idobs, idobs   !indvar_s, indvar_e
      print *, 'processing variable ', trim(cam%description(inddim))

      ! loop obs grid points
      do k = lev_s, lev_e
         do j = obslat_s, obslat_e 

            obslat = cam%var_lat(j,idobs)
            if ( obslat <= -66.5_r8 ) then
               iregion = 1
            elseif ( obslat > -66.5_r8 .and. obslat <= -23.5_r8 ) then
               iregion = 2
            elseif ( obslat > -23.5_r8 .and. obslat <= 23.5_r8 ) then
               iregion = 3
            elseif ( obslat > 23.5_r8 .and. obslat <= 66.5_r8 ) then
               iregion = 4
            elseif ( obslat > 66.5_r8 ) then
               iregion = 5
            endif
!print *, 'diag = ', j, obslat, iregion
!pause
            do i = 1, cam%nlon

               ! compute the pressure of obs
               ref_P  = cam%P0(1)
               surf_P = cam%variables(i,j,1,1,1)
               obs_P  = cam_hyam(k)*ref_P + cam_hybm(k)*surf_P
...
!                  allocate(yprior(ens_size))
                  ...
                  do indk1 = 1, ens_size
                     ...
                     allocate(qv_1d(cam%nlev))
                     ref_P  = cam%P0(1)
                     surf_P = cam%variables(i,j,1,3+indk1,1)
                     call get_pressure(ref_P, surf_P, cam%nlev,                  &
                                       cam_hyam, cam_hybm, cam_var_1d)
                     qv_1d =  cam%variables(i,j,:,3+indk1,5)
                     call get_expected_tpw(cam_var_1d, qv_1d, cam%nlev,          &
                                           yprior(indk1), pressure_top)
                     deallocate(cam_var_1d, qv_1d)
                  enddo
                  yfmean = sum(yprior)/ens_size
                  yprior_var = 0.0_r8
                  do indk1 = 1, ens_size
                     yprior_var = yprior_var + (yprior(indk1)-yfmean)**2
                  enddo
                  yprior_var = yprior_var/(ens_size-1)
...
!               obslat = cam%var_lat(j,idobs) * DEG2RAD
               ...
               do kk = 1, cam%nlev 
                     ! compute the pressure of obs
                     ref_P  = cam%P0(1)
                     surf_P = cam%variables(i,j,1,2,1)
                     var_P  = cam_hyam(kk)*ref_P + cam_hybm(kk)*surf_P

                     ! get target variable information ready
                     xtrue      = cam%variables(i,j,kk,1,inddim)
                     xfmean     = cam%variables(i,j,kk,2,inddim)
                     xprior_var = cam%variables(i,j,kk,3,inddim)**2
                     allocate(xprior(ens_size))
                     xprior(1:ens_size) = cam%variables(i,j,kk,3+1:3+ens_size,inddim)

!print *, 'var info = ', xtrue, xfmean, xprior_var
!print *, 'var prior = ', xprior(1:ens_size:10)


!                     ! NOTE: obs log unit start at 4, end at 13
!                     ! find the obs index 'iobslocind'
!                     if ( log(obs_P) <= 4.0_r8 ) then
!                        iobslocind = 1
!                     else
!                        iobslocind = CEILING((log(obs_P)-4.0_r8)/obsdloc)+1
!                     endif
!                     if ( iobslocind < 0 .or. iobslocind > obslocnum ) then
!                        print *, 'ERROR: wrong index for obs ', obs_P, log(obs_P), iobslocind
!                     endif
                     iobslocind = 1

!print *, 'iobslocind = ', iobslocind
                     ! find the index of the distance between obs and var 'ilocind'
!                     dist_logP = abs(log(obs_P)-log(var_P))
!                     if ( dist_logP <= 0.000000000001_r8 ) then
!                        ilocind = 1
!                     else
!                        ilocind = CEILING(dist_logP/distdloc)+1
!                     endif
!                     if ( ilocind < 0 .or. ilocind > distlocnum ) then
!                        print *, 'ERROR: wrong index for obs ', dist_logP, ilocind
!                     endif
                     ilocind = kk
!print *, 'ilocind = ', ilocind
!print *, 'pressure = ', obs_P, var_P, dist_logP
...
!print *, 'loc = ', xt_xfmean*(covxy*(ytrue-yfmean)/(yprior_var+Robs)),(covxy/(yprior_var+Robs))**2 * ((ytrue-yfmean)**2 + Robs)
!print *, 'loc = ', sublocnumer, sublocdenom, sublocalpha
!pause

                     ! record in distance
                     localization%num(ilocind,iobslocind,iregion,inddim) = localization%num(ilocind,iobslocind,iregion,inddim) + 1

                     localization%sumx(ilocind,iobslocind,iregion,inddim) = localization%sumx(ilocind,iobslocind,iregion,inddim) + sublocx
                     localization%sumy(ilocind,iobslocind,iregion,inddim) = localization%sumy(ilocind,iobslocind,iregion,inddim) + sublocy

                     localization%sumx2(ilocind,iobslocind,iregion,inddim) = localization%sumx2(ilocind,iobslocind,iregion,inddim) + sublocx2
                     localization%sumy2(ilocind,iobslocind,iregion,inddim) = localization%sumy2(ilocind,iobslocind,iregion,inddim) + sublocy2

                     localization%numerator(ilocind,iobslocind,iregion,inddim) = localization%numerator(ilocind,iobslocind,iregion,inddim)     &
                                                                      + sublocnumer
                     localization%denominator(ilocind,iobslocind,iregion,inddim) = localization%denominator(ilocind,iobslocind,iregion,inddim) &
                                                                        + sublocdenom
!                     localization%alpha1(ilocind,k,inddim) = localization%alpha1(ilocind,k,inddim) + sublocalpha
!                     localization%alpha2(ilocind,k,inddim) = localization%alpha2(ilocind,k,inddim) + sublocalpha**2
                     localization%abscorr(ilocind,iobslocind,iregion,inddim) = localization%abscorr(ilocind,iobslocind,iregion,inddim) + abs(corr)
                     localization%correlation(ilocind,iobslocind,iregion,inddim) = localization%correlation(ilocind,iobslocind,iregion,inddim) + corr
!print *, 'loc%num = ', localization%num(ilocind,iobslocind,inddim)
!pause
                     endif
...
!   do inddim = 1, cam%number_of_state_variables
   do inddim = indvar_s, indvar_e   !idobs, idobs   !indvar_s, indvar_e
      writelenloc = distlocnum

      do iregion = 1, obsregion
         write(idchar,'(i1)') iregion

         if ( lev_s < 10 ) then
            write(idchar1,'(i1)') lev_s
         else
            write(idchar1,'(i2)') lev_s
         endif

      ! for each obs kind and target variable kind
      ! write localization for each level where obs are 
!!      do k = lev_s, lev_e
!      do k = 1, obslocnum
!         if ( k < 10 ) then
!            write(idchar,'(i1)') k
...
!!print *, 'idchar = ', idchar
         var_name_tmp = trim(obs%obstype_metadata(idobs))//'_'//         &
                        trim(cam%description(inddim))//'_obslev'//trim(idchar1)//'_region'//idchar
...
!pause
         ...
         do k = 1, obslocnum
            ...
            numtmp(1:writelenloc) = localization%num(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) numtmp 
            write(iunit, FMT=locreal_format) localization%sumx(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumy(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumx2(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%sumy2(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%numerator(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%denominator(1:writelenloc,k,iregion,inddim)
!            write(iunit, FMT=locreal_format) localization%alpha1(1:writelenloc,k,iregion,inddim)
!            write(iunit, FMT=locreal_format) localization%alpha2(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%abscorr(1:writelenloc,k,iregion,inddim)
            write(iunit, FMT=locreal_format) localization%correlation(1:writelenloc,k,iregion,inddim)
            deallocate(numtmp)
...
!          hsfc = wrf_var_2d(i,j)
!          altr = compute_altimeter(pres*0.01,hsfc)          
...
!          oerr = land_pres_error(pres*1.0_r8)
