...
type localization_data
    ...
    real(r8), dimension(:,:,:), pointer   :: abscorrelation
...
LoopDomain: do id = dom_start, wrf_ndom 
   ...
   print *, 'Now processing obs type ', trim(obs%dom(id)%obstype_metadata(idobs))
         ...
         if ( emp_loc_kind == 1 ) then
            ...
            allocate(localization%abscorrelation(locnum,ndims,wrf%dom(id)%number_of_wrf_variables))
            ...
            localization%abscorrelation(1:locnum,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)  = 0.0
         elseif ( emp_loc_kind == 2 ) then
...
! Lili test
      LoopVarTypes: do inddim = 6, wrf%dom(id)%number_of_wrf_variables 
...
!        allocate(wrf_var_1d(wrf%dom(id)%bt))
         call get_model_variable(1,1,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,    &
...
!                     sublocdenomi = ccc**2 + 2.0 * ccc * ddd * ytrue + ddd**2 * (Robs + ytrue**2)
                     sublocdenomi = (ccc+ddd*ytrue)**2 + ddd**2 * Robs
! Lili test
...
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
                        ...
                        localization%correlation(ilocind,1,inddim) = localization%correlation(ilocind,1,inddim) + corr
                        localization%abscorrelation(ilocind,1,inddim) = localization%abscorrelation(ilocind,1,inddim) + abs(corr)
! Lili test
!print *, 'loc diag ', localization%num(ilocind,1,inddim)
...
!                        localization%correlation(k,2,inddim) = localization%correlation(k,2,inddim) + abs(corr)
                        ...
                        if ( obshgt <= 100.0 ) then
                           ...
                           localization%correlation(ilocind,2,inddim) = localization%correlation(ilocind,2,inddim) + corr
                           localization%abscorrelation(ilocind,2,inddim) = localization%abscorrelation(ilocind,2,inddim) + abs(corr)
...
! Lili test
      do inddim = 6, wrf%dom(id)%number_of_wrf_variables
!       do inddim = 11, 11
         writelenloc = locnum
         write(idchar, '(i1.1)') id
         ...
         do ind = 1, ndims
            ...
            write(iunit, FMT=locreal_format) localization%abscorrelation(1:writelenloc,ind,inddim)
            deallocate(numtmp)
         enddo
      ...
      deallocate(localization%alpha1, localization%alpha2, localization%correlation, localization%abscorrelation)
