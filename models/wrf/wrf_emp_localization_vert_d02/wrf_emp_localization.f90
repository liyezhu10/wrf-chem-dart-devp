...
LoopDomain: do id = dom_start, wrf_ndom 
   ...
   !         allocate(localization%alpha1(locnum,ndims))
   !         allocate(localization%alpha2(locnum,ndims))
   ...
   !         localization%alpha1(1:locnum,1:ndims) = 0.0
   !         localization%alpha2(1:locnum,1:ndims) = 0.0
         elseif ( emp_loc_kind == 2 ) then
            ...
            allocate(localization%abscorrelation(2*locnumvert+1,ndims,wrf%dom(id)%number_of_wrf_variables))
            ...
            localization%abscorrelation(1:2*locnumvert+1,1:ndims,1:wrf%dom(id)%number_of_wrf_variables)  = 0.0
         elseif ( emp_loc_kind == 3 ) then
...
!$OMP PARALLEL DO PRIVATE(ind,i,j,k,inddim,ndims,indk1,indk2,iunit,writelenloc,var_name_tmp,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xt_xfmean,covxy,deltaxmean,ilocind,sublocnum,sublocnomi,sublocalpha,sublocdenomi,wrf_var_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf)

!     NOTE: for verticala localization, only use 3D model variablers
! Lili test
...
!pause
! NOTE: obshgt is only for SFC OBS for now 
                  if ( idobs == 1 ) then
                  ...
                  allocate(xfspdprf(wrf%dom(id)%bt))
                  allocate(xpriorprf(wrf%dom(id)%bt, ens_size))
                  allocate(wrf_var_1d(wrf%dom(id)%bt))

                  call get_model_variable_profile(real(i),real(j),wrf%dom(id)%bt,wrf%dom(id)%variables(:,:,:,:,inddim),     &
                                                  wrf%dom(id)%description(inddim),ens_size,                     &
                                                  xtrueprf,xfmeanprf,xfspdprf,xpriorprf)
!print *, 'var diag ', xtrueprf(1:3), xfmeanprf(1:3)
!print *, 'var diag ', xpriorprf(1:3,1), xpriorprf(1:3,ens_size)
!pause

                  call get_model_height_profile(real(i),real(j),wrf%dom(id)%bt,                             &
                  ...
                  do k = 1, wrf%dom(id)%bt
                     ...
                     call find_index_in_locind_2d(ilocind, dist, locnumvert, localization%locind)
                     ...
                     corr = covxy / (sqrt(yprior_var) * xfspdprf(k))
...
!print *, 'emp diag ', sublocnomi, sublocdenomi
                     if ( abs(sublocdenomi) > 0.0000000001_r8 ) then
                         ...
                         localization%correlation(k,2,inddim) = localization%correlation(k,2,inddim) + corr
                         localization%abscorrelation(k,2,inddim) = localization%abscorrelation(k,2,inddim) + abs(corr)

                         ! record terrain less than 100 m
                         if ( obshgt <= 100.0 ) then
                             ! record in distance
                             localization%num(ilocind,3,inddim) = localization%num(ilocind,3,inddim) + 1
                             localization%nominator(ilocind,3,inddim) = localization%nominator(ilocind,3,inddim) + sublocnomi
                             localization%denominator(ilocind,3,inddim) = localization%denominator(ilocind,3,inddim) + sublocdenomi
                             localization%alpha1(ilocind,3,inddim) = localization%alpha1(ilocind,3,inddim) + sublocalpha
                             localization%alpha2(ilocind,3,inddim) = localization%alpha2(ilocind,3,inddim) + sublocalpha**2
                             localization%correlation(ilocind,3,inddim) = localization%correlation(ilocind,3,inddim) + corr
                             localization%abscorrelation(ilocind,3,inddim) = localization%abscorrelation(ilocind,3,inddim) + abs(corr)
                             ...
                             localization%correlation(k,4,inddim) = localization%correlation(k,4,inddim) + corr
                             localization%abscorrelation(k,4,inddim) = localization%abscorrelation(k,4,inddim) + abs(corr)
                         endif

                     endif

                  enddo
!print *, 'loc diag ', localization%nominator(1:3,2,inddim), localization%denominator(1:3,2,inddim), localization%alpha1(1:3,2,inddim)
!pause
                  deallocate(xtrueprf, xfmeanprf, xfspdprf, xpriorprf, wrf_var_1d)
                  deallocate(yprior)
...
!       thus put the writting out of OMP loop
      do inddim = 8, wrf%dom(id)%number_of_wrf_variables
         ...
         do ind = 1, ndims
            write(iunit, FMT=locint_format) localization%num(1:writelenloc,ind,inddim)
