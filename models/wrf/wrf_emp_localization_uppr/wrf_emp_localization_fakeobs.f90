...
!                                    obs%dom(id)%type_size)
   ...
   allcoate(obs_var_4d(wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,obs%dom(id)%obscopysize))
      ...
      elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'SFC_SPECIFIC_HUMIDITY' ) then
       ...
       elseif ( trim(obs%dom(id)%obstype_metadata(ind)) == 'FAKE_SFC_DEWPOINT' ) then
         ...
         call draw_perfect_obs_3d_dewpoint(levk1,levk2,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,     &
                    nbdy_nouse,ens_size,obs%dom(id)%obstype_metadata(ind),wrf_var_4d,wrf_var_3d_2,wrf_var_4d_2,obs_var_4d)
         obs%dom(id)%obs_val(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize,ind) =     &
              obs_var_4d(1:wrf%dom(id)%we,1:wrf%dom(id)%sn,levk1:levk2,1:obs%dom(id)%obscopysize)
...
!        allocate(wrf_var_1d(wrf%dom(id)%bt))
         call get_model_variable(15,15,wrf%dom(id)%we,wrf%dom(id)%sn,wrf%dom(id)%bt,    &
...
!            do i = 60, 60
               if ( obs%dom(id)%obs_val(i,j,1,1,idobs) /= missing_r8 ) then
                  ...
                  ytrue        = obs%dom(id)%obs_val(i,j,1,1,idobs)
                  yfmean       = obs%dom(id)%obs_val(i,j,1,2,idobs)
                  yprior_var   = obs%dom(id)%obs_val(i,j,1,3,idobs) * obs%dom(id)%obs_val(i,j,1,3,idobs)
                  Robs         = obs%dom(id)%obs_val(i,j,1,obs%dom(id)%obscopysize,idobs)
    ...
    do k = k1, k2
       do j = nbdyoff+1, sn-nbdyoff
          do i = nbdyoff+1, we-nbdyoff
             ...
             var1 = 0.0_r8
             do k = 1, ens_size
                var1 = var1 + (temp_var_1d(k)-obs_var_4d(i,j,k,2))**2
             enddo
             var1 = var1 / (ens_size - 1)
