...
echo "`date` -- BEGIN ucoam_ASSIMILATE"
...
# of the form "./${CASE}.ucoam_${ensemble_member}.r.2000-01-06-00000.nc"
#
...
# Since the ensemble manager is not used by ucoam_to_dart or dart_to_ucoam,
...
#    You must manually remove the ucoam_inflation_cookie file
#    from the RUNDIR in this case.
#    - OR -
# 2) create a cookie file called RUNDIR/ucoam_inflation_cookie
...
if ( $PRIOR_INF > 0 ) then
   ...
   else if ( -e ../ucoam_inflation_cookie ) then
...
ex_end
      ...
      (ls -rt1 ../ucoam_${PRIOR_INF_OFNAME}.* | tail -n 1 >! latestfile) > & /dev/null
      ...
      if ( $nfiles > 0 ) then
         ...
         echo "ERROR: expected something like ../ucoam_${PRIOR_INF_OFNAME}.YYYY-MM-DD-SSSSS"
         exit -4
      endif
...
ex_end
      ...
      # Look for the output from the previous assimilation
      (ls -rt1 ../ucoam_${POSTE_INF_OFNAME}.* | tail -n 1 >! latestfile) > & /dev/null
      set nfiles = `cat latestfile | wc -l`
      ...
      if ( $nfiles > 0 ) then
         ...
         echo "ERROR: expected something like ../ucoam_${POSTE_INF_OFNAME}.YYYY-MM-DD-SSSSS"
         exit -5
      endif
   endif
else
...
#=========================================================================
# Block 4: Convert N ucoam restart files to DART initial condition files.
# ucoam_to_dart is serial code, we can do all of these at the same time
...
# &dart_to_ucoam_nml:      dart_to_ucoam_input_file  = 'dart_restart',
...
while ( ${member} <= ${ensemble_size} )
   ...
   $REMOVE output.${member}.ucoam_to_dart

   set OCN_RESTART_FILENAME = `printf ${CASE}.ucoam_%04d.r.${OCN_DATE_EXT}.nc  ${member}`
   set     OCN_NML_FILENAME = `printf ucoam2_in_%04d        ${member}`
   ...
   echo "starting ucoam_to_dart for member ${member} at "`date`
   ${EXEROOT}/ucoam_to_dart >! output.${member}.ucoam_to_dart &
...
set nsuccess = `fgrep 'Finished ... at YYYY' member*/output.[0-9]*.ucoam_to_dart | wc -l`
...
echo "`date` -- END ucoam-TO-DART for all ${ensemble_size} members."
...
${MOVE} Prior_Diag.nc      ../ucoam_Prior_Diag.${OCN_DATE_EXT}.nc
${MOVE} Posterior_Diag.nc  ../ucoam_Posterior_Diag.${OCN_DATE_EXT}.nc
${MOVE} obs_seq.final      ../ucoam_obs_seq.${OCN_DATE_EXT}.final
...
foreach FILE ( ${PRIOR_INF_OFNAME} ${POSTE_INF_OFNAME} ${PRIOR_INF_DIAG} ${POSTE_INF_DIAG} )
   if ( -e ${FILE} ) then
      ${MOVE} ${FILE} ../ucoam_${FILE}.${OCN_DATE_EXT}
   else
...
#=========================================================================
# Block 6: Update the ucoam restart files ... simultaneously ...
#
...
echo "`date` -- BEGIN DART-TO-ucoam"
set member = 1
while ( $member <= $ensemble_size )

   cd member_${member}

   ${REMOVE} output.${member}.dart_to_ucoam

   echo "starting dart_to_ucoam for member ${member} at "`date`
   ${EXEROOT}/dart_to_ucoam >! output.${member}.dart_to_ucoam &
...
set nsuccess = `fgrep 'Finished ... at YYYY' member*/output.[0-9]*.dart_to_ucoam | wc -l`
if (${nsuccess} != ${ensemble_size}) then
   echo "ERROR ... DART died in 'dart_to_ucoam' ... ERROR"
   echo "ERROR ... DART died in 'dart_to_ucoam' ... ERROR"
   exit -8
endif

echo "`date` -- END DART-TO-ucoam for all ${ensemble_size} members."

#-------------------------------------------------------------------------
# Cleanup
#-------------------------------------------------------------------------

echo "`date` -- END ucoam_ASSIMILATE"
