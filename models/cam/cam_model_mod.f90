...
!>  can be further restricted using cam_model_nml namelist variables.
!> 
...
public ::                                                   &
   ...
   write_cam_times, state_vector_to_model, model_to_state_vector
...
if (mismatch_size) then
   write(string1,*) 'Mismatch between state_num_#d and state_names_#d in cam_model_nml'
   call error_handler(E_ERR,'verify_namelist',string1,source,revision,revdate)
endif

if (mismatch_which) then
   write(string1,*) 'Mismatch between state_num_#d and which_vert_#d in cam_model_nml'
   call error_handler(E_ERR,'verify_namelist',string1,source,revision,revdate)
endif
...
!            cycle Alldim3
   ...
   if (s_dim_3d(1,i) == 0 .or. s_dim_3d(2,i) == 0 .or. s_dim_3d(3,i) == 0) then
      ...
      string2 = 'This may mean your input.nml cam_model_nml is configured for FV'
...
subroutine model_interpolate(st_vec, location, obs_kind, interp_val, istatus)
...
subroutine model_to_state_vector(filename, state_vector, model_time)
...
call read_cam_init(filename, var, model_time)

! now convert from var to state vector
call prog_var_to_vector(var, state_vector)

! and release var struct memory
call end_model_instance(var)

end subroutine model_to_state_vector

!-----------------------------------------------------------------------

subroutine state_vector_to_model(state_vector, filename, state_time)
...
call vector_to_prog_var (state_vector, var)

! write fields to the netCDF initial file.
call write_cam_init(filename, state_time, var)

! and release var struct memory
call end_model_instance(var)

end subroutine state_vector_to_model
...
!> It is controlled by cam_model_nml namelist variables.
