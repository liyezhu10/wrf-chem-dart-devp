...
cd ${CASEROOT}
setenv ensemble_size  `./xmlquery NINST_CPL   -value`
setenv cplr_prefix    `./xmlquery NINST_CPL_PREFIX   -value`
...
# Update cycle_count file for the next iteration, since we're done with $cycle.
@ cycle++
echo $cycle >! cycle_count
...
#=========================================================================
# Block 6: Convert N CAM initial files to DART initial condition files.
...
# 
#=========================================================================
# Block 8: Update the cam restart files ... simultaneously ...
#
# Each member will do its job in its own directory, which already exists
# and has the required input files remaining from 'Block 6'
#=========================================================================
...
#=========================================================================
# Block 9: The cam files have now been updated, move them into position.
#
...
while ( ${member} <= ${ensemble_size} )
   ...
   # multi_cplr; cd into each run_cplr#   and out again
   set cplr = `printf %s_%04d ${cplr_prefix} ${member}`
   cd $cplr
   ${LINK} ${CASE}.cam.i.${ATM_DATE_EXT}.nc cam_initial.nc || exit -9
   cd ..
