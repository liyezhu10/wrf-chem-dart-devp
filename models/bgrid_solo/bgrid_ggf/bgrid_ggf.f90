...
integer           :: ens_size         = 32
integer           :: ens_copy         = 36
integer           :: dimtime_start    = 168   ! start from 1 instead of 0 as ncview
integer           :: dimtime_length   = 200
!integer           :: lat_s            = 96+33
!integer           :: lat_e            = 96+75
! integer           :: lev_s            = 7
! integer           :: lev_e            = 7
...
real(r8)         :: sumbeta1, sumbeta2, elfsumbeta1, elfsumbeta2
...
!print *, 'finding the variable size'
!print *, 'ndims = ', ndims
!print *, 'var_size = ', bgrid%var_size(1:ndims,ind)
!print *, 'ncstart = ', ncstart(1:ndims)
!print *, 'nccount = ', nccount(1:ndims)
!pause
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,igs,1,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,igs+33,200,ind)
!pause
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,igs+33,200,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,igs,1,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,igs+33,200,ind)
!pause
...
!      ncdims_true(i) = ncdims(i)-1
!      ncstart_true(1:ncdims_true(i)-1,i) = ncstart(1:ncdims_true(i)-1,i)
!      ncstart_true(ncdims_true(i),i) = ncstart(ncdims_true(i)+1,i)
!      nccount_true(1:ncdims_true(i)-1,i) = nccount(1:ncdims_true(i)-1,i)
!      nccount_true(ncdims_true(i),i) = nccount(ncdims_true(i)+1,i)
!   enddo 
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,bgrid%ncopy,1,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,bgrid%ncopy,200,ind)
!pause
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,bgrid%ncopy,200,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,bgrid%ncopy,1,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,bgrid%ncopy,200,ind)
!pause
...
!  ndims for each level give a obs and a variable, thus 16 = 1 (ps) + 3 (t,u,v) * 5
   ...
   allocate(localization%beta1(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%beta2(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%meangf(locnum,ndims,bgrid%number_of_state_variables))

   allocate(localization%elfbeta1(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%elfbeta2(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%elfmeangf(locnum,ndims,bgrid%number_of_state_variables))

   allocate(localization%num2(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumx(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumy(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumx2(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumy2(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%numerator(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%denominator(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%alpha1(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%alpha2(locnum,group_size,ndims,bgrid%number_of_state_variables))
   allocate(localization%correlation(locnum,group_size,ndims,bgrid%number_of_state_variables))
...
!if ( itime == 1 .or. itime == dimtime_length ) then
!print *, 'itime = ', itime
...
!endif
                        ...
                        localization%meangf(ilocind,ndims,inddim) =                 &
                            localization%meangf(ilocind,ndims,inddim) + (sumbeta1/sumbeta2-1.0_r8)/(group_size-1.0_r8)

!!if ( itime == 1 .or. itime == dimtime_length ) then
!print *, 'reg_coef = ', reg_coef(:)
!print *, 'beta = ', sumbeta1, sumbeta2, (sumbeta1/sumbeta2-1.0_r8)/(group_size-1.0_r8)
!print *, 'loc% = ', localization%beta1(ilocind,ndims,inddim), localization%beta2(ilocind,ndims,inddim), localization%meangf(ilocind,ndims,inddim)
!pause
!!endif
                        ...
                        if ( indk1 > 0 ) then
                           ...
                           localization%elfbeta1(ilocind,ndims,inddim) =                  &
                               localization%elfbeta1(ilocind,ndims,inddim) + elfsumbeta1
                           localization%elfbeta2(ilocind,ndims,inddim) =                  &
                               localization%elfbeta2(ilocind,ndims,inddim) + elfsumbeta2

                           localization%elfmeangf(ilocind,ndims,inddim) =                 &
                               localization%elfmeangf(ilocind,ndims,inddim) + (elfsumbeta1/elfsumbeta2-1.0_r8)/(group_size-1.0_r8)

                           do ig = 1, group_size
                           localization%num2(ilocind,ig,ndims,inddim) =                            &
                               localization%num2(ilocind,ig,ndims,inddim) + 1
...
!print *, 'var_name_tmp = ', var_name_tmp
            ...
            write(iunit, FMT=locreal_format) localization%meangf(1:writelenloc,ndims,inddim)

            write(iunit, FMT=locreal_format) localization%elfbeta1(1:writelenloc,ndims,inddim)
            write(iunit, FMT=locreal_format) localization%elfbeta2(1:writelenloc,ndims,inddim)
            write(iunit, FMT=locreal_format) localization%elfmeangf(1:writelenloc,ndims,inddim)

            if ( ifELF == 1 ) then
            do ig = 1, group_size
               numtmp(1:writelenloc) = localization%num2(1:writelenloc,ig,ndims,inddim)
               write(iunit, FMT=locreal_format) numtmp
