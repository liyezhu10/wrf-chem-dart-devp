...
!-------------------------------------------------------
   ...
   bgrid%ncopy = 1 + ens_size + 2

   var_name_tmp = 'time'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid time')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%ntime),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

   var_name_tmp = 'TmpJ'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid TmpJ')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%nlat),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

   var_name_tmp = 'VelJ'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid VelJ')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%nslat),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

   var_name_tmp = 'TmpI'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid TmpI')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%nlon),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

   var_name_tmp = 'VelI'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid VelI')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%nslon),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

   var_name_tmp = 'lev'
   call nc_check( nf90_inq_dimid(ncidtruth, trim(var_name_tmp), var_id),       &
                  'read_bgrid_dimensions','inq_dimid nlev')
   call nc_check( nf90_inquire_dimension(ncidtruth, var_id, name, bgrid%nlev),  &
                  'read_bgrid_dimensions','inquire_dimension'//trim(name))

print *, 'dim = ', bgrid%ntime, bgrid%nlat, bgrid%nlon, bgrid%nslat, bgrid%nslon, bgrid%nlev
!pause
...
! 1. 1D array
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, bgrid%lat),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, bgrid%slat),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, bgrid%lon),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, bgrid%slon),  &
   ...
   call nc_check( nf90_get_var(ncidtruth, var_id, bgrid%lev),  &
...
!-------------------------------------------------------
   ...
   allocate(bgrid%var_size(3,bgrid%number_of_state_variables))
   allocate(bgrid%variables(bgrid%nlon,bgrid%nlat,bgrid%nlev,bgrid%ncopy,    &
                            bgrid%number_of_state_variables))
...
print *, 'ndims = ', ndims
print *, 'var_size = ', bgrid%var_size(:,ind)
!pause
      ...
      if ( ndims < 5 ) then
         ...
         ! Get 2D variable
         allocate(bgrid_var_2d(1:bgrid%var_size(1,ind),                   &
                               1:bgrid%var_size(2,ind)))
         call nc_check( nf90_get_var(ncidtruth, var_id, bgrid_var_2d),    &
         ...
         bgrid%variables(1:bgrid%var_size(1,ind),                         &
                         ...
                         1,                                               &
                         ind) =                                           &
                    bgrid_var_2d(1:bgrid%var_size(1,ind),                 &
                                 1:bgrid%var_size(2,ind))
         deallocate(bgrid_var_2d)
...
!print *, 'var = ', bgrid%variables(10,1:10,1,1,ind)
!pause
!if ( bgrid%var_size(3,ind) > 1 ) then
!print *, 'var = ', bgrid%variables(10,1:10,2,1,ind)
!pause
!endif
!print *, 'lat = ', bgrid%var_lat(:,ind)
!pause
!print *, 'lon = ', bgrid%var_lon(:,ind)
!pause
   ...
   do ind = 1,bgrid%number_of_state_variables
      ...
      if ( bgrid%var_size(3,ind) == 1 ) then
         ...
         allocate(bgrid_var_3d(bgrid%var_size(1,ind),                    &
                               bgrid%var_size(2,ind),                    &
                               bgrid%ncopy-1))
         call nc_check( nf90_get_var(ncidprior, var_id, bgrid_var_3d),   &
                     'read_in_bgrid_var','get_var '//var_name_tmp )
         bgrid%variables(1:bgrid%var_size(1,ind),                        &
                         1:bgrid%var_size(2,ind),                        &
                         1,                                              &
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,2,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,35,ind)
!pause
         ...
         allocate(bgrid_var_4d(bgrid%var_size(1,ind),                    &
                               ...
                               bgrid%var_size(3,ind),                    &
                               bgrid%ncopy-1))
         call nc_check( nf90_get_var(ncidprior, var_id, bgrid_var_4d),   &
                        'read_in_bgrid_var','get_var '//var_name_tmp )
         bgrid%variables(1:bgrid%var_size(1,ind),                       &
                         1:bgrid%var_size(2,ind),                       &
                         1:bgrid%var_size(3,ind),                       &
...
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),1,35,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,2,ind)
!pause
!print *, 'var = ', bgrid%variables(10,1:bgrid%var_size(2,ind),5,35,ind)
!pause
...
!print *, 'dimension ', bgrid%nlat, bgrid%nslat, bgrid%nlon, bgrid%nslon, bgrid%nlev, bgrid%nilev
!pause
!print *, 'lat = ', bgrid%lat
!pause
!print *, 'slat = ', bgrid%slat
!pause
!print *, 'lon = ', bgrid%lon
!pause
!print *, 'slon = ', bgrid%slon
!pause
!print *, 'lev = ', bgrid%lev
!pause
!print *, 'ilev = ', bgrid%ilev
...
!  ndims for each level give a obs and a variable, thus 16 = 1 (ps) + 3 (t,u,v) * 5
   ...
   ndims = numobslev * 16
   allocate(localization%num(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumx(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumy(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumx2(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%sumy2(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%numerator(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%denominator(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%alpha1(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%alpha2(locnum,ndims,bgrid%number_of_state_variables))
   allocate(localization%correlation(locnum,ndims,bgrid%number_of_state_variables))
   localization%num          = 0
   localization%sumx         = 0.0
   localization%sumy         = 0.0
   localization%sumx2        = 0.0
   localization%sumy2        = 0.0
   localization%numerator    = 0.0
   localization%denominator  = 0.0
   localization%alpha1       = 0.0
   localization%alpha2       = 0.0
   localization%correlation  = 0.0


!$OMP PARALLEL DO PRIVATE(ind,i,j,k,ii,jj,kk,obslevk,varlevk,inddim,ndims,indk1,indk2,iunit,writelenloc,ref_P,surf_P,var_name_tmp,obslon,obslat,varlon,varlat,var_lat_s,var_lat_e,var_lon_s,var_lon_e,numvarlev,obsi,obsj,obsk,obspres,obshgt,is_lev0,yobs,ytrue,yfmean,Robs,yvar,yprior_var,ypost_var,corr,reg_coef,ccc,ddd,dist,yinno_norm,xtrue,xfmean,xprior_var,xt_xfmean,covxy,deltaxmean,ilocind,sublocx,sublocy,sublocx2,sublocy2,sublocnum,sublocnumer,sublocalpha,sublocdenom,bgrid_var_1d,qv_1d,xprior,yprior,xtrueprf,xfmeanprf,xfspdprf,xpriorprf,xtrue3d,xfmean3d,xfspd3d,xprior3d)

!   LoopVarTypes: do inddim = 1, bgrid%number_of_state_variables
   LoopVarTypes: do inddim = indvar_s, indvar_e
      print *, 'processing variable ', trim(bgrid%description(inddim))
      ...
      do obslevk = 1, numobslev
         ...
         do j = obs_lat_s, obs_lat_e 
            do i = obs_lon_s, obs_lon_e
               ...
               ytrue      = bgrid%variables(i,j,k,1,idobs)
               yfmean     = bgrid%variables(i,j,k,2,idobs)
               yprior_var = bgrid%variables(i,j,k,3,idobs)**2
               Robs       = obs%obsvar(idobs)
               ypost_var  = 1.0_r8/(1.0_r8/yprior_var + 1.0_r8/Robs)
!              allocate(yprior(ens_size))
               yprior(1:ens_size) = bgrid%variables(i,j,k,3+1:3+ens_size,idobs)

!print *, 'obs info = ', ytrue, yfmean, yprior_var
!print *, 'obs prior = ', yprior(1:ens_size:10)

               ! get [lon, lat] and convert to radiance
               obslon = bgrid%var_lon(i,idobs) * DEG2RAD
               obslat = bgrid%var_lat(j,idobs) * DEG2RAD

!print *, 'y loc = ', bgrid%var_lon(i,idobs), bgrid%var_lat(j,idobs), obslon, obslat
!pause

               ! loop target variable grid points
!               do kk = lev_s, lev_e
               do varlevk = 1, numvarlev
                  kk = varlevk
                  if ( idobs == 1 ) then
                     if ( inddim == 1 ) then
                          ndims = 1
                     else
                          ndims = 1+(inddim-2)*5+kk
                     endif
                  else
                     if ( inddim == 1 ) then
                          ndims = (k-1)*16+1
                     else
                          ndims = (k-1)*16+1+(inddim-2)*5+kk
                     endif
                  endif
!print *, numobslev, numvarlev
!print *, 'ndims = ', ndims
!pause
                  do jj = var_lat_s, var_lat_e 
                     ...
                     xtrue      = bgrid%variables(ii,jj,kk,1,inddim)
                     xfmean     = bgrid%variables(ii,jj,kk,2,inddim)
                     xprior_var = bgrid%variables(ii,jj,kk,3,inddim)**2
                     allocate(xprior(ens_size))
                     xprior(1:ens_size) = bgrid%variables(ii,jj,kk,3+1:3+ens_size,inddim)
...
!print *, 'var loc = ', bgrid%var_lon(ii,inddim), bgrid%var_lat(jj,inddim), varlon, varlat
!pause
         ...
         do varlevk = 1, numvarlev
            ...
            if ( idobs == 1 ) then
               if ( inddim == 1 ) then
                  ...
                  ndims = 1+(inddim-2)*5+varlevk
               endif
            else
               if ( inddim == 1 ) then
                  ndims = (obslevk-1)*16+1
               else
                  ndims = (obslevk-1)*16+1+(inddim-2)*5+varlevk
               endif
            endif

            if ( obslevk < 10 ) then
               write(idchar1,'(i1)') obslevk
            else
               write(idchar1,'(i2)') obslevk
            endif
            if ( varlevk < 10 ) then
               write(idchar2,'(i1)') varlevk
            else
               write(idchar2,'(i2)') varlevk
            endif
