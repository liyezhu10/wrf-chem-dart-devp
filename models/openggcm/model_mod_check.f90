...
use     dart_openggcm_mod, only : get_openggcm_restart_filename
...
character(len=129) :: openggcm_input_file  ! set with get_model_analysis_filename() if needed
...
if (test1thru > 2) then
   ...
   call get_openggcm_restart_filename(openggcm_input_file)

   write(*,*)'Reading file from  '//trim(openggcm_input_file)
   write(*,*)'Writing data into '//trim(output_file)

   call restart_file_to_sv(openggcm_input_file, statevector, model_time)
...
if (test1thru > 3) then
   ...
   if ( .not. file_exist(dart_input_file) ) then
      ...
      write(string2,*) "Run 'openggcm_to_dart' to generate ",trim(dart_input_file)
