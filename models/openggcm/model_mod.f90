...
! standard openggcm namelist and filled in here.
...
! lat lon boxes a value of 30 is sufficient for the gx3 openggcm grid, 80 for the 
...
! Initializes data structures needed for openggcm interpolation for
...
if(is_pole) then
   ...
   ! This is specific to the dipole openggcm grids that do not go to the south pole
...
! Three different types of grids are used here. The openggcm dipole 
...
! spacing in latitude used for some openggcm applications and testing
...
! write_grid_interptest is only possible after reading a real openggcm grid,
! so static_init_model() must be called to gather the real openggcm grid.
