...
!
      ...
      qzg=dsin(xxp)
      tmpx=tran%pmat(1,1,-1)*qxg+tran%pmat(1,2,-1)*qyg + tran%pmat(1,3,-1)*qzg
      tmpy=tran%pmat(2,1,-1)*qxg+tran%pmat(2,2,-1)*qyg + tran%pmat(2,3,-1)*qzg
      tmpz=tran%pmat(3,1,-1)*qxg+tran%pmat(3,2,-1)*qyg + tran%pmat(3,3,-1)*qzg
      qx  =tran%pmat(1,1,2)*tmpx+tran%pmat(1,2,2)*tmpy + tran%pmat(1,3,2)*tmpz
      qy  =tran%pmat(2,1,2)*tmpx+tran%pmat(2,2,2)*tmpy + tran%pmat(2,3,2)*tmpz
      qz  =tran%pmat(3,1,2)*tmpx+tran%pmat(3,2,2)*tmpy + tran%pmat(3,3,2)*tmpz
      psi=datan2(qy,qz)
      do i=1,3
         do j=1,3
...
!doc-  
!doc-FUNCTION:
!doc-  convert MHD logitude (-180,180) and colatitude (0,180)
!doc-  to magnetic local time (0-24) and colatitude
!doc-
!doc-PARAMETERS:
!doc-  iy,mo,id,ih,mi,se (in):  time in UT
!doc-  pmhd (in,real):  deg longitude in MHD system (-180.0,180.0)
!doc-  tmhd (in,real):  deg colatitude in MHD system (0.0,180.0)
!doc-  xmlt (out,real): hours magnetic local time (0.0-24.0)
!doc-  colat (out,real): deg magnetic colatitude (0.0,180.0)
!doc-
      implicit none
...
!doc-  
!doc-FUNCTION:
!doc-  compute dipole tilt angles for a given time
!doc-
!doc-PARAMETERS:
!doc-  iy,mo,id,ih,mi,se (in):  time in UT
!doc-  degsun (out,real): deg tilt in GSE x-z plane
!doc-  degy (out,real):   deg tilt in GSE y-z plane
!doc-
